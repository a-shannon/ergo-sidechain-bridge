...
$SpecRelativePath =
  'validity-proof/consumer-jvm/BridgeValidityApplicationPooledReserveCompilerSpec.scala'
$LockRelativePath =
  'validity-proof/consumer-jvm/validity-application-pooled-reserve-compiler-lock-v1.json'
$SpecDestination =
  'sc/shared/src/test/scala/sigma/bridge/BridgeValidityApplicationPooledReserveCompilerSpec.scala'
...
if (
  ...
  $lock.schema -ne 'e2s.validity-application-pooled-reserve-compiler-lock.v1' -or
...
$temporaryWorktree = Join-Path `
  ([IO.Path]::GetTempPath()) `
  ("bridge-pooled-reserve-compiler-" + [Guid]::NewGuid().ToString('N'))
$worktreeAdded = $false
    ...
    try {
      $compilerOutput = @(
        & $javaExe `
          ...
          "-Dbridge.validity.application.pooled.reserve.root=$bridge" `
          "-Dbridge.eip0045.validity.application.pooled.reserve.identity.out=$output" `
          '-jar' `
          $sbtLauncher `
          'scJVM/Test/testOnly sigma.bridge.BridgeValidityApplicationPooledReserveCompilerSpec' `
          2>&1 |
          ForEach-Object { $_.ToString() }
      )
