...
$ReceiptSchema = 'e2s.validity-application-pooled-reserve-compiler-batch.v1'
$SpecRelativePath =
  'validity-proof/consumer-jvm/BridgeValidityApplicationPooledReserveBurnFamilyV6CompilerSpec.scala'
$LockRelativePath =
  'validity-proof/consumer-jvm/validity-application-pooled-reserve-burn-family-compiler-lock-v6.json'
$SpecDestination =
  'sc/shared/src/test/scala/sigma/bridge/BridgeValidityApplicationPooledReserveBurnFamilyV6CompilerSpec.scala'
...
if (
  ...
  $lock.schema -ne
    'e2s.validity-application-pooled-reserve-burn-family-compiler-lock.v6' -or
...
$status = @(& git -C $sigma status --porcelain=v1 --untracked-files=no)
if ($LASTEXITCODE -ne 0 -or $status.Count -ne 0) {
  throw 'SigmaState checkout must have no tracked drift before guarded compilation'
}
Assert-Hash `
...
$temporaryWorktree = Join-Path `
  ([IO.Path]::GetTempPath()) `
  ("bridge-pooled-reserve-burn-family-v6-compiler-" +
...
try {
    ...
    try {
      $compilerOutput = @(
        & $javaExe `
          ...
          "-Dbridge.validity.application.pooled.reserve.v6.root=$bridge" `
          "-Dbridge.eip0045.validity.application.pooled.reserve.v6.identity.out=$output" `
          '-jar' `
          $sbtLauncher `
          'scJVM/Test/testOnly sigma.bridge.BridgeValidityApplicationPooledReserveBurnFamilyV6CompilerSpec' `
          2>&1 |
          ForEach-Object { $_.ToString() }
      )
  ...
  for ($index = 0; $index -lt $expectedRoles.Count; $index++) {
    ...
    if (
      $contract.schema -ne
        'e2s.validity-application-pooled-reserve-compiler-receipt.v1' -or
  ...
  if ($worktreeAdded) {
    ...
    try {
      ...
      if (Test-Path -LiteralPath $temporaryWorktree) {
        [IO.Directory]::Delete('\\?\' + $temporaryWorktree, $true)
      }
    ...
    if (
      $pruneExitCode -ne 0 -or
      (Test-Path -LiteralPath $temporaryWorktree)
    ) {
