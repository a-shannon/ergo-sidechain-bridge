...
$SpecRelativePath =
  'validity-proof/consumer-jvm/BridgeValidityApplicationPooledReserveBurnFamilyV5CompilerSpec.scala'
$LockRelativePath =
  'validity-proof/consumer-jvm/validity-application-pooled-reserve-burn-family-compiler-lock-v5.json'
$SpecDestination =
  'sc/shared/src/test/scala/sigma/bridge/BridgeValidityApplicationPooledReserveBurnFamilyV5CompilerSpec.scala'
...
if (
  ...
  $lock.schema -ne
    'e2s.validity-application-pooled-reserve-burn-family-compiler-lock.v5' -or
...
$temporaryWorktree = Join-Path `
  ([IO.Path]::GetTempPath()) `
  ("bridge-pooled-reserve-burn-family-v5-compiler-" +
    ...
    try {
      $compilerOutput = @(
        & $javaExe `
          ...
          "-Dbridge.validity.application.pooled.reserve.v5.root=$bridge" `
          "-Dbridge.eip0045.validity.application.pooled.reserve.v5.identity.out=$output" `
          '-jar' `
          $sbtLauncher `
          'scJVM/Test/testOnly sigma.bridge.BridgeValidityApplicationPooledReserveBurnFamilyV5CompilerSpec' `
          2>&1 |
          ForEach-Object { $_.ToString() }
      )
