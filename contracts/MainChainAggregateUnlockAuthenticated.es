{
  ...
  //
  // Single-burn payout path for SPVTrackerAuthenticated. The tracker remains
  // read-only, while replay prevention and the liquidity vault are consumed
  // atomically. This verifies the authenticated V2 tracker key/value, burn
  // inclusion, payout fields, the proof-bound 264-byte tracker value, and the
  // exact DUP successor. It does not verify GRANDPA: sidechain-finality
  // semantics remain an admission-time authority boundary until Ergo verifies
  // a finality proof or equivalent attestation.
  //
  // Transaction shape:
  //   DATA_INPUTS(0): SPVTrackerAuthenticated singleton
  //   INPUTS(0):      DoubleUnlockPreventionAuthenticated singleton
