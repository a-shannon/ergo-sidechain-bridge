{
  // SPVTrackerPooledReserveBurnSettlementV6 - target-lineage consumer for the
  // retained pooled-reserve burn statement V5.
  //
  ...
  //
  // Registers:
  //   R4: Coll[Byte] - exact pooled-reserve V6 settlement-lineage profile ID
  ...
  //   Var(1): Coll[Byte]       pooled-reserve burn public inputs V5 (981 bytes)
  ...
  val expectedProgramId =
    fromBase16("POOLED_RESERVE_BURN_V5_PROGRAM_ID_PLACEHOLDER")
  val expectedVerifierProfileId =
    fromBase16("POOLED_RESERVE_BURN_V5_VERIFIER_PROFILE_ID_PLACEHOLDER")
  // First 450 bytes of PooledReserveBurnApplicationBindingV5: exact runtime
  // profile, profile ID, source runtime-code identity, and tracker NFT.
  val expectedApplicationBindingPrefix =
    fromBase16("POOLED_RESERVE_BURN_V5_APPLICATION_BINDING_PREFIX_PLACEHOLDER")
  val pooledReserveProfileId =
    fromBase16("POOLED_RESERVE_PROFILE_ID_PLACEHOLDER")

  val payloadDomain =
    fromBase16("4532535f504f4f4c45445f524553455256455f4255524e5f5055424c49435f494e505554535f5635")
  val applicationBindingDomain =
    fromBase16("4532535f504f4f4c45445f524553455256455f4255524e5f4150504c49434154494f4e5f42494e44494e475f5635")
  ...
  val trackerKeyDomain =
    fromBase16("4532535f5350565f56414c49444954595f4150504c49434154494f4e5f4b45595f5635")
  val trackerValueDomain =
    fromBase16("4532535f5350565f56414c49444954595f4150504c49434154494f4e5f56414c55455f563500")
  val payloadDigestDomain =
    fromBase16("4532535f5350565f56414c49444954595f4150504c49434154494f4e5f5041594c4f41445f4449474553545f5635")
  val zero32 =
  ...
  val payloadShapeOk = payload.size == 981
  ...
  val applicationBinding =
    if (payloadShapeOk) payload.slice(45, 531) else Coll[Byte]()
  val suppliedApplicationBindingDigest =
    if (payloadShapeOk) payload.slice(531, 563) else zero32
  val checkpoint =
    if (payloadShapeOk) payload.slice(563, 779) else Coll[Byte]()
  val checkpointCommitment =
    if (payloadShapeOk) payload.slice(779, 811) else zero32
  val targetNativeStateRoot =
    if (payloadShapeOk) payload.slice(811, 843) else zero32
  val suppliedTrustAnchorDigest =
    if (payloadShapeOk) payload.slice(843, 875) else zero32
  val finalityHorizonHeightBytes =
    if (payloadShapeOk) payload.slice(875, 883) else zero8
  val finalityHorizonHeight = byteArrayToLong(finalityHorizonHeightBytes)
  val finalityHorizonHash =
    if (payloadShapeOk) payload.slice(883, 915) else zero32
  val suppliedExtensionKey =
    if (payloadShapeOk) payload.slice(915, 917) else Coll[Byte]()
  val suppliedExtensionValue =
    if (payloadShapeOk) payload.slice(917, 981) else Coll[Byte]()

  val runtimeProfile =
    if (applicationBinding.size == 486)
      applicationBinding.slice(1, 350)
    else Coll[Byte]()
  val suppliedRuntimeProfileId =
    if (applicationBinding.size == 486)
      applicationBinding.slice(350, 382)
    else zero32
  val suppliedTrackerNftId =
    if (applicationBinding.size == 486)
      applicationBinding.slice(418, 450)
    else zero32
  val suppliedTrackerContractId =
    if (applicationBinding.size == 486)
      applicationBinding.slice(450, 482)
    else zero32
  val bindingControlBytes =
    if (applicationBinding.size == 486)
      applicationBinding.slice(482, 486)
    else Coll[Byte]()
  val sourceRuntimeLineageProfileId =
  ...
  val applicationBindingOk =
    applicationBinding.size == 486 &&
    applicationBinding.slice(0, 450) == expectedApplicationBindingPrefix &&
  ...
  val trackerValue =
    trackerValueDomain ++
    Coll(5.toByte, 1.toByte, 1.toByte, 0.toByte) ++
    bridgeEventRoot ++
  ...
  val immutableStateOk =
    sourceRuntimeLineageProfileId != zero32 &&
  ...
  val successorOk =
    ...
    successor.R4[Coll[Byte]].get == pooledReserveProfileId &&
