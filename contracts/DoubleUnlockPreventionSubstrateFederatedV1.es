{
  ...
  // Substrate federated V1 settlement family. It validates the exact
  // application-bound FED-2 tracker value, the tracker-included 205-byte burn
  // leaf, Merkle inclusion, payout bindings, reserve conservation transition
  // and one absent-to-present burn-ID insertion. Source attestations remain an
  // off-chain precondition; the tracker contract's SigmaProp threshold is the
  // on-chain checkpoint-admission authority. This predicate is not trustless.
  //
  // Registers:
  //   R4: Coll[Byte] - exact federated settlement-family ID
  //   R5: AvlTree    - spent burn IDs (key=32, value=1)
  //
  // Transaction:
  //   DATA_INPUTS(0): exact Substrate federated V1 tracker singleton
  ...
  //   Var(2): Coll[Byte] tracker-included 205-byte burn leaf V1
  //   Var(3): Coll[Byte] burn/DUP proof bundle V2

  val trackerNftId =
    fromBase16("FEDERATED_SETTLEMENT_TRACKER_NFT_ID_PLACEHOLDER")
  val dupNftId =
    fromBase16("FEDERATED_SETTLEMENT_DUP_NFT_ID_PLACEHOLDER")
  val pooledReserveNftId =
    fromBase16("FEDERATED_SETTLEMENT_RESERVE_NFT_ID_PLACEHOLDER")
  val settlementFamilyId =
    fromBase16("FEDERATED_SETTLEMENT_FAMILY_ID_PLACEHOLDER")
  val trackerContractId =
    fromBase16("FEDERATED_SETTLEMENT_TRACKER_CONTRACT_ID_PLACEHOLDER")
  val sourceNetworkIdExpected =
    fromBase16("FEDERATED_SETTLEMENT_SOURCE_NETWORK_ID_PLACEHOLDER")
  val sidechainIdExpected =
    fromBase16("FEDERATED_SETTLEMENT_SIDECHAIN_ID_PLACEHOLDER")
  val runtimeProfileIdExpected =
    fromBase16("FEDERATED_SETTLEMENT_RUNTIME_PROFILE_ID_PLACEHOLDER")
  val settlementProfileIdExpected =
    fromBase16("FEDERATED_SETTLEMENT_PROFILE_ID_PLACEHOLDER")
  val federationProfileIdExpected =
    fromBase16("FEDERATED_SETTLEMENT_FEDERATION_PROFILE_ID_PLACEHOLDER")
  val ergoKeySetDigestExpected =
    fromBase16("FEDERATED_SETTLEMENT_ERGO_KEY_SET_DIGEST_PLACEHOLDER")
  val ergoThresholdExpected =
    fromBase16("FEDERATED_SETTLEMENT_ERGO_THRESHOLD_PLACEHOLDER")
  val federationEpochExpected =
    fromBase16("FEDERATED_SETTLEMENT_EPOCH_PLACEHOLDER")
  ...
  val trackerProfileOk =
    ...
    tracker.R4[Coll[Byte]].get == federationProfileIdExpected &&
    tracker.R6[Coll[Byte]].get == sidechainIdExpected &&
    tracker.R9[Coll[Byte]].get == ergoKeySetDigestExpected
  ...
  val trackerValueShapeOk =
    ...
    trackerValue.slice(0, 36) == trackerValueDomain &&
    trackerValue(36) == 1.toByte &&
    trackerValue(37) == 1.toByte &&
    trackerValue(38) == 1.toByte &&
    trackerValue(39) == 0.toByte
  val eventRoot =
    if (trackerValueShapeOk) trackerValue.slice(40, 72) else zero32
  val statementId =
    if (trackerValueShapeOk) trackerValue.slice(72, 104) else zero32
  val anchorHeaderId =
    if (trackerValueShapeOk) trackerValue.slice(104, 136) else zero32
  val anchorHeightBytes =
    if (trackerValueShapeOk) trackerValue.slice(136, 140) else zero4
  val sourceHeightBytes =
    if (trackerValueShapeOk) trackerValue.slice(140, 148) else zero8
  val sourceHeight = byteArrayToLong(sourceHeightBytes)
  val sourceBlockHash =
    if (trackerValueShapeOk) trackerValue.slice(148, 180) else zero32
  val trackerExecutionBlockHash =
    if (trackerValueShapeOk) trackerValue.slice(180, 212) else zero32
  val trackerBurnCountBytes =
    if (trackerValueShapeOk) trackerValue.slice(212, 216) else zero4
  val valueRuntimeProfileId =
    if (trackerValueShapeOk) trackerValue.slice(216, 248) else zero32
  val valueSettlementProfileId =
    if (trackerValueShapeOk) trackerValue.slice(248, 280) else zero32
  val valueFederationProfileId =
    if (trackerValueShapeOk) trackerValue.slice(280, 312) else zero32
  val valueErgoKeySetDigest =
    if (trackerValueShapeOk) trackerValue.slice(312, 344) else zero32
  val valueErgoThreshold =
    if (trackerValueShapeOk) trackerValue.slice(344, 346) else Coll[Byte]()
  val valueFederationEpoch =
    if (trackerValueShapeOk) trackerValue.slice(346, 354) else zero8
  val validFromBytes =
    if (trackerValueShapeOk) trackerValue.slice(354, 362) else zero8
  val validFrom = byteArrayToLong(validFromBytes)
  val expiresAtBytes =
    if (trackerValueShapeOk) trackerValue.slice(362, 370) else zero8
  ...
  val trackerValueProfileOk =
    statementId != zero32 &&
    sourceHeight > 0L &&
    sourceHeight == sidechainHeight &&
    sourceBlockHash != zero32 &&
    trackerExecutionBlockHash != zero32 &&
    trackerBurnCount > 0L &&
    trackerBurnCount <= 256L &&
    trackerBurnCount == leafCount.toLong &&
    valueRuntimeProfileId == runtimeProfileIdExpected &&
    valueSettlementProfileId == settlementProfileIdExpected &&
    valueFederationProfileId == federationProfileIdExpected &&
    valueErgoKeySetDigest == ergoKeySetDigestExpected &&
    valueErgoThreshold == ergoThresholdExpected &&
    valueFederationEpoch == federationEpochExpected &&
    validFrom > 0L &&
    expiresAt > validFrom &&
  ...
  val expectedTrackerKey =
    blake2b256(
      trackerKeyDomain ++
      sourceNetworkIdExpected ++
      sidechainId ++
      sourceHeightBytes ++
      sourceBlockHash ++
      trackerExecutionBlockHash
    )
  ...
  val leafFieldsOk =
    ...
    executionBlockHash == trackerExecutionBlockHash &&
  ...
  val dupSuccessorOk =
    ...
    SELF.R4[Coll[Byte]].get == settlementFamilyId &&
    dupOut.R4[Coll[Byte]].get == settlementFamilyId &&
