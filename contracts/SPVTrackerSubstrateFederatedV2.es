{
  // SPVTrackerSubstrateFederatedV2 (inactive anchor-reference prototype)
  //
  ...
  //
  // The compiled tree is a distinct contract identity. Checkpoint statement V1
  // and tracker value V1 retain their existing semantics and domains.
  // No V1 compiler receipt or runtime activation applies to this prototype.
  //
  // Registers:
  ...
  //   2: Int        - absolute Ergo anchor height (not the V1 header index)
  ...
  val requestedAnchorHeight = getVar[Int](2).get
  ...
  // Widen before subtracting; narrow only after checking the actual window.
  val anchorOffset =
    if (CONTEXT.headers.size > 0)
      CONTEXT.headers(0).height.toLong - requestedAnchorHeight.toLong
    else -1L
  val headerIndexOk =
    requestedAnchorHeight >= 0 &&
    anchorOffset >= 0L &&
    anchorOffset < CONTEXT.headers.size.toLong
  val headerIndex = if (headerIndexOk) anchorOffset.toInt else 0
  val anchorHeader = CONTEXT.headers(headerIndex)
  val extensionLeaf =
  ...
  val extensionMembershipOk =
    headerIndexOk &&
    anchorHeader.height == requestedAnchorHeight &&
