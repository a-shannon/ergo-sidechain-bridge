...
Overlay 0005 changes the node manifest, its local lock dependency,
...
## FED Manual-Seal Inherents

The FED manual-seal service supplies only the timestamp inherent provider.
This runtime contains BaseFee, not DynamicFee. Supplying dynamic-fee data
adds a second `InherentData` entry without generating a second inherent
extrinsic. The pinned SDK manual-seal engine compares the proposal's total
extrinsic count with the data-entry count before import when
`create_empty=false`. The old two-entry provider therefore collides with a
proposal containing one timestamp and one user extrinsic.

The service and its tests share the same provider selection. Native tests
call the selected runtime's `BlockBuilder_inherent_extrinsics` API to check
entry/extrinsic parity, exact first timestamp bytes, clock progression and
gas-price independence. Reinserting only the obsolete dynamic-fee entry
reproduces the count mismatch without changing the runtime's generated bytes.
The timestamp clock and non-FED provider composition are preserved.

Run the focused native tests with the public profile below:

```powershell
cargo test --offline --locked -p frontier-template-node --no-default-features --features bridge-federated-v4-genesis-node federated_manual_seal -- --test-threads=1
```

This validates provider/runtime composition, not pool eligibility, SDK block
import, canonical reservation, mint or the complete two-way lifecycle. It does
not change the SDK's empty-pool guard or the caller's one-shot submission,
...
## Historical Selected Node And WASM Verification

The results in this section and the configuration-only startup above used
overlay 0005 SHA-256
`ff24b3ca5063371462c8257cfd9018e586824f33b0965a4a5025b41dd84c61ce`
and source tree `bdd5aeb8cceef1c487479e482e033b9eac1d1455`, before the
manual-seal provider correction. Their artifact hashes remain historical pins.
