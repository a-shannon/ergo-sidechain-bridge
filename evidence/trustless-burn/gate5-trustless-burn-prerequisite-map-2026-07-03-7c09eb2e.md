# Gate 5 Trustless Burn Prerequisite Map - 7c09eb2e
...
| Field | Value |
| --- | --- |
| Validator commit | 7c09eb2e |
| Candidate target | ../evidence/trustless-burn/gate5-trustless-burn-blocker-map-2026-06-29-5d075bd9.md |
| Validator report | ../evidence/trustless-burn/artifacts/trustless-validate-gate5-blocker-map-blocked-2026-07-03-7c09eb2e.md |
| Command | `npm run trustless:prerequisite-map -- --candidate ../evidence/trustless-burn/gate5-trustless-burn-blocker-map-2026-06-29-5d075bd9.md --validator-commit 7c09eb2e --validator-report-out <report.md> --out <map.md>` |
...
| Required Components: SPV relay contract or tracker: status must be linked before Gate 5 evidence can pass | Sanitized public SPV tracker observation JSON plus completed `npm run trustless:spv-tracker-observe` JSON report binding commitment history, finality, tracker key, value, and digest without private runtime state. |
...
| Reviewer Sign-Off: Security reviewer: notes must state a concrete trustless-burn outcome | Security reviewer approval after independent review, fallback disablement, critical/high closure, and no-broadcast boundaries are complete. |
...
| Reviewer Sign-Off: Operator reviewer: notes must state a concrete trustless-burn outcome | Operator reviewer approval after non-mainnet drill evidence, fallback behavior, recovery boundaries, and no-broadcast rules are complete. |
...
| --- | --- | --- |
| Reconfirm current Gate 5 blocker map | complete | Validator report above: BLOCKED with 22 structural issue(s). |
