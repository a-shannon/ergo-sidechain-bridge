# Completed Gate 5 Event Index Binding Evidence - 2026-06-29 - b8968c16

This artifact captures the local proof-core prerequisite for the Gate 5
`eventIndex` binding. It records the exact event index consumed by the completed
local proof-vector validation report. It does not prove finalized sidechain
event inclusion, live SPV tracker operation, on-chain proof acceptance, DUP
...
| Field | Value |
|---|---|
| Evidence name | Gate 5 event index binding evidence |
| Git commit | b8968c16 |
...
| Field | Value |
|---|---|
| Binding field | `eventIndex` |
| eventIndex | `8` |
| Source path | `reports[0].leaf.eventIndex` |
| Negative-case binding | `wrong-event-index` rejected with `burnId must equal derived sidechain event identity` |
...
| Finalized sidechain event inclusion | no |
