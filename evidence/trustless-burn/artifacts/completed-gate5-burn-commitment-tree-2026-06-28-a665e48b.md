# Completed Gate 5 Burn Commitment Tree Component Evidence - 2026-06-28 - a665e48b

This artifact records row-level local component evidence for the Gate 5 burn
commitment tree. It records only the local proof-core tree construction and
validation facts from `relayer/src/trustless-burn-proof.ts` and the completed
multi-leaf proof-vector report.

## Evidence Classification

| Field | Value |
|---|---|
| Evidence name | Gate 5 burn commitment tree component evidence |
| Git commit | a665e48b |
...
| Field | Value |
|---|---|
| Component row | Burn commitment tree |
| Required property | ErgoScript-friendly burn tree using Blake2b-compatible hashing |
| Component result | completed Gate 5 burn commitment tree component evidence |
| Leaf domain | E2S_TRUSTLESS_BURN_LEAF_V1 |
| Node domain | E2S_TRUSTLESS_BURN_NODE_V1 |
| Hash function | Blake2b-256 from `blakejs.blake2b(data, undefined, 32)` |
| Root field | `bridgeEventRootHex` |
...
| leafCount | 2 |
| proofNodeCount | 1 |
| Inclusion rule | leaf hash plus ordered proof nodes must resolve to `bridgeEventRootHex` |
