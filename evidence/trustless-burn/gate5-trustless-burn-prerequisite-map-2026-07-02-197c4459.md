# Gate 5 Trustless Burn Prerequisite Map - 2026-07-02 - 197c4459
...
| Field | Value |
|---|---|
| Validator commit | 197c4459 |
| Candidate target | ../evidence/trustless-burn/gate5-trustless-burn-blocker-map-2026-06-29-5d075bd9.md |
| Validator report | ../evidence/trustless-burn/artifacts/trustless-validate-gate5-blocker-map-blocked-2026-07-02-197c4459.md |
| Command | `npm run trustless:validate -- ../evidence/trustless-burn/gate5-trustless-burn-blocker-map-2026-06-29-5d075bd9.md --report-out ../evidence/trustless-burn/artifacts/trustless-validate-gate5-blocker-map-blocked-2026-07-02-197c4459.md` |
...
| Required Components: Ergo extension-section anchoring: status must be linked before Gate 5 evidence can pass | Mined or node-backed non-mainnet extension-section anchoring evidence binding the Gate 5 commitment under the intended `0x04xx` keyspace. For the anchor path, the operator must provide sanitized public extension-observation JSON, then run `npm run trustless:anchor-observe -- --bridge-event-root <64hex\|0401:64hex> --observations-json <sanitized-public-observations.json> --min-height <n> --max-height <n> --json-out <completed-report.json>` and link `Anchor observation report: <completed-report.json>` when the component row is moved to `linked`. |
...
| Required Components: SPV relay contract or tracker: status must be linked before Gate 5 evidence can pass | Authenticated SPV relay/tracker evidence on Ergo that binds commitment history, finality, and tracker updates without using private runtime state. For the SPV tracker path, the operator must provide sanitized public observation JSON, then run `npm run trustless:spv-tracker-observe -- --observation-json <sanitized-public-observation.json> --json-out <completed-report.json>` and link `SPV tracker observation report: <completed-report.json>` when the component row is moved to `linked`. |
...
| SPV tracker observation request prepared | yes |
