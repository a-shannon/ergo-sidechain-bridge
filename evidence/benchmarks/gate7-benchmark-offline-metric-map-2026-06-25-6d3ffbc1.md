# Gate 7 Benchmark Offline Metric Map - 2026-06-25 - 6d3ffbc1

This packet adds completed offline unsigned transaction-shape metrics for the
single-claim and batch settlement benchmark rows. It is not completed Gate 7
...
- artifact://benchmarks/artifacts/benchmark-validate-gate7-offline-metric-map-blocked-2026-06-25-6d3ffbc1.md

## Benchmark Classification

| Field | Value |
|---|---|
| Benchmark name | Gate 7 offline benchmark metric map |
| Git commit | 6d3ffbc1 |
...
|---|---|---|---|---|---|---|---|---|---|
| Single-claim settlement baseline | artifact://benchmarks/artifacts/completed-offline-shape-metrics-output-2026-06-25-6d3ffbc1.md; npm run showcase:metric-rows command output PASS exit code 0; single-claim settlement baseline; Sample count 3; Cost-relevant counts inputs=3 outputs=4 vars=15 batch=1; unsigned EIP-12 JSON transaction-shape bytes and offline build latency captured | 3 | 1.2 ms mean build time | tracker proof 137 B, DUP lookup 67 B, DUP insert 67 B | unsigned EIP-12 JSON transaction shape 2744 bytes | inputs=3 outputs=4 vars=15 batch=1 | 1 settlement per Ergo block in the offline single-claim model | 1.2 ms offline build latency | linked |
| Batch settlement | artifact://benchmarks/artifacts/completed-offline-shape-metrics-output-2026-06-25-6d3ffbc1.md; npm run showcase:metric-rows command output PASS exit code 0; batch settlement; Sample count 3; Cost-relevant counts inputs=3 outputs=13 vars=58 batch=10; unsigned EIP-12 JSON transaction-shape bytes and offline build latency captured | 3 | 1.4 ms mean build time for batch 10 | tracker proof 222 B, DUP lookup 67 B, DUP insert 70 B, claim cores 1090 B | unsigned EIP-12 JSON transaction shape 13893 bytes | inputs=3 outputs=13 vars=58 batch=10 | 10 settlements per Ergo block in the offline batch model | 1.4 ms offline build latency | linked |
...
|---|---|---|---|
| ContextExtension var count | artifact://benchmarks/artifacts/offline-showcase-benchmark-output-2026-06-25-782a2bdd.md; npm run showcase:benchmark command output PASS exit code 0; batch 10 reports Vars 58 | ContextExtension Var count is the current visible scaling limit for larger batches | Capture signed transaction-size and validation cost evidence for the ContextExtension Var count limit |
| Batch unlock claim-core size | artifact://benchmarks/artifacts/completed-offline-shape-metrics-output-2026-06-25-6d3ffbc1.md; npm run showcase:metric-rows command output PASS exit code 0; batch settlement reports claim cores 1090 B and unsigned EIP-12 JSON transaction shape 13893 bytes | Batch unlock claim-core size grows with batch settlement size | Capture signed batch unlock claim-core size and transaction byte evidence |
...
| Ergo transaction size limit | artifact://benchmarks/artifacts/completed-offline-shape-metrics-output-2026-06-25-6d3ffbc1.md; npm run showcase:metric-rows command output PASS exit code 0; single-claim settlement baseline unsigned EIP-12 JSON transaction shape 2744 bytes and batch settlement unsigned EIP-12 JSON transaction shape 13893 bytes | Ergo transaction size limit remains unmeasured for signed live-capable transactions | Capture signed Ergo transaction byte size and limit margin for single, batch, and sharded settlement shapes |
...
|---|---|---|---|---|
| Benchmark owner | A. Shannon | block | 2026-06-25 | Measured offline benchmark command outputs, single and batch transaction-shape metrics, sharded-lane evidence, ContextExtension Vars, DUP proof size, SPVTracker shared-input limit, and signed/live transaction-size gaps require live-capable evidence before Gate 7 approval |
