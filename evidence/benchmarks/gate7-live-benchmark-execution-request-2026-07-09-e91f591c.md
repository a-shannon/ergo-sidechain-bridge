...
| Source commit | e91f591c |
| Capture manifest | ../evidence/benchmarks/gate7-live-batch-capture-manifest-2026-07-09-e91f591c.md |
| Gate 7 candidate | ../evidence/benchmarks/gate7-offline-structured-candidate-2026-07-08-05f25f0e.md |
...
| Readiness operator request | ../evidence/readiness/readiness-operator-request-current-lanes-2026-07-09-675f4b1d.md |

## Operator Requests

| Phase | Operator action | Evidence to return | Stop condition |
| --- | --- | --- | --- |
| 1. Bind live-batch identity inputs | Choose one non-mainnet network, one Gate 7 candidate, one ordered burn set, one batch window, and the operator-provided read-only state/deployed-state input targets for the non-broadcast check. | Identity packet with network, candidate target, ordered burn set, batch window, sourceBindings.state target class, sourceBindings.deployedState target class, and no submit/broadcast scope. | Stop if identity inputs are generic, targetless, private runtime defaults, secret-bearing, mainnet-scoped, or grant broader signing/broadcast permission. |
| 2. Produce non-broadcast aggregate check evidence | Run the aggregate check in non-broadcast mode using operator-local state inputs and derive the Expected transaction ID before any explicit live approval is collected. | Aggregate check JSON with sanitized source bindings, /transactions/check PASS, Expected transaction ID, ordered burn set, settlement shape, and broadcast: no. | Stop if the command uses default private runtime DBs, unsanitized deployment state, missing check output, or any submit/broadcast path. |
| 3. Collect explicit live approval | After the non-broadcast aggregate check, collect explicit approval scoped to the derived Expected transaction ID, the selected non-mainnet network, candidate target, ordered burn set, and batch window. | Approval artifact with Expected transaction ID, network, candidate target, ordered burn set, batch window, approver, timestamp, expiry, and statement that no broader live operation is approved. | Stop if approval predates the aggregate check, is generic, targetless, missing the derived Expected transaction ID, not time-scoped, or grants broader signing/broadcast permission. |
| 4. Assemble and validate prebroadcast package | Assemble the completed prebroadcast package and validate it against the aggregate check and explicit approval target without sharing secrets or raw runtime state. | Completed prebroadcast Markdown, sanitized deployment-state digest, singleton inventory, explicit approval target, prebroadcast:validate PASS, and prebroadcast:doctor JSON with zero structural issues. | Stop on non-zero structural issues, target drift, placeholder targets, secret-bearing targets, or private runtime material. |
| 5. Validate readiness, policy, and signing prerequisites | Validate the prebroadcast package and provide readiness, broadcast-policy, signing, and network reconfirmation evidence bound to the same Expected transaction ID. | prebroadcast:validate PASS, prebroadcast:doctor JSON with zero structural issues, readiness PASS, broadcast policy PASS, live settlement signing PASS, and network reconfirmation artifact. | Stop on non-zero structural issues, target drift, placeholder targets, unscoped BRIDGE_BROADCAST_ENABLED=true, signer mismatch, or network identity drift. |
| 6. Approval-gated live submit and observation | Only after explicit approval, run the single scoped live submit and record submission plus observation evidence for the same Expected transaction ID. | Submitted transaction ID, mempool or node observation, confirmation height, confirmation count, finality evidence, ordered burn set, and proof that submitted ID equals Expected transaction ID. | Do not run without explicit approval. Stop if submitted transaction ID differs from Expected transaction ID or if any mainnet target is selected. |
| 7. Reconcile and complete benchmark evidence | Run post-submit reconciliation, update the Gate 7 benchmark evidence, then collect reviewer sign-offs after the linked live evidence exists. | Post-submit observe Markdown and JSON, finality/reconciliation evidence, completed benchmark evidence, benchmark:validate output, and benchmark owner/security/operator reviewer decisions. | Stop if reconciliation omits successor boxes or finality, benchmark validation remains BLOCKED, reviewers approve before evidence exists, or production/mainnet/throughput claims broaden. |
