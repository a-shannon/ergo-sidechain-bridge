...
class BridgePooledReserveBurnTrackerV4AcceptanceSpec
  ...
  private val Message =
    Blake2b256("pooled-reserve burn tracker V4 acceptance").toArray
  ...
  private final class FixtureRuntime(
      acceptAnyPayload: Boolean) extends StarkProfileRuntime {
    override private[sigma] def profileId: Array[Byte] =
      hex(ExpectedProfileId)
    override private[sigma] def exactProofBytes: Int =
      proofChunks.map(_.length).sum
    override private[sigma] def maxApplicationPayloadBytes: Int = 980
    ...
    override private[sigma] def verify(
      ...
      programId.sameElements(hex(ExpectedProgramId)) &&
      contractId.sameElements(hex(ExpectedContractId)) &&
  ...
  test("exact self-bound V4 proof, anchor, AVL insertion, and successor accept") {
    requiredString(cursor.downField("schema"), "fixture schema") shouldBe
      "e2s.pooled-reserve-burn-tracker-context.v4"
    requiredInt(cursor.downField("version"), "fixture version") shouldBe 4
    statement.length shouldBe 1139
    payload.length shouldBe 980
    ...
    hex(payload.slice(449 + 45, 481 + 45)) shouldBe ExpectedContractId
  ...
  test("proof transport and payload are bound by the verifier capability") {
    ...
    mutatedPayload(810) = (mutatedPayload(810) ^ 1).toByte
    verify(
      suppliedExtension = extensionWith(
        1,
  ...
  private def capability(acceptAnyPayload: Boolean) = {
    val entry = right(
      StarkVerificationCapability.active(
        new FixtureRuntime(acceptAnyPayload),
        fixedJit = 100),
  ...
  private def verify(
    ...
    val result = new ErgoLikeTestInterpreter().verify(
      ...
      Message)
    withClue("V4 tracker verification unexpectedly failed: ") {
  ...
  private def verifyTransaction(
    ...
    val result = new ErgoLikeTestInterpreter().verify(
      ...
      spendingProof,
      Message)
    withClue("serialized V4 tracker verification unexpectedly failed: ") {
      result.isSuccess shouldBe true
    }
    result.get._1
  }
  ...
  private def bindingMutation(relativeOffset: Int): Array[Byte] = {
    require(relativeOffset >= 0 && relativeOffset < 485)
    val mutated = payload.clone()
    mutated(45 + relativeOffset) =
      (mutated(45 + relativeOffset) ^ 1).toByte
    val digest = Blake2b256(
      BindingDomain ++ mutated.slice(45, 530)).toArray
    Array.copy(digest, 0, mutated, 530, digest.length)
    mutated
  }
