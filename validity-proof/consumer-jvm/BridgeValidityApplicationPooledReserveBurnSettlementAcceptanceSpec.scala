...
class BridgeValidityApplicationPooledReserveBurnSettlementAcceptanceSpec
    extends AnyFunSuite with Matchers {
  private val FixtureProperty =
    "bridge.validity.application.pooled.reserve.burn.fixture"
  private val FixtureShaProperty =
    "bridge.validity.application.pooled.reserve.burn.fixture.sha256"
  private val Schema =
    "e2s.validity-application-pooled-reserve-burn-settlement-jvm-fixture.v1"
  ...
  private val Message = Blake2b256(
    "pooled reserve V4 burn settlement acceptance fixture").toArray
  ...
  test("exact AF-4C-1 transaction and both compiled predicates accept") {
    ...
    assertAccepted("canonical AF-4C-1 transaction", tx, boxes, data, currentHeight)
  ...
  test("tracker singleton, profile, proposition, and AVL policy reject") {
    ...
    assertRejectedWithTracker(
      "wrong tracker singleton", base, boxes, wrongToken, Set(0, 1))
    ...
    assertRejectedWithTracker(
      "wrong tracker profile", base, boxes, wrongProfile, Set(0, 1))

    val wrongTree = copyBox(tracker)(ergoTree = falseTree)
    assertRejectedWithTracker(
      "wrong tracker proposition", base, boxes, wrongTree, Set(0, 1))
    ...
    assertRejectedWithTracker(
      "permissive tracker AVL policy", base, boxes, wrongPolicy, Set(0, 1))
  }
  ...
  test("burn leaf, authenticated root, Merkle path, and count reject") {
    ...
    val wrongLeaf = mutateExtension(base, 0, 2, 0)
    val wrongLeafBoth = copyExtensionVariable(wrongLeaf, 0, 1, 2)
    assertRejected(
      "burn leaf version",
      wrongLeafBoth,
      boxes,
    ...
    val bundle = extensionBytes(base, 0, 3)
    val burnProofStart = 90
    require(bundle.length > burnProofStart + 1, "fixture burn proof missing")
    val wrongPath = mutateExtension(base, 0, 3, burnProofStart + 1)
    val wrongPathBoth = copyExtensionVariable(wrongPath, 0, 1, 3)
    assertRejected(
      "burn Merkle path",
      wrongPathBoth,
      boxes,
  ...
  test("V4 exposes a DUP-authoritative split burn attribution limit") {
    val base = transaction
    val divergentLeaf = extensionBytes(base, 0, 2)
    divergentLeaf(97) = (divergentLeaf(97) ^ 1).toByte
    val divergentBurnId = Blake2b256(Array.concat(
      "E2S_TRUSTLESS_BURN_ID_V1".getBytes(StandardCharsets.US_ASCII),
      divergentLeaf.slice(1, 33),
      divergentLeaf.slice(97, 129),
      divergentLeaf.slice(129, 133))).toArray
    Array.copy(divergentBurnId, 0, divergentLeaf, 65, divergentBurnId.length)
    val mixed = replaceExtensionVariable(base, 0, 2, divergentLeaf)

    extensionBytes(mixed, 0, 2).sameElements(
      extensionBytes(mixed, 1, 2)) shouldBe false
    withClue(
      "V4 compatibility behavior: DUP proves and records the tracker-included burn, " +
        "while the reserve can accept another self-consistent leaf with the " +
        "same payout: ") {
      verifyAll(mixed, inputBoxes, dataBoxes, currentHeight) shouldBe
  ...
  test("DUP NFT, proposition, profile, replay, proofs, and successor reject") {
    ...
    val proofLayout = bundleLayout(extensionBytes(base, 0, 3))
    val wrongLookup =
      mutateExtension(base, 0, 3, proofLayout.dupLookupStart)
    val wrongLookupBoth = copyExtensionVariable(wrongLookup, 0, 1, 3)
    assertRejected(
      "DUP lookup proof",
      wrongLookupBoth,
      boxes,
    ...
    val wrongInsert =
      mutateExtension(base, 0, 3, proofLayout.dupInsertStart)
    val wrongInsertBoth = copyExtensionVariable(wrongInsert, 0, 1, 3)
    assertRejected(
      "DUP insert proof",
      wrongInsertBoth,
      boxes,
  ...
  test("missing, mistyped, and extra ContextExtension variables reject") {
    val base = transaction
    val boxes = inputBoxes
    val data = dataBoxes
    ...
    Vector(0, 1).foreach { inputIndex =>
      Vector(0, 1, 2, 3).foreach { variableId =>
        val extension = base.inputs(inputIndex).extension
        val missing = withExtension(
          base,
          inputIndex,
          ContextExtension(extension.values - variableId.toByte))
        assertRejected(
          s"input $inputIndex missing Var($variableId)",
          missing,
          boxes,
          data,
          currentHeight,
    ...
    val extra = withExtension(
      ...
      0,
      ContextExtension(
        base.inputs(0).extension.values.toMap.updated(
          4.toByte,
  ...
  private def withTrackerVariant(
    ...
    val withReserveProof =
      replaceExtensionVariable(tx, 0, 1, variant._2)
    val withDupProof =
      replaceExtensionVariable(withReserveProof, 1, 1, variant._2)
  ...
  private def assertExactExtensionShape(tx: ErgoLikeTransaction): Unit = {
    ...
    require(
      ...
      "reserve ContextExtension must contain exactly Vars 0..3")
    require(
  ...
  private def copyExtensionVariable(
      tx: ErgoLikeTransaction,
      sourceInput: Int,
      targetInput: Int,
      variableId: Int): ErgoLikeTransaction =
    replaceExtensionVariable(
      tx,
      targetInput,
      variableId,
      extensionBytes(tx, sourceInput, variableId))
