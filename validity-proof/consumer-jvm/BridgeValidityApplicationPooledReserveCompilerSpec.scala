...
class BridgeValidityApplicationPooledReserveCompilerSpec
    extends CompilerTestingCommons with Matchers {
  private val RootProperty =
    "bridge.validity.application.pooled.reserve.root"
  private val OutputProperty =
    "bridge.eip0045.validity.application.pooled.reserve.identity.out"
  ...
  // Exact normalized-LF template inputs and canonical TypeScript-derived V4
  // profile/application bindings.
  ...
  private val DupTemplateSha256 =
    "13c7d3a217f918a5b8ac22173e23696d7373666f22ddef95fd73ce208752bab7"
  private val SourceLockTemplateSha256 =
    "c93a7dbcfb2a4236ec365ca7f9c50aa90b478e103c172899643d0aec608baf33"
  private val PooledReserveTemplateSha256 =
    "525c3f1fb7fb5da472a96931447de4e04f42e31c1be2f22ee8ad8408acd48aa6"
  ...
  private val SourceRuntimeCodeSha256 = "dd" * 32
  private val SourceRuntimeCodeSize = "00002000"
  private val ApplicationBindingPrefix =
    RuntimeProfile + RuntimeProfileId + SourceRuntimeCodeSha256 +
      SourceRuntimeCodeSize + TrackerNft
  private val ApplicationBindingDomain =
    "E2S_POOLED_RESERVE_BURN_APPLICATION_BINDING_V4"
  ...
  property(
    ...
    val dupTemplate = exactTemplate(
      root.resolve("contracts/DoubleUnlockPreventionPooledReserveV4.es"),
      DupTemplateSha256)
    val sourceLockTemplate = exactTemplate(
      root.resolve("contracts/MainChainLockPooledReserveV4.es"),
    ...
    val trackerSource = exactSource(
      replaceAll(trackerTemplate.source, Vector(
        "POOLED_RESERVE_BURN_V4_PROGRAM_ID_PLACEHOLDER" -> ProgramId,
  ...
  private def assertConcreteInputs(): Unit = {
    Vector(
      ...
      ("application binding prefix", ApplicationBindingPrefix, 449)
    ...
    hex(runtime.slice(1, 33)) shouldBe PooledReserveProfile
    ...
    hex(bindingPrefix.slice(0, 349)) shouldBe RuntimeProfile
    hex(bindingPrefix.slice(349, 381)) shouldBe RuntimeProfileId
    hex(bindingPrefix.slice(381, 413)) shouldBe SourceRuntimeCodeSha256
    hex(bindingPrefix.slice(413, 417)) shouldBe SourceRuntimeCodeSize
    hex(bindingPrefix.slice(417, 449)) shouldBe TrackerNft
  }
