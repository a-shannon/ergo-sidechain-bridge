...
class BridgePooledReserveBurnTrackerV4ContractSpec
  ...
  private val ExpectedPropositionBytes = 2942
  ...
  property("pinned compiler emits one self-bound pooled-reserve burn V4 tracker") {
    ...
    val bindingPrefix = binding.take(449)
    binding.length shouldBe 485
    bindingPrefix.length shouldBe 449
    ...
    val resolved = replaceExactly(source, Vector(
      ...
      "POOLED_RESERVE_BURN_V4_APPLICATION_BINDING_PREFIX_PLACEHOLDER" ->
        hex(bindingPrefix)))
    resolved should include("SPVTrackerPooledReserveBurnV4")
    resolved should include("blake2b256(SELF.propositionBytes)")
    resolved should include("payload.size == 980")
    ...
    println(s"pooled_reserve_burn_tracker_v4_proposition_bytes=${first.length}")
    println(s"pooled_reserve_burn_tracker_v4_proposition_sha256=${sha256(first)}")
    println(s"pooled_reserve_burn_tracker_v4_contract_id_hex=$contractId")
    ...
    val value = json.hcursor.downField("expected")
      ...
      failure => throw new IllegalArgumentException(
        "pooled-reserve burn statement vector binding missing: " +
          failure.getMessage),
      identity)
    decodeHex(value, 485, "pooled-reserve burn statement binding")
  }
  ...
  private def writeIdentityIfRequested(
    ...
    val json =
      s"""{
         |  "schema": "e2s.pooled-reserve-burn-tracker-contract.v4",
         |  "version": 4,
