...
class BridgeValidityApplicationPooledReserveBurnSettlementV5AcceptanceSpec
    extends AnyFunSuite with Matchers {
  private val FixtureProperty =
    "bridge.validity.application.pooled.reserve.burn.v5.fixture"
  private val FixtureShaProperty =
    "bridge.validity.application.pooled.reserve.burn.v5.fixture.sha256"
  private val Schema =
    "e2s.validity-application-pooled-reserve-burn-settlement-jvm-fixture.v5"
  ...
  private val Message = Blake2b256(
    "pooled reserve V5 burn settlement acceptance fixture").toArray
  ...
  test("exact V5 transaction and both compiled predicates accept") {
    ...
    assertAccepted("canonical V5 transaction", tx, boxes, data, currentHeight)
  ...
  test("required ContextExtension faults reject and exact producer shape excludes extras") {
    val base = transaction
    val boxes = inputBoxes
    val data = dataBoxes
    ...
    Vector(0, 1).foreach { inputIndex =>
      val extra = withExtension(
        ...
        inputIndex,
        ContextExtension(
          base.inputs(inputIndex).extension.values.toMap.updated(
            4.toByte,
            ByteArrayConstant(Array(1.toByte)))))
      withClue(
        s"input $inputIndex extra Var(4) is outside predicate semantics: ") {
