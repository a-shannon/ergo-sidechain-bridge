...
use super::pooled_reserve_burn_statement_v4::{
...
/// Domain prefix for pooled-reserve burn public inputs V5.
pub const POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN: &[u8] =
    b"E2S_POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5";
/// Domain used to identify the complete V5 EIP-0045 statement bytes.
pub const POOLED_RESERVE_BURN_STATEMENT_V5_DOMAIN: &[u8] = b"E2S_POOLED_RESERVE_BURN_STATEMENT_V5";
/// Frozen application V2 guest identity that V5 must never reinterpret.
pub const POOLED_RESERVE_BURN_V5_REJECTED_APPLICATION_V2_PROGRAM_ID: [u8; 32] = [
...
/// Exact bytes of the pooled-reserve burn application binding V5.
pub const POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_BYTES: usize = 486;
/// Exact bytes of pooled-reserve burn public inputs V5.
pub const POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES: usize = 981;
/// Exact bytes of the standard EIP-0045 statement carrying burn public inputs V5.
pub const EIP0045_POOLED_RESERVE_BURN_STATEMENT_V5_BYTES: usize =
    EIP0045_ERGO_STATEMENT_V1_FIXED_BYTES + POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES;

/// Domain used to bind the exact pooled-reserve burn application identity.
pub const POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_DOMAIN: &[u8] =
    b"E2S_POOLED_RESERVE_BURN_APPLICATION_BINDING_V5";
const PUBLIC_INPUTS_PREFIX_BYTES: usize = 45;
const BINDING_OFFSET: usize = PUBLIC_INPUTS_PREFIX_BYTES;
const BINDING_DIGEST_OFFSET: usize = 531;
const CHECKPOINT_OFFSET: usize = 563;
const CHECKPOINT_COMMITMENT_OFFSET: usize = 779;
const TARGET_STATE_ROOT_OFFSET: usize = 811;
const TRUSTED_ANCHOR_OFFSET: usize = 843;
const FINALITY_HEIGHT_OFFSET: usize = 875;
const FINALITY_HASH_OFFSET: usize = 883;
const EXTENSION_KEY_OFFSET: usize = 915;
const EXTENSION_VALUE_OFFSET: usize = 917;

const RUNTIME_PROFILE_OFFSET: usize = 1;
const PROFILE_ID_OFFSET: usize = 350;
const SOURCE_RUNTIME_CODE_HASH_OFFSET: usize = 382;
const SOURCE_RUNTIME_CODE_BYTES_OFFSET: usize = 414;
const TRACKER_NFT_OFFSET: usize = 418;
const TRACKER_CONTRACT_OFFSET: usize = 450;
const PREACTIVATION_STATE_OFFSET: usize = 482;
const AUTHORIZATION_FLAGS_OFFSET: usize = 483;
const RESERVED_OFFSET: usize = 484;

const _: [(); 40] = [(); POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN.len()];
const _: [(); 349] = [(); POOLED_RESERVE_MINT_RESERVATION_RUNTIME_PROFILE_V4_BYTES];
const _: [(); 486] = [(); POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_BYTES];
const _: [(); 981] = [(); POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES];
const _: [(); 1_140] = [(); EIP0045_POOLED_RESERVE_BURN_STATEMENT_V5_BYTES];

/// Exact public application binding for pooled-reserve burn settlement V5.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PooledReserveBurnApplicationBindingV5 {
    ...
    /// Preactivation state, fixed to zero in V5.
    pub preactivation_state: u8,
    /// Authorization flags, fixed to zero in V5.
    pub authorization_flags: u8,
    /// Reserved bytes, fixed to zero in V5.
    pub reserved: [u8; 2],
}

/// Canonical pooled-reserve burn public inputs V5.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PooledReserveBurnPublicInputsV5 {
    /// Strictly decoded application binding.
    pub application_binding: PooledReserveBurnApplicationBindingV5,
    ...
    pub encoded: [u8; POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES],
}

/// Decoded standard EIP-0045 statement carrying pooled-reserve burn public inputs V5.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct Eip0045PooledReserveBurnStatementV5 {
    ...
    pub public_inputs: PooledReserveBurnPublicInputsV5,
    /// Exact canonical statement bytes.
    pub encoded: [u8; EIP0045_POOLED_RESERVE_BURN_STATEMENT_V5_BYTES],
}

/// Encode one exact pooled-reserve burn application binding V5.
pub fn encode_pooled_reserve_burn_application_binding_v5(
    binding: &PooledReserveBurnApplicationBindingV5,
) -> Result<[u8; POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_BYTES], StatementError> {
    validate_application_binding(binding)?;
    let profile =
        encode_pooled_reserve_mint_reservation_runtime_profile_v4(&binding.runtime_profile)?;
    let mut encoded = [0u8; POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_BYTES];
    encoded[0] = 5;
    encoded[RUNTIME_PROFILE_OFFSET..PROFILE_ID_OFFSET].copy_from_slice(&profile);
...
/// Decode and validate one exact pooled-reserve burn application binding V5.
pub fn decode_pooled_reserve_burn_application_binding_v5(
    bytes: &[u8],
) -> Result<PooledReserveBurnApplicationBindingV5, StatementError> {
    let encoded = exact_array::<POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_BYTES>(
        bytes,
        "pooled-reserve burn application binding V5",
    )?;
    if encoded[0] != 5 {
        return Err(StatementError::Discriminator(
            "pooled-reserve burn application binding version",
        ));
    }
    let binding = PooledReserveBurnApplicationBindingV5 {
        runtime_profile: decode_pooled_reserve_mint_reservation_runtime_profile_v4(
            &encoded[RUNTIME_PROFILE_OFFSET..PROFILE_ID_OFFSET],
        )?,
...
/// Derive the domain-separated digest of one exact V5 application binding.
pub fn derive_pooled_reserve_burn_application_binding_v5_digest(
    binding: &PooledReserveBurnApplicationBindingV5,
) -> Result<[u8; 32], StatementError> {
    let encoded = encode_pooled_reserve_burn_application_binding_v5(binding)?;
    Ok(domain_hash(
...
/// Encode the exact 981-byte pooled-reserve burn public inputs V5.
pub fn encode_pooled_reserve_burn_public_inputs_v5(
    binding: &PooledReserveBurnApplicationBindingV5,
    checkpoint: &[u8],
...
) -> Result<[u8; POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES], StatementError> {
    let binding = encode_pooled_reserve_burn_application_binding_v5(binding)?;
    let checkpoint = exact_array::<CHECKPOINT_BYTES>(checkpoint, "bridge checkpoint V1")?;
    validate_payload_fields(
        &decode_pooled_reserve_burn_application_binding_v5(&binding)?,
        &checkpoint,
    ...
    let binding_digest = domain_hash(POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_DOMAIN, &binding);
    let checkpoint_commitment = domain_hash(CHECKPOINT_DOMAIN, &checkpoint);

    let mut encoded = [0u8; POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES];
    encoded[..POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN.len()]
        .copy_from_slice(POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN);
    encoded[POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN.len()] = 0;
    encoded[41..PUBLIC_INPUTS_PREFIX_BYTES].copy_from_slice(&[5, 1, 1, 0]);
...
/// Decode and validate the exact 981-byte pooled-reserve burn public inputs V5.
pub fn decode_pooled_reserve_burn_public_inputs_v5(
    bytes: &[u8],
) -> Result<PooledReserveBurnPublicInputsV5, StatementError> {
    let encoded = exact_array::<POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES>(
        bytes,
        "pooled-reserve burn public inputs V5",
    )?;
    if &encoded[..POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN.len()]
        != POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN
        || encoded[POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN.len()] != 0
        || encoded[41..PUBLIC_INPUTS_PREFIX_BYTES] != [5, 1, 1, 0]
    {
        return Err(StatementError::Discriminator(
    ...
    let application_binding = decode_pooled_reserve_burn_application_binding_v5(
    ...
    Ok(PooledReserveBurnPublicInputsV5 {
        application_binding,
...
/// Encode the standard EIP-0045 StatementV1 envelope for burn public inputs V5.
pub fn encode_eip0045_pooled_reserve_burn_statement_v5(
    chain_domain_id: [u8; 32],
    profile_id: [u8; 32],
    program_id: [u8; 32],
    contract_id: [u8; 32],
    public_inputs: &[u8],
) -> Result<[u8; EIP0045_POOLED_RESERVE_BURN_STATEMENT_V5_BYTES], StatementError> {
    validate_outer_identities(&chain_domain_id, &profile_id, &program_id, &contract_id)?;
    let public_inputs = decode_pooled_reserve_burn_public_inputs_v5(public_inputs)?;
    validate_outer_bindings(&chain_domain_id, &contract_id, &public_inputs)?;
    let mut encoded = [0u8; EIP0045_POOLED_RESERVE_BURN_STATEMENT_V5_BYTES];
    ...
    encoded[155..159]
        .copy_from_slice(&(POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES as u32).to_le_bytes());
    encoded[159..].copy_from_slice(&public_inputs.encoded);
    Ok(encoded)
}

/// Decode the standard EIP-0045 StatementV1 envelope for burn public inputs V5.
pub fn decode_eip0045_pooled_reserve_burn_statement_v5(
    bytes: &[u8],
) -> Result<Eip0045PooledReserveBurnStatementV5, StatementError> {
    let encoded = exact_array::<EIP0045_POOLED_RESERVE_BURN_STATEMENT_V5_BYTES>(
        bytes,
        "EIP-0045 pooled-reserve burn statement V5",
    )?;
    ...
    validate_outer_identities(&chain_domain_id, &profile_id, &program_id, &contract_id)?;
    let public_inputs = decode_pooled_reserve_burn_public_inputs_v5(&encoded[159..])?;
    validate_outer_bindings(&chain_domain_id, &contract_id, &public_inputs)?;
    Ok(Eip0045PooledReserveBurnStatementV5 {
        chain_domain_id,
        profile_id,
        program_id,
        contract_id,
...
/// Derive the V5-family identity of one exact standard EIP-0045 statement.
pub fn derive_eip0045_pooled_reserve_burn_statement_v5_digest(
    statement: &[u8],
) -> Result<[u8; 32], StatementError> {
    let statement = decode_eip0045_pooled_reserve_burn_statement_v5(statement)?;
    Ok(domain_hash(
        POOLED_RESERVE_BURN_STATEMENT_V5_DOMAIN,
        &statement.encoded,
    ))
}
...
fn validate_outer_identities(
    ...
    if program_id == &POOLED_RESERVE_BURN_V5_REJECTED_APPLICATION_V2_PROGRAM_ID {
        return Err(StatementError::EipStatement(
            "application V2 program is invalid for V5",
        ));
    }
    Ok(())
}
...
fn validate_outer_bindings(
    chain_domain_id: &[u8; 32],
    contract_id: &[u8; 32],
    public_inputs: &PooledReserveBurnPublicInputsV5,
) -> Result<(), StatementError> {
...
mod tests {
    ...
    fn deterministic_round_trip_freezes_offsets_and_endianness() {
        ...
        assert_eq!(public_inputs.len(), 981);
        assert_eq!(statement.len(), 1_140);
        assert_eq!(
            &public_inputs[..40],
            POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_DOMAIN
        );
        assert_eq!(&public_inputs[40..45], &[0, 5, 1, 1, 0]);
        assert_eq!(public_inputs[45], 5);
        assert_eq!(&public_inputs[46 + 169..46 + 173], &4_104u32.to_le_bytes());
        assert_eq!(&public_inputs[46 + 205..46 + 209], &2_356u32.to_le_bytes());
        assert_eq!(&public_inputs[46 + 337..46 + 345], &0u64.to_le_bytes());
        assert_eq!(&public_inputs[46 + 345..46 + 349], &48u32.to_le_bytes());
        assert_eq!(&public_inputs[45 + 414..45 + 418], &9_999u32.to_be_bytes());
        assert_eq!(&public_inputs[875..883], &1_100u64.to_be_bytes());
        assert_eq!(&statement[155..159], &981u32.to_le_bytes());
        assert_eq!(
            &public_inputs[531..563],
            &domain_hash(
                POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_DOMAIN,
                &public_inputs[45..531],
            ),
        );
        assert_eq!(
            &public_inputs[779..811],
            &domain_hash(CHECKPOINT_DOMAIN, &checkpoint),
        );
        assert_eq!(
            decode_pooled_reserve_burn_public_inputs_v5(&public_inputs)
                .unwrap()
                .encoded,
            public_inputs,
        );
        assert_eq!(
            decode_eip0045_pooled_reserve_burn_statement_v5(&statement)
                .unwrap()
                .encoded,
            statement,
        );
        assert_eq!(
            derive_eip0045_pooled_reserve_burn_statement_v5_digest(&statement).unwrap(),
            domain_hash(POOLED_RESERVE_BURN_STATEMENT_V5_DOMAIN, &statement),
        );
    }
    ...
    fn matches_the_shared_typescript_golden_vector_byte_for_byte() {
        ...
        let binding_bytes = vector_hex::<POOLED_RESERVE_BURN_APPLICATION_BINDING_V5_BYTES>(
            &expected["encodedBindingHex"],
        );
        let binding = decode_pooled_reserve_burn_application_binding_v5(&binding_bytes).unwrap();
        assert_eq!(
            encode_pooled_reserve_burn_application_binding_v5(&binding).unwrap(),
            binding_bytes
        );
        assert_eq!(
            derive_pooled_reserve_burn_application_binding_v5_digest(&binding).unwrap(),
            vector_hex::<32>(&expected["bindingDigestHex"]),
        );

        let public_input_bytes = vector_hex::<POOLED_RESERVE_BURN_PUBLIC_INPUTS_V5_BYTES>(
            &expected["encodedPublicInputsHex"],
        );
        let public_inputs =
            decode_pooled_reserve_burn_public_inputs_v5(&public_input_bytes).unwrap();
        assert_eq!(
            encode_pooled_reserve_burn_public_inputs_v5(
        ...
        let statement_bytes = vector_hex::<EIP0045_POOLED_RESERVE_BURN_STATEMENT_V5_BYTES>(
            &expected["encodedStatementHex"],
        );
        let statement = decode_eip0045_pooled_reserve_burn_statement_v5(&statement_bytes).unwrap();
        assert_eq!(
        ...
        assert_eq!(
            derive_eip0045_pooled_reserve_burn_statement_v5_digest(&statement_bytes).unwrap(),
    ...
    fn rejects_profile_shape_identity_and_required_field_drift() {
        ...
        assert!(encode_pooled_reserve_burn_application_binding_v5(&wrong_id).is_err());
        ...
        for mutation in 0..4 {
            ...
            assert!(encode_pooled_reserve_burn_application_binding_v5(&changed).is_err());
        }
    }
    ...
    fn rejects_preactivation_authorization_and_reserved_bytes() {
        ...
        let canonical = encode_pooled_reserve_burn_application_binding_v5(&binding).unwrap();
        let mut wrong_version = canonical;
        wrong_version[0] = 4;
        assert!(decode_pooled_reserve_burn_application_binding_v5(&wrong_version).is_err());
        for offset in [482usize, 483, 484, 485] {
            let mut changed = canonical;
            changed[offset] = 1;
            assert!(decode_pooled_reserve_burn_application_binding_v5(&changed).is_err());
        }
    }
    ...
    fn rejects_every_payload_cross_field_substitution() {
        let (binding, checkpoint) = fixture();
        let encode = |binding: &PooledReserveBurnApplicationBindingV5,
                      checkpoint: &[u8],
                      state_root: [u8; 32],
                      anchor: [u8; 32],
                      height: u64,
        ...
        for offset in [0usize, 40, 41, 42, 43, 44] {
            let mut changed = canonical;
            changed[offset] ^= 1;
            assert!(decode_pooled_reserve_burn_public_inputs_v5(&changed).is_err());
        }
        for offset in [531usize, 779, 915, 917, 949] {
            let mut changed = canonical;
            changed[offset] ^= 1;
            assert!(decode_pooled_reserve_burn_public_inputs_v5(&changed).is_err());
        }
    ...
    fn rejects_outer_identity_and_cross_binding_substitution() {
        ...
        for identity in 0..4 {
            ...
            assert!(encode_eip0045_pooled_reserve_burn_statement_v5(
                changed[0],
                changed[1],
                changed[2],
                changed[3],
        ...
        aliased_program[91..123]
            .copy_from_slice(&POOLED_RESERVE_BURN_V5_REJECTED_APPLICATION_V2_PROGRAM_ID);
        assert!(decode_eip0045_pooled_reserve_burn_statement_v5(&aliased_program).is_err());
    }
    ...
    fn statement_families_are_not_cross_decodable() {
        ...
        let v4_binding = crate::PooledReserveBurnApplicationBindingV4 {
            runtime_profile: binding.runtime_profile.clone(),
            runtime_profile_id: binding.runtime_profile_id,
            source_runtime_code_sha256: binding.source_runtime_code_sha256,
            source_runtime_code_bytes: binding.source_runtime_code_bytes,
            tracker_nft_id: binding.tracker_nft_id,
            settlement_tracker_contract_id: binding.settlement_tracker_contract_id,
            preactivation_state: binding.preactivation_state,
            authorization_flags: binding.authorization_flags,
            reserved: binding.reserved,
        };
        let v4_binding_bytes =
            crate::encode_pooled_reserve_burn_application_binding_v4(&v4_binding).unwrap();
        assert!(decode_pooled_reserve_burn_application_binding_v5(&v4_binding_bytes).is_err());
        assert!(crate::decode_pooled_reserve_burn_application_binding_v4(
            &public_inputs[BINDING_OFFSET..BINDING_DIGEST_OFFSET]
        )
        .is_err());
        let v4_public_inputs = crate::encode_pooled_reserve_burn_public_inputs_v4(
            &v4_binding,
            &checkpoint,
            [1; 32],
            [2; 32],
        ...
        .unwrap();
        let v4_statement = crate::encode_eip0045_pooled_reserve_burn_statement_v4(
            v4_binding.runtime_profile.source_network_id,
            [4; 32],
            [5; 32],
            v4_binding.settlement_tracker_contract_id,
            &v4_public_inputs,
        )
        .unwrap();
        assert!(decode_pooled_reserve_burn_public_inputs_v5(&v4_public_inputs).is_err());
        assert!(crate::decode_pooled_reserve_burn_public_inputs_v4(&public_inputs).is_err());
        assert!(decode_eip0045_pooled_reserve_burn_statement_v5(&v4_statement).is_err());
        assert!(crate::decode_eip0045_pooled_reserve_burn_statement_v4(&statement).is_err());
        #[cfg(feature = "application-v2")]
        {
            ...
            assert!(
                crate::decode_bridge_causal_application_binding_v2(&public_inputs[45..531])
                    .is_err()
            );
            ...
            assert!(decode_pooled_reserve_burn_application_binding_v5(&v2_binding).is_err());
            assert!(decode_pooled_reserve_burn_public_inputs_v5(&v2_payload).is_err());
            assert!(decode_eip0045_pooled_reserve_burn_statement_v5(&v2_statement).is_err());
        }
    }
    ...
    fn fixture() -> (
        PooledReserveBurnApplicationBindingV5,
        [u8; CHECKPOINT_BYTES],
    ) {
