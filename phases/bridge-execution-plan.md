...
| Active internal slice | **WP-08G public research-alpha source promotion, followed by return to the FED-6 external boundary.** The exact source package may be published to attract testing and independent review after promotion checks pass. This does not activate a profile, close Gate 5, establish independent custody, support a release, enable deployment, or authorize funds. The completed WP-07A-DN1 source-locked recovery campaign remains local operational evidence and must not be extended with another synthetic lifecycle. After source publication, the next value-bearing milestone remains the externally blocked FED-6 target/custody intake. |
...
| WP-08G state | The auditor package now has a tracked `public-research-alpha` manifest, exact critical/high gap ownership, one offline `audit:alpha` command, auditor-first documentation, dual-layout source-lock verification, standalone-compatible recursive CI and explicit runtime-state exclusion. The current superproject checkout procedure initializes only `ergo-sidechain-bridge/substrate-node`, so unrelated sibling gitlinks cannot enter or break the bridge audit boundary; a future standalone repository retains normal recursive initialization. WP-08I adds the complete standalone consensus-source job, but no hosted execution result is claimed. WP-08J adds the local alert drill and inert reviewed runbook references; WP-08K adds a separately invoked bounded HTTPS worker and authenticated acknowledgement audit path. WP-08L packages the Apache-2.0 repository license and explicit third-party boundaries. The legacy cutover inventory now derives every state-mutating `ErgoBridge` function from the tracked compiled ABI and explicitly includes the permissionless `pegOut(uint256,bytes)` surface, so an omitted or newly added mutable bridge entrypoint fails closed. Policy permits source publication for public research only after the exact candidate passes promotion checks; supported release status remains blocked. The package does not provide evidence of delivery to a reviewed external target, a real operator acknowledgement, current-HEAD non-mainnet recovery or clean-checkout execution, complete independent review, Gate 5 closure or any live funds capability |
...
| WP-08I state | The bridge-local recursive workflow now carries one exact standalone `consensus-sources` job. It restores the existing gitlink, verifies the canonical source lock and Solidity identities, applies only the two locked patches, runs the Frontier commitment/finality/state/checkpoint matrices and node build, runs the Ergo extension test and assembly, then rechecks both source trees. `sources:verify:workflow` parses the YAML and binds its exact ordered graph to `sources/consensus-source-lock.json`, rejecting moving or foreign inputs, superproject paths and live-capability commands. Local syntax and command-graph validation is complete; a hosted run bound to the exact candidate commit remains separate promotion evidence and no profile activation, deployment identity, Gate 5, publication or readiness claim follows |
...
| Latest completed slice | **WP-07A-DN1 source-locked dual-node recovery campaign, local and non-authorizing.** The same exact source-built binary accepted the unchanged Aura target and a distinct manual-seal drill spec. The exact source-locked wrapper and complete lifecycle digest were consumed; four snapshots were captured inside one owned-process lifetime; deterministic source disagreement rejected; an ephemeral SQLite database was physically deleted and recreated; and the promoted composition reconstructed only the complete inventory plus continuity hold. The campaign receipt is `8064efe244a4a1702c019e55763bcd688f3a00cd272783f7ecc1288205a992b08`; all fixed listeners were released. This closes the planned local campaign, not independent custody, source consensus, FED-6 activation, funds authority, Gate 5, trustless status or readiness. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row or injected port may expose a concrete signer or transport capability after the authorized run. Reward delay 1 or 720 remains digest-bound to the exact signer-owned reward proposition. The exact FED-6 reobservation invocation must use `NonMiningResume`, which requires `ResumeExistingDataDir`, rejects a runner-supplied mining-mnemonic reference and forces both primary and witness processes to non-mining mode; discovery and the separate observer use bounded GET-only requests and verify chain identity. This leaves the documented Phase 011b mining-enabled resume workflow unchanged and outside FED-6 evidence. **Owned paths:** the existing patched-devnet runner and no-submit conformance/check modules; the signer-first session, reward-input discovery and canonical history producers; the bounded read-only node client; `relayer-core/substrate-federated-local-devnet-genesis-execution-v1{.test,}.ts`; the generic operational lifecycle; and the StateTracker operational profile. **Current checks:** exact delay and reciprocal-mismatch matrices; deterministic signer-bound three-input selection with dual-source disagreement, ambiguity, immaturity and signer-drift negatives; canonical header/transaction/UTXO construction and mutation negatives; pinned JVM conformance; pure lifecycle ordering and provenance negatives; unresolved-attempt/restart journal constraints; runner source guards; Node 24 typecheck; layer imports; broadcast-surface isolation; the live exact-three local execution; and the exact dual-origin non-mining reobservation above. There is no tracked execution CLI, root submission signer, `/transactions` adapter or live port registrar. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import and every value-path or activation check remain unvalidated. |
| Following boundary | **FED-6 operational custody, target approval, and separately authorized setup lineage execution; externally and authorization blocked.** A future run requires a fresh packet whose source-attestation threshold and Ergo-admission role use independently reviewed, operationally durable custody, exact target identity and approval, and authentic target-owned history. The disposable G1dI3c packet cannot be reused. Signing/checking, setup submission, broadcast, canonical confirmation, profile activation, and later value-path execution remain distinct approvals and must not be collapsed into one operator action. |
| Next implementation action | **Promote the exact WP-08G public research-alpha source candidate, then resume FED-6 only from authentic target and custody inputs.** Commit the source-publication policy split, create the deterministic standalone bundle, run the complete audit from a fresh recursive checkout against those exact bytes, retain its candidate identity, and publish only after that promotion passes. This source milestone does not require Gate 5, independent custody, or supported-release evidence and does not upgrade their claims. Afterward, obtain an explicitly approved non-mainnet target identity, independently controlled source-attestation and Ergo-admission roles, target-owned canonical setup history, complete route-retirement/replay lineage, and separate authorization for each check/sign/submit/confirmation step. Preserve the completed WP-07A-DN1 campaign and do not create another local node wrapper, synthetic activation, manual-seal surrogate, disposable authority packet or recovery lifecycle. |
...
|---|---|---|
| Main implementation | FED-6G authorized isolated-devnet packet and no-submit acceptance | Consume the frozen exact G1dA process-owned history bundle through a separately versioned G1dB isolated-devnet descriptor/statement and matching replay to freeze the exact three setup identities. Only afterward obtain exact signed JVM and same-target `/transactions/check` receipts without submission, broadcast, registration or funds authority. Frozen FED-5G V1 is regression evidence, not the local-devnet packet format. |
...
| Parallel implementation/research | Approved-target identity plus migration-retirement or greenfield-history inputs | Collect only exact source-owned target, deployment, genesis, history and retirement inputs that can satisfy the selected launch-mode consumer. Do not create another local candidate, opaque evidence wrapper, synthetic acceptance result or partial retirement/non-instantiation claim. Independent review follows one stable complete evidence join. |
...
| **WP-06-FED** | EIP-independent federated peg-in and peg-out profile | WP-01/04/05 foundations plus reviewed V4/V6 conservation and WP-07 lifecycle | Federated statement/tracker, peg-in admission, settlement family, daemon integration, and cutover are sequential authority joins; disjoint target/retirement evidence work may run in parallel only when real inputs exist | **Migration targets remain at the FED-6 external boundary; greenfield FED-5G1/G2 are complete locally. FED-1 through FED-4, FED-5A/B migration generation, and the distinct greenfield baseline/generation/provisioning/portable-replay joins are complete as non-authorizing checkpoints. Isolated delay-1 and delay-720 signing/JVM/node-check conformance cannot substitute for target-owned evidence. A concrete authenticated packet, exact target acceptance, canonical setup lineages, static registration and value-path execution remain open.** |
...
and `template/node/src/bridge_atomicity_tests.rs`; the complete 107-file map
remains machine-bound in `sources/consensus-source-lock.json`. The runtime and node
...
| Independent security review | Open | Review the exact final candidate and retain every unresolved critical/high finding as a blocker |
| Current-HEAD clean recursive checkout evidence | Open | Run the documented gate from a fresh recursive checkout and bind it to the final candidate commit |
| Standalone consensus-source build CI | Locally complete; WP-08I | Retain the exact parser-validated standalone job; capture a hosted run only for the exact promotion candidate |
...
| External alert delivery, acknowledgement, and live recovery evidence | Local mechanism complete; external evidence blocked | Exercise the WP-08K worker against a reviewed target with reviewed credential custody, retain one real Ed25519 acknowledgement bound to the exact delivered alert and receipt, and run the current-HEAD recovery procedure on an exact non-mainnet deployment. Local worker/verifier tests, synthetic transport and acknowledgement fixtures do not satisfy this evidence |
...
supported release status remains blocked. A green local audit command proves
only its declared deterministic implementation and containment checks. It does
not satisfy independent security review, external activation, Gate 5,
