...
| Active internal slice | **FED-6-LAB - fresh paired committed-reserve-to-mint campaign.** The source-locked `runtime-v4-lab` TestClient now consumes one shared 603-byte TypeScript/Rust reservation statement, admits it in native block 5, rejects an otherwise valid reserved mint when an unreserved sibling appears in block 6 without changing chain or EVM state, and then consumes the exact direct-parent reservation when the corrected single-mint block 6 is imported. The consumed record, transaction hash, execution-block hash, `processedPegIns`, token supply and recipient balance are checked exactly. This closes the local runtime join, not the source-to-runtime evidence join. The next objective is a fresh same-lifetime Ergo/Frontier campaign that constructs this statement from the actually confirmed committed-reserve receipt and exercises the same atomic consumer without caller-supplied authority. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| **WP-06-FED** | EIP-independent federated peg-in and peg-out profile | WP-01/04/05 foundations plus reviewed V4/V6 conservation and WP-07 lifecycle | Federated statement/tracker, peg-in admission, settlement family, daemon integration, and cutover are sequential authority joins; disjoint target/retirement evidence work may run in parallel only when real inputs exist | **Migration targets remain at the FED-6 external boundary. FED-6-LAB now has two separately verified halves: exact committed-reserve confirmation through source checkpoint `49218bcd`, and a source-locked TestClient import that consumes a cross-language direct-parent reservation into one exact mint while atomically rejecting an unreserved sibling. FED-1 through FED-4, FED-5A/B migration generation, and the distinct greenfield baseline/generation/provisioning/portable-replay joins remain frozen. The actual committed-reserve receipt has not yet produced and driven the reservation in one fresh paired campaign. Peg-out, recovery, static registration, independent custody and externally approved target evidence remain open.** |
...
`f1972eb4ae803a9a6181a6380282e408c4fea0f16590a1c558e709f028b393b5`.
...
transaction status and preserves both contracts' code, nonce and balance. A
candidate block containing the reserved mint plus an unreserved sibling rejects
