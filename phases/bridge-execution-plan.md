...
| Active internal slice | **WP-08G public research-alpha hosted closure and audit intake, followed by return to the FED-6 external boundary.** The initial standalone source candidate is public. Its first hosted run exposed one Windows npm-toolcache overlay and one Linux native-verifier invocation that incorrectly assumed the Windows-only pinned local toolchain profile. The current slice preserves both strict boundaries while making the public workflow reproducible: isolate the exact locked audit Node/npm tree, build the Frontier executables in the same Linux job, and exercise them under a separately capped supplied-executable conformance mode. The exact workflow graph, not that mode, establishes same-job source-build provenance. Require a green hosted run for the exact promoted bytes before treating hosted reproducibility as observed. This does not activate a profile, close Gate 5, establish independent custody, support a release, enable deployment, or authorize funds. The completed WP-07A-DN1 source-locked recovery campaign remains local operational evidence and must not be extended with another synthetic lifecycle. |
...
| WP-08G state | The auditor package has a tracked `public-research-alpha` manifest, exact critical/high gap ownership, one offline `audit:alpha` command, auditor-first documentation, dual-layout source-lock verification, standalone-compatible recursive CI and explicit runtime-state exclusion. The first standalone snapshot was published as commit `5162583d47b64bb9c35d38d4b988a932ababadeb`, bound to superproject source commit `0f8cc972ae7819e7684152db3945f3ced9c43e98` and bridge tree `02a9035c04af700be17489a8609752dba0444aac`. Its first hosted run is failure evidence, not a green promotion receipt; the two observed reproducibility defects are owned by WP-08I and the current remediation slice. WP-08J adds the local alert drill and inert reviewed runbook references; WP-08K adds a separately invoked bounded HTTPS worker and authenticated acknowledgement audit path. WP-08L packages the Apache-2.0 repository license and explicit third-party boundaries. The legacy cutover inventory derives every state-mutating `ErgoBridge` function from the tracked compiled ABI and explicitly includes the permissionless `pegOut(uint256,bytes)` surface, so an omitted or newly added mutable bridge entrypoint fails closed. Source publication is permitted for research review after exact local promotion checks; every promoted milestone must preserve the claim boundary and obtain its own green hosted result. Supported release status remains blocked. The package does not provide evidence of delivery to a reviewed external target, a real operator acknowledgement, current-HEAD non-mainnet recovery, complete independent review, Gate 5 closure or any live funds capability |
...
| Next implementation action | **Promote the WP-08G hosted-reproducibility remediation, require a green exact public workflow, then open the research-alpha audit intake and resume FED-6 only from authentic target and custody inputs.** Keep the public repository synchronized at coherent reviewed milestones. After the hosted closure, solicit review against the exact public commit while obtaining an explicitly approved non-mainnet target identity, independently controlled source-attestation and Ergo-admission roles, target-owned canonical setup history, complete route-retirement/replay lineage, and separate authorization for each check/sign/submit/confirmation step. Preserve the completed WP-07A-DN1 campaign and do not create another local node wrapper, synthetic activation, manual-seal surrogate, disposable authority packet or recovery lifecycle. |
...
| Current-HEAD clean recursive checkout evidence | Initial hosted attempt failed on a known runner npm overlay; remediation implemented locally | Promote the exact remediation and retain a green hosted audit result for that commit |
| Standalone consensus-source build CI | Initial hosted attempt reached native tests, then rejected the Windows-only pinned-local CLI assumption; remediation implemented locally | Promote the exact workflow-bound same-job build plus supplied-executable vector path and retain a green hosted result for that commit |
...
jobs, the audit isolation command and both executable paths. A successful
future hosted run remains bounded reproducibility evidence for its exact public
commit.
