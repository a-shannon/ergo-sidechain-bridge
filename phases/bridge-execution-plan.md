...
| Active internal slice | **FED-6-LAB - isolated local execution campaign.** Fresh synthetic role keys, local signing, setup submission and broadcast are enabled exclusively for task-owned loopback devnet processes with simulated funds. Build this as a separately versioned execution family that consumes the frozen accepted target/history/provisioning producers without changing their bytes or reinterpreting the V1 `no-submit` receipts. Close setup check, submission and canonical confirmation first; then execute exact peg-in and peg-out candidates; then exercise recovery against the same target. Every capability must bind the exact process-owned target, transaction bytes, authorization, durable attempt and confirmation policy. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Following boundary | **FED-6-LAB setup lineage execution on one fresh isolated local devnet.** The V1 `no-submit` packet and its disposable keys remain frozen regression evidence and cannot authorize transport. Generate fresh synthetic source-attestation and Ergo-admission roles, retain their separation, and bind a new local-only authorization to the exact loopback target, process ownership, frozen transaction bytes and durable attempt. Check, sign, authorize, submit, observe mempool admission, confirm canonical inclusion and reconcile as distinct stages. Stop on target drift, non-loopback transport, unresolved prior attempt, source disagreement, transaction mutation or confirmation rollback. |
| Next implementation action | **Implement the separately versioned FED-6-LAB setup execution root and fail-closed local transport boundary.** Reuse the frozen accepted target/history/provisioning producers and existing signer/JVM-checker components without modifying their bytes or semantics. Add only the missing local authorization, `/transactions` submission, confirmation and reconciliation consumers; prove that the existing V1 `no-submit` result cannot enter them. After all three setup lineages confirm on the same process-owned target, move to the exact simulated-funds peg-in and peg-out paths, then the same-target restart/reorg/RPC-disagreement/database-loss campaign. Keep issues #1 and #2 open for external review and independently operated evidence. |
...
|---|---|---|
| Main implementation | FED-6-LAB isolated setup execution, value paths and recovery | Consume the frozen exact G1dA/G1dB accepted target, history and provisioning closure through a new local-only execution authorization. Preserve all V1 bytes and `no-submit` semantics. Establish the three setup lineages through exact signed JVM and same-target `/transactions/check`, separately authorized loopback submission, canonical confirmation and reconciliation. Only then execute separately approved simulated-funds peg-in and peg-out candidates, followed by restart, rollback, RPC-disagreement and database-loss recovery on that same target. |
