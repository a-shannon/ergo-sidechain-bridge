...
| Latest completed slice | **FED-6-LAB checkpoint attestation capability and dynamic mint-proof burn runner.** The retained V2 source-attestation session can now sign one exact versioned checkpoint statement under the same synthetic 2-of-3 profile used for mint. The V2 application runner accepts only the exact process-proven mint receipt, injects its profile, statement and proof envelope into the pinned Rust TestClient environment, commits those bytes in the execution-input digest, requires the exact proof-envelope digest marker from Rust, and binds the resulting application, sidechain, recipient and minted amount before returning a provenance-bound receipt. Focused negatives and TypeScript are green; the exact dynamic Rust execution remains deliberately pending the V3 end-to-end composition. This is not packet lifecycle ordering, source consensus/finality, tracker admission, replay insertion, payout, Gate 5, trustless status or readiness. |
...
| Following boundary | **FED-6-LAB V3 launch-to-checkpoint composition.** Retain one exact launched V2 session across packet construction, mint proof, dynamic application burn and checkpoint attestation. The wrapper must enforce `launch -> packet -> mint -> burn -> checkpoint`, consume exact receipt objects rather than copied fields, and reject cross-target application, block, root, amount or recipient identities. The result remains a disclosed federated policy decision, not GRANDPA/STARK verification or trustless finality. |
| Next implementation action | **Build and execute the V3 composition before tracker or payout work.** Reuse the frozen peg-in campaign producers, V2 dynamic runner and checkpoint-attestation capability in one fresh isolated campaign. Run the exact dynamic Rust path once, bind the burn receipt's native/execution block and `bridge_event_root` into the checkpoint statement, and prove failure cleanup plus one-shot capability consumption. Then compose tracker admission, global absent-to-present replay insertion and external-fee payout as separate joins. Reject wrong bridge/token/code identity, stale or mismatched checkpoint, root/proof drift, payout or amount drift, replay across every retained profile lineage, fee-from-backing, burn disappearance before transport, and every local-status or timeout payout path. Recovery follows the value-path composition; every public-network, real-funds, Gate-5, trustless and readiness boundary remains false. |
