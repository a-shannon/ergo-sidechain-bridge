...
| Latest completed slice | **FED-6-LAB same-process application-burn source-block checkpoint.** One source-locked Rust TestClient test deploys the reviewed application, consumes the exact federated reservation into mint, executes the real burn call, decodes the emitted PegOut fields, reconstructs the runtime leaf/root, and checks the net supply delta plus fee escrow. Overlay SHA-256 `b275a0e44306e465e61369d80763945e3e8a0cdf96fac2efcc7914f77eb53bb5` also exports the distinct Substrate native block height/hash and EVM execution block height/hash from the exact passing execution. The same-process runner binds the pinned source, tools, fresh target and stdout consumer, then removes the overlay before returning. This closes the local application-burn-to-source-block producer join only; source attestation/finality, tracker admission, replay insertion, payout, confirmation, Gate 5, trustless status and readiness remain open. |
...
| Following boundary | **FED-6-LAB burn-to-checkpoint source-attestation join.** Consume only the process-proven application-burn receipt, map its native block, execution block, application and root fields into the exact versioned federated checkpoint statement, and obtain the threshold source-attestation result under the already bound synthetic session. The result remains a disclosed federated policy decision, not GRANDPA/STARK verification or trustless finality. Tracker admission, global replay insertion, externally funded Ergo payout and confirmation follow as separate joins. |
