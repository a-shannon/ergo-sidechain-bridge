...
| Active internal slice | **FED-6-LAB - isolated local execution campaign, committed-vault transition stage.** Commit `3ace2682` repaired the process-local setup credential handoff without widening signer or transport authority. A fresh same-lifetime campaign then repeated the exact setup lineage, constructed one simulated-funds source-lock transaction, passed the pinned JVM and same-node `/transactions/check`, durably reserved it, broadcast it only to the owned loopback devnet and canonically confirmed transaction `866978ffe743e519f8d4fbacf1f4bb284e715348a61607d4ab2176cd9866b58f` at height `804`. Request SHA-256 `14f42fe80aabdababb2aaada9b15c2cba5487579b7da9e1e0bd31f5e8fb192a5` is bound to command receipt `8c6387204e674112ef2afcc3ce35bb517baf7fe7fa2c15000ced89a91a24313f`. The resulting source lock remains refundable: it was not consumed, no committed vault or reserve lineage was established, and mint remains unauthorized. The next objective is a fresh same-lifetime campaign that consumes the exact confirmed source lock into the exact non-refundable committed-vault/reserve successor under a separately authorized, checked and durably reconciled transition. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB refundable source-lock creation, local and self-operated.** The sole static root started one fresh loopback campaign, repeated and confirmed the setup lineage, constructed the exact simulated-funds source-lock candidate, retained one synthetic signer only through guarded WASM signing, passed the pinned JVM plus same-node `/transactions/check`, persisted the exact attempt before transport, broadcast only to the process-owned local node, and canonically confirmed transaction `866978ffe743e519f8d4fbacf1f4bb284e715348a61607d4ab2176cd9866b58f`. The tracked command bound request SHA-256 `14f42fe80aabdababb2aaada9b15c2cba5487579b7da9e1e0bd31f5e8fb192a5` to command receipt `8c6387204e674112ef2afcc3ce35bb517baf7fe7fa2c15000ced89a91a24313f` and worker receipt `b6df822e0c4b29e36b00a08502195f4248bf4b3394999ed4025478e9725e5238`; all owned processes and monitored listeners were absent afterward. Signed bytes were neither returned nor persisted. This proves creation and canonical local confirmation of one refundable source lock only: source consumption, committed-vault/reserve lineage, mint, funds authority, Gate 5, trustless status and readiness remain false. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row or injected port may expose a concrete signer or transport capability after the authorized run. Reward delay 1 or 720 remains digest-bound to the exact signer-owned reward proposition. The exact FED-6 reobservation invocation must use `NonMiningResume`, which requires `ResumeExistingDataDir`, rejects a runner-supplied mining-mnemonic reference and forces both primary and witness processes to non-mining mode; discovery and the separate observer use bounded GET-only requests and verify chain identity. This leaves the documented Phase 011b mining-enabled resume workflow unchanged and outside FED-6 evidence. **Owned paths:** the existing patched-devnet runner and no-submit conformance/check modules; the signer-first session, reward-input discovery and canonical history producers; the bounded read-only node client; `relayer-core/substrate-federated-local-devnet-genesis-execution-v1{.test,}.ts`; the generic operational lifecycle; and the StateTracker operational profile. **Current checks:** exact delay and reciprocal-mismatch matrices; deterministic signer-bound three-input selection with dual-source disagreement, ambiguity, immaturity and signer-drift negatives; canonical header/transaction/UTXO construction and mutation negatives; pinned JVM conformance; pure lifecycle ordering and provenance negatives; unresolved-attempt/restart journal constraints; runner source guards; Node 24 typecheck; layer imports; broadcast-surface isolation; the live exact-three local execution; the exact dual-origin non-mining reobservation; and the source-lock creation receipt above. The tracked setup CLI, sole static execution root, process-owned local transport and live port registration are closed for the exact LAB setup family. The value-path root now reaches exact source-lock signing, JVM/node checking, operation-specific authorization, durable reservation, loopback submission/broadcast and canonical confirmation while returning no signed bytes or capabilities. It still establishes no source consumption, committed-vault/reserve lineage or mint authority. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import and every peg-out or activation check remain unvalidated. |
| Following boundary | **FED-6-LAB committed-vault transition on one fresh isolated local devnet.** Repeat setup and refundable source-lock creation inside one retained target lifetime, then construct and separately authorize the exact checked transaction that consumes that source lock into the exact non-refundable committed-vault/reserve successor. Durably reserve before loopback transport, reconcile accepted/ambiguous outcomes, and canonically confirm exact source absence plus exact successor presence at policy depth. Stop on target drift, non-loopback transport, unresolved prior attempt, dual-node disagreement, transaction mutation, source restoration or confirmation rollback. Mint remains unavailable until this transition is independently revalidated. |
| Next implementation action | **Extend the sole FED-6-LAB execution root through the committed-vault transition, but stop before mint.** Compose the existing deterministic source-lock-to-reserve candidate and committed-vault verifier with a new exact operation-specific check, authorization, durable attempt and process-owned transport path. Bind every stage to the fresh campaign, confirmed source-lock box, expected successor identity, amount, asset, recipient, transaction ID and expiry; prove the earlier source-lock receipt and local rows cannot authorize this second transport; and require exact source consumption plus committed-vault/reserve successor agreement after canonical confirmation. Do not add mint, peg-out or recovery code until this producer-to-consumer join closes. Keep issues #1 and #2 open for external review and independently operated evidence. |
...
|---|---|---|
| Main implementation | FED-6-LAB value paths and recovery | The exact isolated setup lineage is closed at `620a075e`. Extend the same sole root through one fresh campaign that repeats setup and retains that target for a separately approved simulated-funds peg-in. After the deposit-to-committed-vault-to-mint join closes, add the independently authorized peg-out, then restart, rollback, RPC-disagreement and database-loss recovery. Preserve all V1 bytes and `no-submit` semantics. |
