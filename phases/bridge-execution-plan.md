...
| Active internal slice | **Return to the FED-6 external boundary after WP-08G hosted closure.** Public `main` now points to standalone commit `ee0686b84483e6f0af85c764e93b8a43383cc54a`, tree `da384d215ddbd87a23595fd871dd7d5c917502bd`. Exact hosted run `32009155091` rebuilt the pinned Frontier and Ergo sources and passed the complete public-audit candidate gate for those bytes. Reuse that receipt while its workflow, source locks, candidate tree and runner contract remain unchanged. Resume WP-06-FED only from an authentic reviewed target, independent custody and target-owned history; do not manufacture another synthetic lifecycle. The hosted result does not activate a profile, close Gate 5, establish custody, support a release, enable deployment or authorize funds. |
