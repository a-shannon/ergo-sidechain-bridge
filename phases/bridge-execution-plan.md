...
| Active internal slice | **FED-6-LAB - V137 terminal diagnosis and V138 source-boundary preparation.** V137 ran exactly once and failed closed at `source target binary and base spec`, before request creation or V11 invocation. The current batch preserves that historical phase for old-result projection but replaces the producer's aggregate phase with five finite boundaries: built-binary artifact, binary identity/version, base-spec process, stderr policy and byte-exact reproduction. It changes no accepted bytes, contracts, proof formats, transaction construction, process behavior or authority. After this batch is committed, publicly promoted and green on the exact pull-request head, V138 must use a wholly fresh chain-42 synthetic identity, spec, ports and mutable roots. Gate 5, funds authority, trustless status, deployment readiness and support for public funds remain false. |
| Public coordination | Public `main` is protected by both required CI jobs, one approving review from someone other than the last pusher, stale-review dismissal, conversation resolution, linear history and disabled force-push/deletion. Pull request #3 is open and mergeable into `a-shannon/research-alpha`; it is not merged. Its exact head `8ace4a341930bb46acf110ee5f03c9dc9964e440` passed push run `33818391759` and pull-request run `33818394669`, four successful jobs in total. The finer source-boundary batch must replace that head only through the guarded non-force promotion flow and receive its own exact-head hosted closure before V138 begins. [Issue #1](https://github.com/a-shannon/ergo-sidechain-bridge/issues/1) owns independent review of exact commit `ee0686b84483e6f0af85c764e93b8a43383cc54a`; [issue #2](https://github.com/a-shannon/ergo-sidechain-bridge/issues/2) owns external target and custody evidence for a later independently operated FED-6 campaign. Private vulnerability reporting is enabled. The public issues remain parallel assurance lanes and do not authorize or describe FED-6-LAB. |
...
| Latest completed slice | **FED-6-LAB V137 terminal binary/base-spec diagnosis.** V137 passed pure validation and exact-head promotion checks, then was invoked exactly once. It stopped at `genesis_calibration` with finite source phase `source target binary and base spec` after creating only the authority-safe chain spec. Request, receipt and artifact outputs remained absent, the journal remained empty, no Frontier/Cargo/Rust process survived, the memory-only synthetic key was not persisted or printed, and V11 was never invoked. V137 is terminal and none of its identity, spec, ports or mutable roots may be reused. This result changes no proof, contract, transaction, authorization, funds authority or release claim. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row, injected tool path, error message or injected port may expose a reusable signer or transport capability or become funds authority. V7 freezes the dual-node read-only target and same-target JVM check; V8 persists and revalidates the exact reservation across a real file-backed close/reopen boundary; V9 creates one single-use transport authorization and durable attempt, then persists an exact accepted or ambiguous V1 outcome before confirmation. V10 preserves those boundaries while starting the primary miner before the sole POST and requiring the same primary/witness process identities through canonical confirmation. V11 preserves all historical bytes and adds a separately digest-bound process-local response classification; a successful HTTP response is still not canonical admission. **Frozen inputs:** exact V137 bridge/source, patch, toolchain, build-report, base-spec and runtime identities; all V112-V137 mutable campaign state is terminal or abandoned and ineligible for reuse. **Reusable checks:** build-environment, V11 receipt, WP-08A dependency, WP-07 containment, exact source/patch, setup/check, dual-layout, deadline-composition and offline application-runner matrices remain reusable while their transitive inputs stay unchanged. **Changed-input closure:** only the finite source-phase vocabulary, acceptance phase assignments, direct phase-isolation tests and this checkpoint change in the current batch; the compatibility API, accepted bytes, proof formats, contracts, transaction bytes, process implementation and source build remain unchanged. **Next due execution:** after exact source promotion and hosted checks, one fresh V138 lineage must either project one finer finite binary/base-spec failure phase or reach canonical tracker confirmation. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import, peg-out, recovery and activation remain unvalidated. |
| Following boundary | **FED-6-LAB V138 fresh classified application-checkpoint campaign.** Never retry V137 or consume any earlier mutable state. After the finer source-boundary classifier is committed, published and green on the exact pull-request head, create one wholly fresh request, ports, processes, storage and authorization lineage. Run the complete owned application-checkpoint path once and require either canonical tracker confirmation or one terminal finite source phase without emitting arbitrary diagnostics. |
| Next implementation action | **Close and promote the five-way binary/base-spec classifier, then run V138 once.** Run the focused acceptance matrix, TypeScript, architecture check and independent review; stage only the two classifier/acceptance modules, direct test and this plan checkpoint; run staged and exact-range publication guards; update pull request #3 through the guarded non-force push; and wait for exact-head hosted checks. Then create the wholly fresh local-only V138 lineage. If V138 canonically confirms the exact tracker successor, move directly to burn/checkpoint binding, global replay insertion, external-fee payout and profile-specific recovery. Public networks, existing wallet material, real funds, Gate 5, trustless status and readiness remain outside this LAB action. |
...
```text
frozen foundations
  -> canonical tracker admission (V137 isolated the binary/base-spec block; promote its five finite subphases, then use one wholly fresh bounded V138 campaign)
...
|---|---|---|---|
| V137 source/build inputs and locked tools | Exact bridge/source, patch, command, tool, cache-source, build-report, base-spec and runtime identities | Source relocation or any source, patch, command, toolchain, cache-source or generated-byte drift | Reuse immutable pins for focused root-cause falsification; rebuild only on drift and never reuse V137 mutable campaign state or identity |
...
| Mutable local campaign evidence | One exact fresh process and storage lineage | Terminal attempt, process replacement, request drift or target-state change | V112-V133 and V135-V137 are terminal; V134 was abandoned. Create V138 only after the five-way source-phase projection is promoted, and never reuse an earlier attempt as authority |
