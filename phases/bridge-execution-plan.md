...
| Active internal slice | **FED-6-LAB - isolated local execution campaign, value-path stage.** The exact three-transaction setup check, signing, separately authorized loopback submission and canonical confirmation are complete on a fresh task-owned devnet through commit `620a075e` and command receipt `4047dd3355dc75fb0e00faefd9d3208b5d6a11c5864e67f508527ad0cbac3779`. The next objective is one exact simulated-funds peg-in lifecycle, followed by peg-out and recovery. Every capability must bind the exact process-owned target, transaction bytes, authorization, durable attempt and confirmation policy. The setup receipt cannot authorize a later value path, and its ephemeral target state is not reusable. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB exact setup lineage execution, local and self-operated.** The sole static root started one fresh loopback target, created fresh synthetic roles, rebuilt the frozen setup candidates, signed and checked them, durably reserved each separately authorized attempt, submitted the three transactions in dependency order and confirmed every exact output at policy depth before teardown. The tracked command bound request SHA-256 `f8f3a6a73a337b21b34fbd0824f388e7d980bda2acd5dcd43b6524ecd6488c10` to command receipt `4047dd3355dc75fb0e00faefd9d3208b5d6a11c5864e67f508527ad0cbac3779`; all owned processes and monitored listeners were absent afterward. This closes only the isolated LAB setup lineage, not value paths, independent custody, source consensus, Gate 5, trustless status or readiness. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row or injected port may expose a concrete signer or transport capability after the authorized run. Reward delay 1 or 720 remains digest-bound to the exact signer-owned reward proposition. The exact FED-6 reobservation invocation must use `NonMiningResume`, which requires `ResumeExistingDataDir`, rejects a runner-supplied mining-mnemonic reference and forces both primary and witness processes to non-mining mode; discovery and the separate observer use bounded GET-only requests and verify chain identity. This leaves the documented Phase 011b mining-enabled resume workflow unchanged and outside FED-6 evidence. **Owned paths:** the existing patched-devnet runner and no-submit conformance/check modules; the signer-first session, reward-input discovery and canonical history producers; the bounded read-only node client; `relayer-core/substrate-federated-local-devnet-genesis-execution-v1{.test,}.ts`; the generic operational lifecycle; and the StateTracker operational profile. **Current checks:** exact delay and reciprocal-mismatch matrices; deterministic signer-bound three-input selection with dual-source disagreement, ambiguity, immaturity and signer-drift negatives; canonical header/transaction/UTXO construction and mutation negatives; pinned JVM conformance; pure lifecycle ordering and provenance negatives; unresolved-attempt/restart journal constraints; runner source guards; Node 24 typecheck; layer imports; broadcast-surface isolation; the live exact-three local execution; and the exact dual-origin non-mining reobservation above. The tracked setup CLI, sole static execution root, process-owned local transport and live port registration are now closed for the exact LAB setup family. No value-path composition root or authorization exists yet. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import and every peg-in, peg-out or activation check remain unvalidated. |
| Following boundary | **FED-6-LAB simulated-funds peg-in on one fresh isolated local devnet.** Start a new target lifetime, replay the exact setup lineage inside that lifetime, then construct one exact deposit-to-committed-vault-to-mint candidate. Preserve the refundable-source exclusion, exact vault successor, amount/asset/recipient and mint identity bindings. Check, sign, authorize, reserve, submit, confirm and reconcile as distinct stages. Stop on target drift, non-loopback transport, unresolved prior attempt, source disagreement, transaction mutation, restored refundable source or confirmation rollback. |
| Next implementation action | **Extend the sole FED-6-LAB execution root through one exact peg-in lifecycle.** Reuse the frozen committed-vault verifier and existing process-owned signer/checker/transport boundaries without changing their bytes or semantics. Keep the target alive after setup, derive the value candidate only from the canonically confirmed setup outputs and fresh source observations, require a new operation-specific authorization, and prove that neither the setup receipt nor local journal state can authorize mint. Do not add peg-out or recovery code until this producer-to-consumer join closes. Keep issues #1 and #2 open for external review and independently operated evidence. |
...
reused as authority. Step 4 now starts with one fresh campaign that repeats
setup and keeps the same process-owned target alive for one independently
authorized deposit-to-committed-vault-to-mint lifecycle. Peg-out and recovery
