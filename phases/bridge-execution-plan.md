...
| Active internal slice | **FED-6-LAB - V134 fresh one-shot lineage in preparation.** V133 remains terminal and none of its mutable state is reused. Its volume-root runtime ownership defect was corrected and promoted at exact commit `191c11b3bc786a5f54d9cf890e139cad07a4e141`. V134 uses fresh source checkouts, immutable cache copies, toolchain pins, ports, request, runtime roots, journal, artifacts, receipts and synthetic identities. The request producer, pure preflight and V11 worker bind one nonzero request-bound bridge owner, signed-probe signer and mint recipient while separately requiring the exact deterministic base Sudo identity removed by authority-safe genesis construction; owner, bridge and token must be pairwise distinct. The V1 fixed-owner rule remains unchanged for its terminal V10 compatibility worker, so a dynamic-owner request fails closed there and is executable only under the V11-or-later owner policy. V11 accepts its legacy mechanism projection only inside the complete V11 envelope; extracting that projection and presenting it to either standalone V10 parser fails closed under the fixed V10 owner policy. V11 receives only public pre-signed bytes and no wallet secret: its historical `existingWalletMaterialUsed=false` boundary describes worker capability, not proof that the external signer key was freshly generated. V134 preparation must generate that one-shot signer in memory. The next boundary is a fresh chain-42 synthetic identity, regenerated authority-safe chain spec and native genesis, followed by canonical request creation and pure preflight. V11 may be invoked at most once only after that preflight passes. Gate 5, funds authority, trustless status, deployment readiness, and support for public funds remain false. |
| Public coordination | Public `main` is protected by both required CI jobs, one approving review from someone other than the last pusher, stale-review dismissal, conversation resolution, linear history and disabled force-push/deletion. Pull request #3 is open and mergeable at exact head `191c11b3bc786a5f54d9cf890e139cad07a4e141`; push run `33765908403` and pull-request run `33765915163` passed both required jobs. [Issue #1](https://github.com/a-shannon/ergo-sidechain-bridge/issues/1) owns independent review of exact commit `ee0686b84483e6f0af85c764e93b8a43383cc54a`; [issue #2](https://github.com/a-shannon/ergo-sidechain-bridge/issues/2) owns external target and custody evidence for a later independently operated FED-6 campaign. Private vulnerability reporting is enabled. The public issues remain parallel assurance lanes and do not authorize or describe FED-6-LAB. |
...
| Latest completed slice | **FED-6-LAB V133 volume-root runtime-boundary closure.** A wholly fresh V133 lineage passed pure preflight and invoked V11 exactly once, then failed closed at `ergo node startup runtime creation` before any command receipt or campaign artifact existed. The process adapter inherited `TEMP`/`TMP` from the parent of a drive-root worktree, making the volume root a valid temporary root; the cleanup guard nevertheless appended another separator and rejected the owned `mkdtempSync` child. A red regression isolated that normalization defect. The corrected predicate preserves strict descendant and dedicated namespace checks while accepting an already separator-terminated volume or UNC root. The affected closure passed 503 tests with two unchanged live skips, TypeScript and the 128-module/846-file architecture check; the exact two-node live lifecycle also passed and cleaned up its processes, listeners and runtime. Independent review found no P1-P3 issue. This closure changes only local runtime ownership normalization, not proofs, contracts, transaction bytes, authorization, funds authority or release claims. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row, injected tool path, error message or injected port may expose a reusable signer or transport capability or become funds authority. V7 freezes the dual-node read-only target and same-target JVM check; V8 persists and revalidates the exact reservation across a real file-backed close/reopen boundary; V9 creates one single-use transport authorization and durable attempt, then persists an exact accepted or ambiguous V1 outcome before confirmation. V10 preserves those boundaries while starting the primary miner before the sole POST and requiring the same primary/witness process identities through canonical confirmation. V11 preserves all historical bytes and adds a separately digest-bound process-local response classification; a successful HTTP response is still not canonical admission. **Frozen inputs:** exact V133 source, patch, tool, base-spec, runtime, authority-safe-spec, native-genesis and request identities. **Reusable checks:** build-environment, V11 receipt, WP-08A dependency, WP-07 containment, exact source/patch, setup/check, dual-layout, deadline-composition and offline application-runner matrices remain reusable while their transitive inputs stay unchanged. All V112-V133 mutable campaign state is terminal and ineligible for reuse. **Changed-input closure:** owned-runtime path normalization, startup-phase consumer tests, the exact real two-node lifecycle, TypeScript and the architecture boundary were run once for the stable two-file source slice; no WASM, JVM contract, proof-vector or broad suite replay is caused by this local runtime-path correction. **Next due execution:** after exact source promotion and hosted checks, one fresh V134 lineage must prove whether the application path reaches canonical tracker confirmation. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import, peg-out, recovery, and activation remain unvalidated. |
| Following boundary | **FED-6-LAB V134 fresh application-checkpoint campaign.** Never retry V133 or consume its mutable state. After the reviewed volume-root correction is committed, published and green on the exact PR head, create one wholly fresh request, ports, processes, storage and authorization lineage. Re-run the complete owned application-checkpoint path once and require either a canonical tracker confirmation or one terminal typed failure. |
| Next implementation action | **Promote the volume-root closure, then run V134 once.** Commit the reviewed two-file runtime-boundary correction with the current plan checkpoint, run staged and exact-range publication guards, update pull request #3 through the guarded non-force push, and wait for exact-head hosted checks. Only then create the wholly fresh local-only V134 lineage. If V134 canonically confirms the exact tracker successor, move directly to burn/checkpoint binding, global replay insertion, external-fee payout, and profile-specific recovery rather than another evidence-only slice. Public networks, existing wallet material, real funds, Gate 5, trustless status, and readiness remain outside this LAB action. |
...
```text
frozen foundations
  -> canonical tracker admission (V133 stopped during owned runtime creation; promote the reviewed volume-root fix, then use the wholly fresh bounded V134 campaign)
...
|---|---|---|---|
| V133 source/build inputs and locked tools | Exact source, patch, command, tool, cache-source, base-spec, runtime, authority-safe-spec and native-genesis identities | Source relocation or any source, patch, command, toolchain, cache-source or generated-byte drift | Reuse for focused root-cause falsification; rebuild only on drift and never reuse V133 mutable campaign state |
...
| Mutable local campaign evidence | One exact fresh process and storage lineage | Terminal attempt, process replacement, request drift or target-state change | V112-V133 are terminal; create V134 only after the volume-root cause is promoted and never reuse a terminal attempt as authority |
