...
| Active internal slice | **FED-6-LAB - receipt-derived end-to-end peg-in campaign.** The application root now composes setup, refundable source lock, committed-reserve transition, canonical mint draft, source-owned evidence collection, one-shot V2 packet proof, and source-locked atomic Frontier TestClient consumption inside one owned node/process lifetime. Only this new action selects the V2 continuation; every historical root retains its V1 packet behavior. Exact object and digest joins bind the committed reserve through the final consumer, and consumer failure still closes the journal, packet/setup sessions, and node. Focused orchestration tests establish that composition and teardown, but use mocked component implementations: no fresh real-component campaign receipt exists yet. The next objective is a config-free local command that resolves the exact reviewed Frontier/toolchain inputs and executes this root once without fixture substitution, followed by a sanitized digest-bound receipt. The disclosed finality object remains a dual-RPC depth policy and does not authenticate Ergo proof of work. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
