...
| Active internal slice | **FED-6-LAB - V141 terminal diagnosis and V142 node/runtime-boundary preparation.** V141 ran exactly once from integrated commit `8c146ac89d11fbca86aab16a5dfa237e809e52f0`, crossed the V140 genesis-calibration parser blocker, created its canonical request and entered V11, then failed closed on the explicitly untrusted worker phase hint `source target node identity and runtime validation`. The current batch replaces only that broad diagnostic phase with five finite source-owned boundaries: node identity, peer health, EVM chain identity, runtime-version decoding and exact runtime-code validation. Every acceptance predicate and accepted byte remains unchanged. After this batch is committed, publicly promoted and green on the exact pull-request head, V142 must use a wholly fresh chain-42 synthetic identity, spec, ports and mutable roots. Gate 5, funds authority, trustless status, deployment readiness and support for public funds remain false. |
| Public coordination | Public `main` remains protected by both required CI jobs, one approving review from someone other than the last pusher, stale-review dismissal, conversation resolution, linear history and disabled force-push/deletion. Pull request #3 merged into `a-shannon/research-alpha` as exact commit `8c146ac89d11fbca86aab16a5dfa237e809e52f0`; integration run `33873203171` passed the dependency audit, pinned-source rebuild and public-audit gate. The node/runtime-boundary batch starts from that exact green commit and must use a new guarded non-force feature-branch promotion plus exact-head hosted closure before V142 begins. [Issue #1](https://github.com/a-shannon/ergo-sidechain-bridge/issues/1) owns independent review of exact commit `ee0686b84483e6f0af85c764e93b8a43383cc54a`; [issue #2](https://github.com/a-shannon/ergo-sidechain-bridge/issues/2) owns external target and custody evidence for a later independently operated FED-6 campaign. Private vulnerability reporting is enabled. The public issues remain parallel assurance lanes and do not authorize or describe FED-6-LAB. |
...
| Latest completed slice | **FED-6-LAB V141 terminal node/runtime diagnosis.** V141 passed pure validation, strict TypeScript, exact-head integration checks and independent critical review, then was invoked exactly once. It used a fresh patched Ergo checkout, recovered the canonical genesis through the strict homogeneous-aggregate parser, created a 3,948,062-byte authority-safe spec and a 2,567-byte canonical request, and entered V11. It then stopped on the untrusted worker phase hint `source target node identity and runtime validation`. Receipt and artifact outputs remained absent; one journal entry and one opaque calibration entry are terminal and uninspected; no Node, Frontier, Cargo, Rust or Java process survived. V141 is terminal and none of its identity, request, spec, ports or mutable roots may be reused. This result changes no proof, contract, transaction, authorization, funds authority or release claim. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row, injected tool path, error message or injected port may expose a reusable signer or transport capability or become funds authority. V7 freezes the dual-node read-only target and same-target JVM check; V8 persists and revalidates the exact reservation across a real file-backed close/reopen boundary; V9 creates one single-use transport authorization and durable attempt, then persists an exact accepted or ambiguous V1 outcome before confirmation. V10 preserves those boundaries while starting the primary miner before the sole POST and requiring the same primary/witness process identities through canonical confirmation. V11 preserves all historical bytes and adds a separately digest-bound process-local response classification; a successful HTTP response is still not canonical admission. **Frozen inputs:** exact integrated bridge commit `8c146ac89d11fbca86aab16a5dfa237e809e52f0`, source, patch, toolchain, build-report, base-spec and runtime identities; V141 request/spec and all V112-V141 mutable campaign state are terminal or abandoned and ineligible for reuse, with V134 abandoned before request creation. **Reusable checks:** build-environment, V11 receipt, WP-08A dependency, WP-07 containment, exact source/patch, setup/check, dual-layout, deadline-composition and offline application-runner matrices remain reusable while their transitive inputs stay unchanged. **Changed-input closure:** only the five-way source-failure vocabulary, observation phase assignments, isolated negative fixtures and this checkpoint; the compatibility API, accepted bytes, proof formats, contracts, transaction bytes, process implementation and source build remain unchanged. **Next due execution:** after exact source promotion and hosted checks, one fresh V142 lineage must either project one finer finite node/runtime failure phase or reach canonical tracker confirmation. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import, peg-out, recovery and activation remain unvalidated. |
| Following boundary | **FED-6-LAB V142 fresh node/runtime-classified application-checkpoint campaign.** Never retry V141 or consume any earlier mutable state. After the five-way node/runtime classifier is committed, published and green on the exact pull-request head, create one wholly fresh request, ports, processes, storage and authorization lineage. Run the complete owned application-checkpoint path once and require either canonical tracker confirmation or one terminal finite source phase without emitting arbitrary diagnostics. |
| Next implementation action | **Close and promote the five-way node/runtime classifier, then run V142 once.** Run the focused observation/acceptance and direct-consumer matrices, TypeScript, architecture check and independent review; stage only the source-failure registry, observation module, their direct tests and this plan checkpoint; run staged and exact-range publication guards; create a new guarded feature-branch pull request; and wait for exact-head hosted checks. Then create the wholly fresh local-only V142 lineage. If V142 canonically confirms the exact tracker successor, move directly to burn/checkpoint binding, global replay insertion, external-fee payout and profile-specific recovery. Public networks, existing wallet material, real funds, Gate 5, trustless status and readiness remain outside this LAB action. |
...
```text
frozen foundations
  -> canonical tracker admission (V141 crossed genesis calibration and request construction; promote the five finite node/runtime subphases, then use one wholly fresh bounded V142 campaign)
...
|---|---|---|---|
| V141 source/build/request inputs and locked tools | Exact bridge/source root, patch, command, tool, cache-source, build-report, base-spec, runtime and canonical request identities | Source relocation or any source, patch, command, toolchain, cache-source, generated-byte or request drift | Reuse immutable pins for focused root-cause falsification; rebuild only on drift and never reuse V141 mutable campaign state or identity |
...
| Mutable local campaign evidence | One exact fresh process and storage lineage | Terminal attempt, process replacement, request drift or target-state change | V112-V133 and V135-V141 are terminal; V134 was abandoned. Create V142 only after the five-way node/runtime phase projection is promoted, and never reuse an earlier attempt as authority |
