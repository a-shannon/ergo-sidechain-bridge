...
| Active internal slice | **FED-6-LAB - V139 terminal diagnosis and V140 observation-boundary preparation.** V139 ran exactly once, reused the exact retained V135 source root, reproduced the accepted base-spec bytes and created its canonical request, then failed closed at the broad historical phase `source target readiness and observation`. The current batch preserves that historical phase and default fail-fast behavior for compatibility while adding twelve finite source-owned observation boundaries: input/source binding, deployment identity, native/EVM tip, RPC snapshot, node/runtime identity, application identity, owner-mint quarantine, top-trie policy, tip stability, two-node finalization, provenance and generated-target join. Classified two-node observation deterministically lets a failure that is non-retryable under the bounded snapshot retry policy dominate a simultaneous retryable failure and maps incompatible concurrent deciding phases to the bounded finalization phase. It changes no accepted bytes, contracts, proof formats, transaction construction, network methods or authority. After this batch is committed, publicly promoted and green on the exact pull-request head, V140 must use a wholly fresh chain-42 synthetic identity, spec, ports and mutable roots. Gate 5, funds authority, trustless status, deployment readiness and support for public funds remain false. |
| Public coordination | Public `main` is protected by both required CI jobs, one approving review from someone other than the last pusher, stale-review dismissal, conversation resolution, linear history and disabled force-push/deletion. Pull request #3 is open and mergeable into `a-shannon/research-alpha`; it is not merged. Its exact head `176271a6d25f1ddd848c4bdae46fca14861d7c41` passed push run `33826889781` and pull-request run `33826891796`, four successful jobs in total. The observation-boundary batch must replace that head only through the guarded non-force promotion flow and receive its own exact-head hosted closure before V140 begins. [Issue #1](https://github.com/a-shannon/ergo-sidechain-bridge/issues/1) owns independent review of exact commit `ee0686b84483e6f0af85c764e93b8a43383cc54a`; [issue #2](https://github.com/a-shannon/ergo-sidechain-bridge/issues/2) owns external target and custody evidence for a later independently operated FED-6 campaign. Private vulnerability reporting is enabled. The public issues remain parallel assurance lanes and do not authorize or describe FED-6-LAB. |
...
| Latest completed slice | **FED-6-LAB V139 terminal target-observation diagnosis.** V139 passed pure validation, strict TypeScript, independent critical review and exact-head promotion checks, then was invoked exactly once. It reused the exact retained V135 source root, reproduced the accepted base-spec bytes, created a 3,948,062-byte authority-safe spec and its canonical request, then stopped at `genesis_calibration` with the broad historical source phase `source target readiness and observation`. Receipt and artifact outputs remained absent, the journal and opaque calibration roots each contain one non-reusable entry, and no Frontier/Cargo/Rust/Java process survived. V139 is terminal and none of its identity, request, spec, ports or mutable roots may be reused. This result changes no proof, contract, transaction, authorization, funds authority or release claim. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row, injected tool path, error message or injected port may expose a reusable signer or transport capability or become funds authority. V7 freezes the dual-node read-only target and same-target JVM check; V8 persists and revalidates the exact reservation across a real file-backed close/reopen boundary; V9 creates one single-use transport authorization and durable attempt, then persists an exact accepted or ambiguous V1 outcome before confirmation. V10 preserves those boundaries while starting the primary miner before the sole POST and requiring the same primary/witness process identities through canonical confirmation. V11 preserves all historical bytes and adds a separately digest-bound process-local response classification; a successful HTTP response is still not canonical admission. **Frozen inputs:** exact V139 bridge/source, patch, toolchain, build-report, base-spec, runtime and request identities; all V112-V139 mutable campaign state is terminal or abandoned and ineligible for reuse, with V134 abandoned before request creation. **Reusable checks:** build-environment, V11 receipt, WP-08A dependency, WP-07 containment, exact source/patch, setup/check, dual-layout, deadline-composition and offline application-runner matrices remain reusable while their transitive inputs stay unchanged. **Changed-input closure:** only the finite source-failure vocabulary, opt-in observation and acceptance phase assignments, direct negative tests and this checkpoint change in the current batch; the compatibility API, accepted bytes, proof formats, contracts, transaction bytes, process implementation and source build remain unchanged. **Next due execution:** after exact source promotion and hosted checks, one fresh V140 lineage must either project one finer finite target-observation failure phase or reach canonical tracker confirmation. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import, peg-out, recovery and activation remain unvalidated. |
| Following boundary | **FED-6-LAB V140 fresh classified application-checkpoint campaign.** Never retry V139 or consume any earlier mutable state. After the observation-boundary classifier is committed, published and green on the exact pull-request head, create one wholly fresh request, ports, processes, storage and authorization lineage. Run the complete owned application-checkpoint path once and require either canonical tracker confirmation or one terminal finite source phase without emitting arbitrary diagnostics. |
| Next implementation action | **Close and promote the twelve-way target-observation classifier, then run V140 once.** Run the focused observation/acceptance and direct-consumer matrices, TypeScript, architecture check and independent review; stage only the source-failure registry, observation/acceptance modules, their direct tests and this plan checkpoint; run staged and exact-range publication guards; update pull request #3 through the guarded non-force push; and wait for exact-head hosted checks. Then create the wholly fresh local-only V140 lineage. If V140 canonically confirms the exact tracker successor, move directly to burn/checkpoint binding, global replay insertion, external-fee payout and profile-specific recovery. Public networks, existing wallet material, real funds, Gate 5, trustless status and readiness remain outside this LAB action. |
...
```text
frozen foundations
  -> canonical tracker admission (V139 crossed exact base-spec reproduction and isolated target observation; promote its twelve finite subphases, then use one wholly fresh bounded V140 campaign)
...
|---|---|---|---|
| V139 source/build/request inputs and locked tools | Exact bridge/source root, patch, command, tool, cache-source, build-report, base-spec, runtime and canonical request identities | Source relocation or any source, patch, command, toolchain, cache-source, generated-byte or request drift | Reuse immutable pins for focused root-cause falsification; rebuild only on drift and never reuse V139 mutable campaign state or identity |
...
| Mutable local campaign evidence | One exact fresh process and storage lineage | Terminal attempt, process replacement, request drift or target-state change | V112-V133 and V135-V139 are terminal; V134 was abandoned. Create V140 only after the twelve-way observation-phase projection is promoted, and never reuse an earlier attempt as authority |
