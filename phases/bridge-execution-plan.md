...
| Active internal slice | **FED-6-LAB - canonical committed-reserve evidence production.** A process-proven V2 packet now carries the exact 603-byte reservation statement, selected source-proof profile SCALE bytes, and source-proof envelope into the source- and toolchain-locked `runtime-v4-lab` TestClient. TypeScript re-decodes and binds the runtime profile, source-proof profile, statement, mint identity, and proof envelope; the patched Rust test consumes the exact dynamic envelope, rejects an unreserved sibling without state drift, and consumes only the corrected direct-parent reservation. The named one-test result and dynamic proof digest are required. Cargo/Rust/Git executable hashes are authenticated before their first invocation, Cargo receives a fresh consumer-owned target and config-free home, the Cargo process tree is contained and closed before workspace cleanup, and source/tool identities are revalidated after execution. The complete build-tool closure and shared dependency cache remain unattested. Pre/post hashing is not an atomic source/tool snapshot and assumes exclusive execution by a non-adversarial local OS user. This closes packet-to-runtime proof transport only. The committed-reserve evidence fields inside that proof remain caller-attested opaque bytes. The next objective is a source-owned collector that derives those fields from the actually confirmed reserve transition and checkpoint before the same packet/consumer path runs. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB packet-bound dynamic proof consumption.** The packet receipt now retains the canonical selected-profile SCALE bytes and the V2 consumer re-decodes every profile, statement, mint-identity, and proof binding before invoking the exact source-locked Frontier test. Pinned Cargo 1.82.0, Rustc 1.82.0 and Git 2.54.0 executable hashes are authenticated before first use; Cargo runs offline in a fresh consumer-owned target with a config-free home and a contained process tree that closes before cleanup. One exact atomic mint test and one proof-byte SHA-256 marker must appear, and source/tool drift after execution rejects without consuming the packet capability. Concurrent use of the same packet capability is rejected. The Rust matrix still permits its historical fixture only when no dynamic proof is supplied, so the dynamic run cannot be confused with fallback evidence. Complete build-tool and shared dependency-cache attestation remain false; pre/post hashes are not an atomic snapshot and require exclusive non-adversarial same-user execution. This closes the packet-to-TestClient transport and atomic reservation/mint join, not the production of canonical source evidence. The evidence digests remain caller-attested, and funds authority, Gate 5, trustless status and readiness remain false. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row or injected port may expose a concrete signer or transport capability after the authorized run. Reward delay 1 or 720 remains digest-bound to the exact signer-owned reward proposition. The exact FED-6 reobservation invocation must use `NonMiningResume`, which requires `ResumeExistingDataDir`, rejects a runner-supplied mining-mnemonic reference and forces both primary and witness processes to non-mining mode; discovery and the separate observer use bounded GET-only requests and verify chain identity. This leaves the documented Phase 011b mining-enabled resume workflow unchanged and outside FED-6 evidence. **Owned paths:** the existing patched-devnet runner and no-submit conformance/check modules; the signer-first session, reward-input discovery and canonical history producers; the bounded read-only node client; `relayer-core/substrate-federated-local-devnet-genesis-execution-v1{.test,}.ts`; the generic operational lifecycle; and the StateTracker operational profile. **Current checks:** exact delay and reciprocal-mismatch matrices; deterministic signer-bound three-input selection with dual-source disagreement, ambiguity, immaturity and signer-drift negatives; canonical header/transaction/UTXO construction and mutation negatives; pinned JVM conformance; pure lifecycle ordering and provenance negatives; unresolved-attempt/restart journal constraints; runner source guards; Node 24 typecheck; layer imports; broadcast-surface isolation; the live exact-three local execution; the exact dual-origin non-mining reobservation; the source-lock creation receipt; and the committed-vault execution receipt above. The tracked setup CLI, sole static execution root, process-owned local transport and live port registration are closed for the exact LAB setup family. The value-path root now reaches exact source-lock creation plus the separately checked, authorized, durably reserved, locally transported and canonically confirmed source-lock-to-reserve transition while returning no signed bytes or capabilities. It establishes exact local source consumption and committed-reserve lineage, but no mint authority. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import and every mint, peg-out or activation check remain unvalidated. |
| Following boundary | **FED-6-LAB canonical committed-reserve evidence collector.** In one source-owned same-lifetime campaign, derive the exact source-lock box, reserve-transition transaction, successor reserve box, inclusion, checkpoint ancestry, and disclosed federated-finality evidence from the confirmed Ergo transition rather than accepting those digests from the caller. Bind that collector output to the existing V2 request and packet proof, then reuse the already closed source-locked atomic Frontier consumer. Stop before peg-out. |
| Next implementation action | **Implement the committed-reserve evidence collector and its canonical codec.** Consume the process-proven committed-reserve execution/confirmation receipt, reconstruct every proof-result digest from its retained canonical bytes and chain coordinates, and reject a caller-provided replacement, missing retained input, lineage mismatch, stale checkpoint, or cross-campaign splice. Feed only that collector result into the existing source-attestation session and packet producer. Do not rerun or redesign the atomic Frontier consumer unless this producer changes one of its exact inputs. Peg-out and recovery remain out of scope, and every public-network, funds, Gate-5, trustless and readiness boundary remains false. Keep issues #1 and #2 open for external review and independently operated evidence. |
...
|---|---|---|
| Main implementation | FED-6-LAB value paths and recovery | The exact isolated setup lineage is closed at `620a075e`, and one fresh same-lifetime campaign at source checkpoint `49218bcd` has now closed refundable source-lock creation plus exact committed-reserve transition while stopping before mint. Close the committed-reserve-to-federated-mint join next without restoring owner-key authority. Then add the independently authorized peg-out, followed by restart, rollback, RPC-disagreement and database-loss recovery. Preserve all V1 bytes and `no-submit` semantics. |
...
|---|---|---|---|
| **WP-06-FED - federated reference** | One statically registered profile binds two explicit cryptographic roles: the source-attestation Ed25519 key set and the Ergo-admission SigmaProp key set, each with its own digest and threshold under one federation epoch. Their respective authority over mint and payout is disclosed; neither role is implied to be the other. | Functional institutional reference profile under a federated trust model; never trustless. | **Migration execution remains at the FED-6 external boundary; the local packet proof now reaches an atomic reserved mint in a source-locked TestClient.** FED-1 through migration-only FED-5B plus the distinct greenfield baseline/generation/provisioning and fresh-process replay joins are complete as non-authorizing checkpoints. One self-operated Ergo campaign has signed, checked, submitted and confirmed the setup, refundable source-lock and source-lock-to-reserve lineages through source checkpoint `49218bcd`; separately, the V2 packet consumer replays the exact selected profile, statement, proof envelope, source lock, toolchain, and direct-parent atomic mint matrix. The source evidence digests are still caller-attested, so these remain separate evidence halves rather than a receipt-derived end-to-end peg-in. A canonical committed-reserve evidence collector, peg-out, recovery, independent custody and the externally approved migration campaign remain open. |
...
| **WP-06-FED** | EIP-independent federated peg-in and peg-out profile | WP-01/04/05 foundations plus reviewed V4/V6 conservation and WP-07 lifecycle | Federated statement/tracker, peg-in admission, settlement family, daemon integration, and cutover are sequential authority joins; disjoint target/retirement evidence work may run in parallel only when real inputs exist | **Migration targets remain at the FED-6 external boundary. FED-6-LAB now has two separately verified halves: exact committed-reserve confirmation through source checkpoint `49218bcd`, and a packet-bound source-proof envelope consumed by the source-locked TestClient direct-parent atomic mint path. FED-1 through FED-4, FED-5A/B migration generation, and the distinct greenfield baseline/generation/provisioning/portable-replay joins remain frozen. The proof envelope still receives caller-attested evidence digests; the actual committed-reserve receipt has not yet produced those bytes through a source-owned collector in one fresh paired campaign. Peg-out, recovery, static registration, independent custody and externally approved target evidence remain open.** |
...
| Caller-attested evidence bytes and all-false authority boundary | The packet-to-TestClient transport is local evidence only; source canonicality, Ergo PoW, external target acceptance, funds authority, Gate 5, trustless status and readiness remain false | Replace this row only when a source-owned canonical committed-reserve evidence collector drives the proof result |
