...
| Active internal slice | **FED-6-LAB - canonical tracker admission.** Fresh campaign `z` against commit `9503bc0c` closes the unmocked packet-to-application-checkpoint lifecycle. V4 binds that checkpoint, the same-process JVM tracker compiler receipt and the canonically confirmed tracker setup output into one deterministic proofless tracker candidate. V5 mines the exact `bridge_event_root || statementId` under `0x0401`, stops the owned isolated Ergo devnet and independently observes the stable anchor from both nodes with exact header ancestry and extension-root membership. V6 replaces the synthetic tracker context with that exact observed-header shape, binds the real setup box and admission key, signs locally through the WASM root signer and accepts only a same-target `/transactions/check` receipt. Fresh create-only campaign V6e, bound to commit `709bb356912cb5445c4b4d6a006ce8dba0fcc33e` and request SHA-256 `672f5cfbd479c0eb4567ad06c45cfcd3671603cf3ebc3115c84d8d3900460d44`, completed that full local-devnet join under receipt digest `627393a39a7bfe1eeac056b73d40957cc27cf2a9cd91642f6c5399ba961f599d`. It persisted no signed tracker bytes and exposed no tracker submission or broadcast capability. The local implementation now also carries an uncomposed, one-shot post-checkpoint process continuation: it can restart the same extension-bound owned pair for future authorization work, but no tracker check is promoted and no reservation, submission, broadcast or confirmation path consumes it yet. The next implementation boundary is to rebind the exact V6 check to that active target, then add canonical tracker authorization and durable reservation; actual tracker transport requires separate authorization. Global replay insertion, external-fee payout and recovery remain later joins. The disclosed source finality remains a dual-RPC depth policy and the anchor observation does not authenticate Ergo proof of work. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB observed-anchor tracker-check implementation checkpoint.** The V6 composition carries the V5 process-owned mined anchor into a strict ten-header observed context, independently recomputes every canonical header identity and lineage, verifies exact `0x0401` extension membership, reuses the canonically confirmed tracker setup output and exact compiler binding, and requires the setup admission key to equal the local synthetic session signer. The retained signer constructs the exact EIP-12 input, independently derives its transaction ID, signs through the local WASM root, binds the signing tip to the newest observed header, and sends only the signed candidate to the same owned node's `/transactions/check` endpoint. The sanitized receipt binds the candidate, signed bytes digest, signer context, checker identity and unchanged owned target while keeping submission, broadcast, tracker admission, replay, payout, funds, Gate 5, trustless and readiness authority false. The affected six-file test closure passes 149/149 tests and TypeScript compilation passes with the compiler-locked Node 24.14.0. This is a locally tested implementation checkpoint; an exact fresh commit-bound campaign is still pending and no unmocked node-acceptance claim follows yet. |
...
| Following boundary | **FED-6-LAB fresh V6 execution and promotion.** Commit the reviewed implementation checkpoint, generate a create-only canonical bootstrap request bound to that exact clean HEAD, and rerun the complete owned loopback campaign from setup through mined `0x0401`, dual-node observation, observed-anchor tracker signing and same-target `/transactions/check`. Preserve only the sanitized non-broadcast receipt. Header, extension, checkpoint, setup lineage, signer, state-context, signed bytes, target process or node-result drift must reject; a failed campaign remains an implementation finding rather than evidence. |
| Next implementation action | **Promote only an exact green commit-bound V6 campaign, then begin canonical tracker admission.** After the fresh campaign passes, freeze its receipt and exact source/target pins in a separate evidence checkpoint. The next code slice may then introduce tracker authorization, durable reservation, submission and canonical confirmation as distinct capabilities with immediate pre-transport revalidation. Only after canonical tracker confirmation may the global absent-to-present replay and external-fee payout joins begin. Recovery follows the composed value path; every public-network, real-funds, Gate-5, trustless and readiness boundary remains false. |
...
and production readiness remain false. The next implementation boundary is a
separate canonical tracker authorization, durable reservation, owned-loopback
submission and confirmation lifecycle; no replay or payout work precedes that
admission.
