...
| Active internal slice | **FED-6-LAB - classified V137 preparation.** V133, V135 and V136 are terminal; V134 was abandoned before request creation, and none of their mutable state or synthetic identities may be reused. V135 exposed and V136 corrected the create-only artifact-root orchestration contradiction. V136 then stopped at the outer `genesis_calibration` boundary before request creation or V11 invocation, but its intentionally bounded result could not distinguish build, runtime-test, process-startup, target-observation or cleanup failure. The current source batch adds a separate acceptance entry point that does not rewrite a source error object or message and associates only the existing finite source-failure phase out of band; a cleanup cofailure still retains both errors inside an aggregate. The compatibility entry point keeps its existing native-genesis diagnostic augmentation, and all accepted bytes remain unchanged. After this source batch is committed, publicly promoted and green on the exact pull-request head, V137 must use a wholly fresh chain-42 synthetic identity, spec, ports and mutable roots; V11 remains callable at most once and only after canonical request creation plus pure preflight. Gate 5, funds authority, trustless status, deployment readiness and support for public funds remain false. |
| Public coordination | Public `main` is protected by both required CI jobs, one approving review from someone other than the last pusher, stale-review dismissal, conversation resolution, linear history and disabled force-push/deletion. Pull request #3 is open and mergeable. Its exact pre-classifier head `a019d46eb8ed31cab5385309e85a66242cb0d813` passed push run `33804681814` and pull-request run `33804685983`, four successful jobs in total. The classifier batch must replace that head only through the guarded non-force promotion flow and must receive its own exact-head hosted closure before V137 begins. [Issue #1](https://github.com/a-shannon/ergo-sidechain-bridge/issues/1) owns independent review of exact commit `ee0686b84483e6f0af85c764e93b8a43383cc54a`; [issue #2](https://github.com/a-shannon/ergo-sidechain-bridge/issues/2) owns external target and custody evidence for a later independently operated FED-6 campaign. Private vulnerability reporting is enabled. The public issues remain parallel assurance lanes and do not authorize or describe FED-6-LAB. |
...
| Latest completed slice | **FED-6-LAB V136 terminal calibration diagnosis.** V136 passed its non-mutating input validation, focused request-constructor matrix, strict TypeScript check, independent critical review and exact pre-launch recheck, then was invoked exactly once. It stopped at `genesis_calibration` after creating only the authority-safe chain spec. Request, receipt, artifact and journal outputs remained absent or empty, no Frontier process survived, the in-memory synthetic key was not persisted or printed, and V11 was never invoked. V136 is terminal and cannot establish which inner source phase failed because its local orchestrator deliberately retained only the outer phase. This result changes no proof, contract, transaction, authorization, funds authority or release claim. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row, injected tool path, error message or injected port may expose a reusable signer or transport capability or become funds authority. V7 freezes the dual-node read-only target and same-target JVM check; V8 persists and revalidates the exact reservation across a real file-backed close/reopen boundary; V9 creates one single-use transport authorization and durable attempt, then persists an exact accepted or ambiguous V1 outcome before confirmation. V10 preserves those boundaries while starting the primary miner before the sole POST and requiring the same primary/witness process identities through canonical confirmation. V11 preserves all historical bytes and adds a separately digest-bound process-local response classification; a successful HTTP response is still not canonical admission. **Frozen inputs:** exact V136 bridge/source, patch, toolchain, build-report, base-spec and runtime identities; all V112-V136 mutable campaign state is terminal or abandoned and ineligible for reuse. **Reusable checks:** build-environment, V11 receipt, WP-08A dependency, WP-07 containment, exact source/patch, setup/check, dual-layout, deadline-composition and offline application-runner matrices remain reusable while their transitive inputs stay unchanged. **Changed-input closure:** only the acceptance API and its focused source-failure projection tests change in the current batch; the compatibility API, proof formats, contracts, transaction bytes, process implementation and source build remain unchanged. **Next due execution:** after exact source promotion and hosted checks, one fresh V137 lineage must project a finite inner failure phase or reach canonical tracker confirmation. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import, peg-out, recovery and activation remain unvalidated. |
| Following boundary | **FED-6-LAB V137 fresh classified application-checkpoint campaign.** Never retry V136 or consume any earlier mutable state. After the source-failure classifier is committed, published and green on the exact pull-request head, create one wholly fresh request, ports, processes, storage and authorization lineage. Run the complete owned application-checkpoint path once and require either canonical tracker confirmation or one terminal finite source phase without emitting arbitrary diagnostics. |
| Next implementation action | **Promote the classified acceptance boundary, then run V137 once.** Close the focused acceptance tests, TypeScript, architecture check and independent review; stage only the source, direct test and this plan checkpoint; run staged and exact-range publication guards; update pull request #3 through the guarded non-force push; and wait for exact-head hosted checks. Then create the wholly fresh local-only V137 lineage. If V137 canonically confirms the exact tracker successor, move directly to burn/checkpoint binding, global replay insertion, external-fee payout and profile-specific recovery. Public networks, existing wallet material, real funds, Gate 5, trustless status and readiness remain outside this LAB action. |
...
```text
frozen foundations
  -> canonical tracker admission (V136 stopped at an unclassified inner genesis-calibration phase; promote the finite source-phase projection, then use the wholly fresh bounded V137 campaign)
...
|---|---|---|---|
| V136 source/build inputs and locked tools | Exact bridge/source, patch, command, tool, cache-source, build-report, base-spec and runtime identities | Source relocation or any source, patch, command, toolchain, cache-source or generated-byte drift | Reuse for focused root-cause falsification; rebuild only on drift and never reuse V136 mutable campaign state or identity |
...
| Mutable local campaign evidence | One exact fresh process and storage lineage | Terminal attempt, process replacement, request drift or target-state change | V112-V133, V135 and V136 are terminal; V134 was abandoned. Create V137 only after the finite source-phase projection is promoted, and never reuse an earlier attempt as authority |
