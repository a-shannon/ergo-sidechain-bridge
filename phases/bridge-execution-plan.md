...
| Active internal slice | **FED-6-LAB - federated peg-out campaign.** Commit `da2f99d9` and one fresh real-component run now join setup, refundable source lock, committed-reserve transition, canonical source evidence, V2 threshold packet proof, and source-locked direct-parent Frontier TestClient reservation/mint consumption in one owned node/process lifetime. The sanitized command receipt is request-bound and every process/listener was absent after completion. The next objective is the distinct burn-to-proof-to-global-replay-insertion-to-payout lifecycle on another fresh isolated campaign, with exact application binding, external-fee conservation, atomic replay insertion, pre-submit revalidation, and no timeout or local-state payout authority. Recovery follows after both value directions close. The disclosed source finality remains a dual-RPC depth policy and does not authenticate Ergo proof of work. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB fresh real-component peg-in closure.** Commit `da2f99d9` aligns the campaign validator with the real Frontier consumer receipt's mandatory `boundary` and adds an isolated missing-boundary rejection. A fresh exact campaign bound request SHA-256 `0bd776a23d32585061dd0074a1178d1aa9bdd6874109767e22ce95051f166372` to command receipt `5b3f70cf70b8f8d1810a1ebb3ae791e580d6dbfcd25e42f367b3a545be6d009b`, worker receipt `454e4a344b8bae84dda4b92b52a50b22cba6cf30b07034db98538987b7d24835`, and root receipt `d2b11658d8731fbe9071f44a475e5a210853c8d0447c976d3857f4ad36f43982`. It executed setup and the Ergo source-lock-to-reserve path on two owned loopback nodes, derived evidence from that confirmed state, produced the V2 threshold packet proof, and consumed the exact reservation/mint in the source-locked Frontier TestClient. Receipt digests and all checks revalidate; no signed bytes, local paths, reusable capabilities, public network, real funds, independent source canonicality, Ergo PoW, external custody, operational mint authority, Gate 5, trustless status or readiness follow. |
...
| Following boundary | **FED-6-LAB federated peg-out campaign.** Construct one canonical burn from the reviewed bridge/token application, bind it to the exact federated checkpoint statement and source proof, admit only that application-bound root, insert the burn ID into the global replay lineage atomically with the exact payout, and fund the Ergo miner fee externally. Execute and confirm the complete path on one fresh process-owned loopback campaign, stopping before recovery. |
