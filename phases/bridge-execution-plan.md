...
| Following boundary | **FED-6 operational custody, target approval, and separately authorized setup lineage execution; externally and authorization blocked under public issue #2.** A future run requires a fresh packet whose source-attestation threshold and Ergo-admission role use independently reviewed, operationally durable custody, exact target identity and approval, and authentic target-owned history. The disposable G1dI3c packet cannot be reused. Signing/checking, setup submission, broadcast, canonical confirmation, profile activation, and later value-path execution remain distinct approvals and must not be collapsed into one operator action. |
| Next implementation action | **Process independent findings from public issue #1 while obtaining the exact target and custody inputs requested by issue #2.** Keep the public repository synchronized only at coherent reviewed milestones. Once the issue #2 intake has an approved non-mainnet target identity, independently controlled source-attestation and Ergo-admission roles, target-owned canonical setup history, and complete route-retirement/replay lineage, resume with signed JVM and same-target `/transactions/check` acceptance under an exact no-submit approval. Preserve the completed WP-07A-DN1 campaign and do not create another local node wrapper, synthetic activation, manual-seal surrogate, disposable authority packet or recovery lifecycle. |
