...
| Active internal slice | **FED-6-LAB - no-value federated reservation admission.** The fresh same-lifetime Ergo campaign at commit `49218bcd` remains the exact local source checkpoint: setup, refundable source-lock creation and the separately checked, authorized, durably reserved, transported and canonically confirmed transition produced a non-refundable reserve successor. The source-locked Frontier patch now compiles a dedicated LAB node that selects only `runtime-v4-lab`, rejects simultaneous main-runtime selection, accepts only the built-in `bridge-lab` chain spec, and embeds that runtime's exact WASM in genesis. The main runtime remains unchanged and disabled for this route. The next objective is exact application-bound source-proof admission into one idempotent pending reservation, with no token-supply mutation. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB dedicated Frontier target composition.** The template node now provides the explicit `bridge-federated-v4-lab-node` aggregate feature as the supported composition for selecting `runtime-v4-lab`, rejects builds that also select the main runtime, and accepts only the built-in `bridge-lab` selector. Its deterministic chain spec embeds the selected LAB runtime WASM exactly and carries no legacy owner-mint quarantine fallback. The normal template-node build remains green, the LAB build and selector/genesis tests pass, the cumulative patch applies to the exact pinned Frontier base, and the source lock validates against pinned Frontier and Ergo checkouts. This closes target selection and reproducibility only: no reservation was admitted, no token supply changed, and funds authority, Gate 5, trustless status and readiness remain false. |
...
| Following boundary | **FED-6-LAB exact no-value federated reservation admission.** Feed the application-bound FED-1 source proof to the dedicated LAB runtime and create one idempotent pending reservation bound to the exact profile, bridge, token, runtime, deposit and committed-reserve identities. Reject missing, stale, replayed or mismatched inputs. The main runtime must remain disabled, the historical owner mint must remain unreachable, and no token supply may change in this boundary. |
| Next implementation action | **Implement and exercise the pending-reservation call on the dedicated LAB target.** Reuse the existing V4 reservation state machine and federated proof verifier, expose only their statically registered LAB composition, and add one canonical admission plus isolated profile, proof, replay, deposit, reserve and application-identity negatives. Stop before the direct-parent mint callback. After this boundary is green and independently reviewed, run a fresh paired Ergo/Frontier campaign that binds its exact committed-reserve receipt to the accepted reservation and only then add atomic mint execution as a separate slice. Peg-out and recovery remain out of scope, and every public-network, funds, Gate-5, trustless and readiness boundary remains false. Keep issues #1 and #2 open for external review and independently operated evidence. |
