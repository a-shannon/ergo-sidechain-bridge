...
| Active internal slice | **FED-6-LAB - isolated local execution campaign, source-lock execution stage.** A fresh same-lifetime campaign repeated the exact setup lineage, constructed one simulated-funds source-lock transaction, signed it locally, and passed the pinned JVM plus same-node `/transactions/check`. Request SHA-256 `ea4306a37fbbda90c7ad0fdd9f2e47e6e5c05c5f1099572f8b6dd79ae6d0047d` is bound to command receipt `68adf6726e321f148aa956fea7d16b04485a0c195cd7f1a06ebbe8339d51ccff`. The source-lock was not submitted or broadcast, no refundable source was consumed, no committed vault or reserve lineage was established, and mint remained unauthorized. The next objective is an operation-specific authorization, durable reservation, local transport and canonical confirmation of that exact source-lock transition before any mint step. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB source-lock signing and node acceptance, local and self-operated.** The sole static root started one fresh loopback campaign, repeated and confirmed the setup lineage, constructed the exact simulated-funds source-lock candidate, retained one synthetic signer only through guarded WASM signing, and obtained the pinned JVM plus same-node `/transactions/check` result. The tracked command bound request SHA-256 `ea4306a37fbbda90c7ad0fdd9f2e47e6e5c05c5f1099572f8b6dd79ae6d0047d` to command receipt `68adf6726e321f148aa956fea7d16b04485a0c195cd7f1a06ebbe8339d51ccff` and execution receipt `0890f146b4935dee43ba47b320e20646dbe417d326f20930eae578f9d1abb5ad`; all owned processes and monitored listeners were absent afterward. Signed bytes were neither returned nor persisted, and no value-path submission, broadcast, source consumption, reserve lineage, mint, funds authority, Gate 5, trustless status or readiness follows. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row or injected port may expose a concrete signer or transport capability after the authorized run. Reward delay 1 or 720 remains digest-bound to the exact signer-owned reward proposition. The exact FED-6 reobservation invocation must use `NonMiningResume`, which requires `ResumeExistingDataDir`, rejects a runner-supplied mining-mnemonic reference and forces both primary and witness processes to non-mining mode; discovery and the separate observer use bounded GET-only requests and verify chain identity. This leaves the documented Phase 011b mining-enabled resume workflow unchanged and outside FED-6 evidence. **Owned paths:** the existing patched-devnet runner and no-submit conformance/check modules; the signer-first session, reward-input discovery and canonical history producers; the bounded read-only node client; `relayer-core/substrate-federated-local-devnet-genesis-execution-v1{.test,}.ts`; the generic operational lifecycle; and the StateTracker operational profile. **Current checks:** exact delay and reciprocal-mismatch matrices; deterministic signer-bound three-input selection with dual-source disagreement, ambiguity, immaturity and signer-drift negatives; canonical header/transaction/UTXO construction and mutation negatives; pinned JVM conformance; pure lifecycle ordering and provenance negatives; unresolved-attempt/restart journal constraints; runner source guards; Node 24 typecheck; layer imports; broadcast-surface isolation; the live exact-three local execution; the exact dual-origin non-mining reobservation; and the fresh source-lock sign/check command receipt above. The tracked setup CLI, sole static execution root, process-owned local transport and live port registration are closed for the exact LAB setup family. A value-path composition root now reaches exact source-lock signing and node acceptance, but exposes no submission authorization or signed bytes and establishes no source consumption, vault/reserve lineage or mint authority. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import and every peg-out or activation check remain unvalidated. |
| Following boundary | **FED-6-LAB source-lock consumption on one fresh isolated local devnet.** Repeat setup and source-lock construction inside one retained target lifetime, then create one exact operation-specific authorization for the already checked source-lock bytes. Durably reserve before local transport, reconcile accepted/ambiguous outcomes, and canonically confirm exact source absence plus the exact non-refundable vault/reserve successor at policy depth. Stop on target drift, non-loopback transport, unresolved prior attempt, dual-node disagreement, transaction mutation, source restoration or confirmation rollback. Mint remains unavailable until this transition is independently revalidated. |
| Next implementation action | **Extend the sole FED-6-LAB execution root through source-lock submission and committed-vault confirmation, but stop before mint.** Reuse the frozen checked-submission authorization, durable journal, process-owned transport, confirmation observer and committed-vault verifier without changing their bytes or semantics. Bind the authorization to the exact source-lock check receipt, target, transaction ID and expiry; prove setup receipts and local rows cannot authorize transport; persist the attempt before `/transactions`; and require exact source consumption plus vault/reserve successor agreement after confirmation. Do not add mint, peg-out or recovery code until this producer-to-consumer join closes. Keep issues #1 and #2 open for external review and independently operated evidence. |
