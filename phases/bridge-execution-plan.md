...
| Active internal slice | **FED-6-LAB - federated peg-out campaign.** The peg-in campaign is closed through commit `da2f99d9`. The current peg-out checkpoint adds a source-locked same-process runner that applies the exact bounded application overlay to an external scratch checkout, builds and executes the exact Rust TestClient test, consumes its stdout directly, and reverses the overlay before returning. The producer deploys the reviewed SERG/bridge application, consumes one reservation into mint, executes the real `approve` and `pegOut`, decodes the emitted ABI log, and verifies the runtime burn leaf/root plus supply and fee conservation; a pure TypeScript consumer independently rebuilds that transcript against exact patch bytes. The immediate next boundary is to bind this process-proven burn/root into the versioned federated checkpoint and finality statement, then compose tracker admission, global replay insertion and the external-fee payout. Confirmation and recovery remain later joins. The disclosed source finality remains a dual-RPC depth policy and does not authenticate Ergo proof of work. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB retained packet/mint/checkpoint continuation.** A separate V3 continuation now retains one exact launched V2 source-attestation session across packet production, one packet-bound mint source proof and one versioned checkpoint attestation. It accepts only the exact process-proven packet and mint receipt objects, revalidates their target binding immediately before checkpoint production, enforces `packet -> mint -> checkpoint`, consumes the capability after every post-mint checkpoint attempt and recursively binds the exact checkpoint receipt. Calls rejected before the required state do not consume the still-valid earlier capability. The inner checkpoint receipt correctly retains its narrower `mintBeforeCheckpointLifecycleEstablished = false` boundary; only the outer V3 composition establishes that local order. The existing V2 packet-to-mint continuation keeps its immediate disposal behavior. The direct matrix covers ordering, copied-object rejection, packet/mint/checkpoint failure cleanup, target drift, disposal and one-shot provenance. The packet consumer fixture now uses the canonical deterministic LAB bridge/token addresses. The application-burn receipt is deliberately not bound yet, so this is not source consensus/finality, tracker admission, replay insertion, payout, Gate 5, trustless status or readiness. |
...
| Following boundary | **FED-6-LAB V3 application-burn join.** Compose the exact process-proven dynamic application runner between the retained mint proof and checkpoint capability so the root enforces `packet -> mint -> burn -> checkpoint`. The checkpoint input must be derived from, not copied beside, the exact burn receipt and must bind its application identity, source native block, execution block, `bridge_event_root`, leaf count, amount and recipient. Cross-target or copied receipts and every failed dynamic run must consume and dispose the root capability. The result remains a disclosed federated policy decision, not GRANDPA/STARK verification or trustless finality. |
| Next implementation action | **Execute the exact dynamic Rust path once through the V3 root before tracker or payout work.** Reuse the frozen peg-in campaign producers, V2 dynamic runner and retained packet/mint/checkpoint continuation in one fresh isolated composition. Bind the runner's exact application-burn receipt into the checkpoint statement, prove failure cleanup and one-shot consumption, then perform one real offline Rust TestClient execution against the pinned source/tool closure. Only afterward compose tracker admission, global absent-to-present replay insertion and external-fee payout as separate joins. Reject wrong bridge/token/code identity, stale or mismatched checkpoint, root/proof drift, payout or amount drift, replay across every retained profile lineage, fee-from-backing, burn disappearance before transport, and every local-status or timeout payout path. Recovery follows the value-path composition; every public-network, real-funds, Gate-5, trustless and readiness boundary remains false. |
