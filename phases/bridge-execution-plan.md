...
| Active internal slice | **FED-6-LAB - federated peg-out campaign.** Fresh campaign `z` against commit `9503bc0c` closes the unmocked packet-to-application-checkpoint lifecycle. The V4 implementation checkpoint binds that exact checkpoint shape, the same-process JVM tracker compiler receipt and the canonically confirmed tracker setup output into one deterministic proofless tracker candidate. The V5 checkpoint now mines its exact `bridge_event_root || statementId` value under `0x0401` on the same isolated Ergo devnet, stops mining, and independently observes the stable anchor from both owned nodes with exact prior-header ancestry and extension-root membership. This anchor is explicitly non-authorizing. The immediate next boundary is to replace the V4 synthetic header context with that exact observed anchor, then run exact JVM reduction and same-target `/transactions/check`, still without tracker submission. Global replay insertion and external-fee payout follow as separate joins; confirmation and recovery remain later joins. The disclosed source finality remains a dual-RPC depth policy and the anchor observation does not authenticate Ergo proof of work. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB checkpoint-to-tracker candidate construction.** The V4 composition retains the exact V3 application checkpoint, one exact setup tracker lineage and its same-process JVM compiler binding. It waits beyond the checkpoint admission start, requires the tracker setup transaction to remain at the same canonical inclusion, rematerializes its exact predicted output, and uses that real setup box as the sole tracker input. The composed integration carries the production setup execution batch and exact JVM receipt through rematerialization into the real compiler-bound builder, while the root test proves those same identities are forwarded by the campaign. Isolated contract, value, ErgoTree, token, register cardinality and R4-R9 values, creation-height bounds, statement, confirmation, construction-boundary and nested authority drift reject; every returned authority, signing, node-check, submission and broadcast flag remains false. The affected architecture/tracker/root/launch closure passes 289/289 tests with the compiler-locked Node 24.14.0. This is a deterministic proofless candidate over a synthetic header context, not a mined Ergo anchor, JVM/node acceptance, tracker admission, replay insertion, payout, Gate 5, trustless status or readiness. |
...
| Following boundary | **FED-6-LAB real `0x0401` tracker-check join.** Reuse the owned loopback Ergo campaign and exact V4 checkpoint-bound candidate inputs, mine one exact extension commitment under prefix `0x04`, observe its canonical header and replace the synthetic header context with that exact anchor. Then execute the pinned JVM reduction and same-target `/transactions/check` no-submit path against the canonically confirmed tracker setup box. Header, extension, checkpoint, setup lineage, reorg or node-result drift must reject before any tracker transport exists. |
| Next implementation action | **Add the smallest real-anchor continuation to the existing owned campaign.** The extension producer must accept only the exact checkpoint commitment and process-owned target; the tracker builder must consume the observed mined header rather than caller-selected header bytes. Freeze the exact checked tracker transaction and its non-broadcast receipt, then close a separate authorization/reservation/submission slice. Only after canonical tracker confirmation may the global absent-to-present replay and external-fee payout joins begin. Recovery follows the composed value path; every public-network, real-funds, Gate-5, trustless and readiness boundary remains false. |
