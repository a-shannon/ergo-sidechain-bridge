...
| Active internal slice | **FED-6-LAB - direct-parent federated mint join.** The fresh same-lifetime Ergo campaign at commit `49218bcd` remains the exact local source checkpoint: setup, refundable source-lock creation and the separately checked, authorized, durably reserved, transported and canonically confirmed transition produced a non-refundable reserve successor. The dedicated `runtime-v4-lab` node now admits the exact application-bound federated source proof into one idempotent pending reservation while preserving bridge/token code, nonce, balance and supply state. Missing activation, stale proof, wrong proof profile, wrong application identity, rebound deposit/reserve identity and replay all reject. The next objective is a fresh paired Ergo/Frontier campaign that binds the exact committed-reserve receipt to that reservation and exercises only the reservation's direct-parent atomic mint consumer. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB no-value federated reservation admission.** The dedicated `runtime-v4-lab` node deploys the pinned bridge/token fixture bytes, activates the exact federated V4 profile through its bounded LAB composition, and admits one canonical application-bound proof into an idempotent pending reservation. The deterministic bridge address is present only as the genesis legacy-mint quarantine identity needed by profile activation; the disabled legacy mutation call remains disabled. The reservation block has no Ethereum transaction status and preserves bridge/token code, nonce and balance. Missing activation, stale proof, wrong proof profile, wrong application identity, rebound deposit/reserve identity, replay and rejected-overlay persistence all fail closed. The cumulative patch applies to the exact pinned Frontier base and its 113-file lock validates with the exact Ergo checkout. This closes local no-value reservation admission only: no paired campaign or mint executed, and funds authority, Gate 5, trustless status and readiness remain false. |
...
| Following boundary | **FED-6-LAB direct-parent atomic mint join.** Run one fresh paired Ergo/Frontier campaign that turns the exact canonically confirmed committed-reserve receipt into the admitted V4 reservation and consumes only that reservation through its direct-parent runtime callback. Prove exact deposit, reserve, bridge, token, runtime, recipient, amount and mint-identity binding; reject missing, expired, invalidated, replayed or rebound candidates; and prove the owner-key mint route remains unreachable. Stop before peg-out. |
| Next implementation action | **Join the accepted reservation to its direct-parent mint consumer in one fresh paired LAB campaign.** Bind the exact committed-reserve receipt from the Ergo campaign to the canonical V4 statement, admit it on the dedicated LAB runtime, and exercise only the atomic runtime callback that consumes that exact pending reservation. Isolate missing, expired, invalidated, replayed, wrong-deposit, wrong-reserve and wrong-application cases, and prove that no owner-key mint route is restored. Stop before peg-out. Peg-out and recovery remain out of scope, and every public-network, funds, Gate-5, trustless and readiness boundary remains false. Keep issues #1 and #2 open for external review and independently operated evidence. |
...
|---|---|---|---|
| **WP-06-FED - federated reference** | One statically registered profile binds two explicit cryptographic roles: the source-attestation Ed25519 key set and the Ergo-admission SigmaProp key set, each with its own digest and threshold under one federation epoch. Their respective authority over mint and payout is disclosed; neither role is implied to be the other. | Functional institutional reference profile under a federated trust model; never trustless. | **Migration execution remains at the FED-6 external boundary; local greenfield execution now reaches a no-value pending reservation on the dedicated FED-6-LAB runtime.** FED-1 through migration-only FED-5B plus the distinct greenfield baseline/generation/provisioning and fresh-process replay joins are complete as non-authorizing checkpoints. One fresh self-operated Ergo campaign has signed, checked, submitted and canonically confirmed the exact setup, refundable source-lock and source-lock-to-reserve lineages through source checkpoint `49218bcd`; the separate source-locked LAB runtime now admits the exact federated V4 proof without changing token state. These local results are not target approval, reusable funds authority or an end-to-end paired campaign. Atomic federated mint, peg-out, recovery, independent custody and the externally approved migration campaign remain open. |
...
| **WP-06-FED** | EIP-independent federated peg-in and peg-out profile | WP-01/04/05 foundations plus reviewed V4/V6 conservation and WP-07 lifecycle | Federated statement/tracker, peg-in admission, settlement family, daemon integration, and cutover are sequential authority joins; disjoint target/retirement evidence work may run in parallel only when real inputs exist | **Migration targets remain at the FED-6 external boundary; FED-6-LAB now joins exact committed-reserve confirmation through source checkpoint `49218bcd` with a separate no-value pending reservation admitted by the dedicated runtime. FED-1 through FED-4, FED-5A/B migration generation, and the distinct greenfield baseline/generation/provisioning/portable-replay joins remain frozen. The committed-reserve receipt and accepted reservation have not yet been exercised in one paired campaign, and no mint has executed. Atomic federated mint, peg-out, recovery, static registration, independent custody and externally approved target evidence remain open.** |
...
`069cf64b91e648ef04a2b320aedfa63bd7262583a165370585638f36e8338407`.
...
activates the exact federated profile, admits one canonical pending reservation,
proves replay rejection, and isolates missing activation, stale proof, wrong
proof profile, wrong application and rebound deposit/reserve identities. The
reservation block contains no Ethereum transaction status and preserves both
contracts' code, nonce and balance. The complete 113-file map remains
