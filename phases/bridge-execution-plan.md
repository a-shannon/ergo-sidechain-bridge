...
| Latest completed slice | **FED-6-LAB collected-evidence consumer join.** The source-owned committed-reserve collector serializes and one-shot binds the canonical source-lock, reserve transition, successor reserve, confirmation, depth path and federated-finality disclosure to the exact draft/candidate/packet lineage. V2 packet proof production rejects raw, copied or reused evidence receipts. The source-locked Frontier consumer now requires that exact collected-evidence boundary, retains the source-evidence receipt digest, and rejects any receipt that still describes caller-supplied evidence before invoking Cargo. Pre/post Node/module/source/tool hashes remain explicitly non-atomic same-user evidence, not loaded-image or independent-build attestation. This closes the module-level reserve-evidence-to-atomic-consumer join, not one fresh live campaign. It establishes neither independent source canonicality, Ergo PoW, external custody, mint authority, Gate 5, trustless status nor readiness. |
