...
| Active internal slice | **FED-6-LAB - fresh frozen-tracker campaign reconstruction.** V4 through V8 close the application-checkpoint, `0x0401` anchor, frozen same-target JVM check and durable reservation joins. Campaign V9k remains terminal without automatic retry and its accepted-versus-ambiguous private-runtime outcome is not promoted. The Windows command boundary now reports only finite safe parent and worker phases, filters linked npm path shims and retains no raw child stdout, stderr, paths or arbitrary details. A fresh campaign reached the exact Frontier source-acceptance check after the patched Ergo node had built successfully; a separate real two-node exercise then proved mining, freeze-to-read-only observation and clean shutdown without submission. The campaign exposed one reproducibility defect: Git with `core.autocrlf=true` materialized the eight pinned Frontier `.hex` vectors as CRLF, while the source inspector did not classify `.hex` as controlled text. The inspector now accepts only CRLF-to-LF equivalence for that format, and the original public patch applied to a fresh checkout validates against every locked blob without changing any vector, patch, contract or source lock. The failed campaign lineage is terminal and must not be reused. The next action is one completely fresh frozen-tracker campaign from current reviewed code; transport V9, global replay insertion, external-fee payout and recovery remain later joins. Source finality is still a disclosed dual-RPC depth policy, and the anchor observation does not authenticate Ergo proof of work. This self-operated research evidence does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
