...
| Active internal slice | **FED-6-LAB - receipt-derived end-to-end peg-in campaign.** The source-owned collector now derives canonical source-lock, reserve-transition, successor-reserve, inclusion, ancestry, and disclosed federated-finality bytes from the exact process-proven committed-reserve observation. Its one-shot receipt is bound to the same draft, candidate, execution target and packet; copied receipts, second consumption, missing retained inputs, lineage drift, stale ancestry, and cross-campaign reuse fail closed. Node and collector-module files are hashed before and after collection under an explicitly non-atomic, exclusive same-user execution assumption; this is not loaded-image attestation. The V2 source-attestation and packet APIs no longer accept caller-supplied evidence bytes and bind the collector receipt digest before producing the existing source-proof envelope. This closes the module-level reserve-evidence-to-packet join only. The next objective is one fresh same-lifetime campaign that runs the already reviewed committed-reserve transition, collector, packet proof, and source-locked atomic Frontier consumer without fixture substitution. The disclosed finality object remains a dual-RPC depth policy and does not authenticate Ergo proof of work. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB canonical committed-reserve evidence join.** A new source-owned collector serializes the retained canonical source-lock box, reserve-transition transaction, successor-reserve box, confirmation observation, exact depth path, and explicit federated-finality disclosure. It binds those bytes to the same process-proven observation, candidate, packet and V4 mint-reservation draft. The receipt also records pre/post hashes of the Node executable file and collector module file under a disclosed non-atomic same-user assumption; it does not attest the loaded images. V2 source-attestation and packet production accept only that branded one-shot receipt; raw evidence fields, copied receipts and reuse across fresh sessions are rejected. Focused tests compose one real collector receipt through the packet signer while retaining isolated negatives for missing inputs, lineage substitution, stale ancestry and cross-campaign splice. This establishes evidence collection provenance, not independent source canonicality, Ergo PoW, external custody, mint authority, Gate 5, trustless status or readiness. |
...
| Following boundary | **FED-6-LAB receipt-derived end-to-end peg-in campaign.** Run source-lock creation, exact committed-reserve execution and confirmation, canonical evidence collection, V2 packet-bound threshold attestation, and the existing source-locked direct-parent Frontier consumer in one fresh same-lifetime campaign. Preserve every receipt and identity join without exposing signer, submitter, or reusable mint capabilities. Stop before peg-out. |
| Next implementation action | **Compose the fresh campaign root and one bounded acceptance command.** Reuse the existing execution receipts, collector, packet continuation, and Frontier consumer without redesigning their closed codecs. Require the exact collector receipt to reach the consumer, reject fixture/fake evidence, target or campaign splices, and prove that every capability is consumed or disposed on both success and failure. Do not repeat the atomic Frontier matrix unless its exact proof inputs change. Peg-out and recovery remain out of scope, and every public-network, funds, Gate-5, trustless and readiness boundary remains false. Keep issues #1 and #2 open for external review and independently operated evidence. |
