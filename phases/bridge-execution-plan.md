...
| Latest completed slice | **FED-6-LAB dynamic runner and static application/checkpoint composition.** The V2 runner now accepts one exact process-proven mint receipt, commits its proof environment into the Cargo execution, applies reproducible host/WASM remaps, executes the real reservation-to-mint plus `approve`/`pegOut` TestClient path, and rejects application or supply drift. A separate static V3 composition passes the exact retained inner mint proof to that runner, derives source block, execution block, `bridge_event_root` and leaf count from the returned burn evidence, independently cross-checks packet-owned sidechain/bridge/token identities, and creates the checkpoint only afterward. Exact packet, mint, runner and checkpoint provenance is revalidated after execution; target, receipt and statement drift fail before any downstream join, and every outcome disposes retained custody. Focused runner integration and composition negatives pass, but the complete root has not yet run with the real packet producer and checkpoint continuation in the same test. Source consensus/finality, Ergo anchor admission, tracker authority, replay insertion, payout, Gate 5, trustless status and readiness remain open. |
...
| Following boundary | **FED-6-LAB unmocked application/checkpoint campaign.** Exercise the complete producer-to-consumer join with the real packet producer, exact inner mint proof, V2 process runner and real V3 checkpoint continuation in one opt-in isolated execution. The tracked test must retain the exact receipt graph, prove source restoration and capability disposal, and keep every authority/readiness boundary false. Only then may the checkpoint-to-tracker/replay/payout join begin. |
| Next implementation action | **Reuse the existing real-component peg-in campaign instead of inventing another fixture stack.** Extend the retained campaign boundary, or add the smallest dedicated opt-in harness around its real producers, so one exact packet and mint proof continue directly through the dynamic application burn and checkpoint. No producer or provenance assertion may be mocked. After that passes, compose the exact V3 receipt through the existing federated tracker, global absent-to-present replay lineage and external-fee settlement families. Recovery follows the composed value path; every public-network, real-funds, Gate-5, trustless and readiness boundary remains false. |
