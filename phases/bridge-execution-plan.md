...
| Active internal slice | **FED-6-LAB - federated peg-out campaign.** Fresh campaign `z` against commit `9503bc0c` closes the unmocked packet-to-application-checkpoint lifecycle. V4 binds that checkpoint, the same-process JVM tracker compiler receipt and the canonically confirmed tracker setup output into one deterministic proofless tracker candidate. V5 mines the exact `bridge_event_root || statementId` under `0x0401`, stops the owned isolated Ergo devnet and independently observes the stable anchor from both nodes with exact header ancestry and extension-root membership. The V6 implementation checkpoint replaces the synthetic tracker context with that exact observed-header shape, binds the real setup box and admission key, signs locally through the WASM root signer and accepts only a same-target `/transactions/check` receipt. It exposes no tracker submission or broadcast capability. A fresh clean-checkout campaign bound to the V6 commit remains required before this checkpoint may be promoted as unmocked local-devnet evidence. Canonical tracker admission and confirmation, global replay insertion, external-fee payout and recovery remain separate later joins. The disclosed source finality remains a dual-RPC depth policy and the anchor observation does not authenticate Ergo proof of work. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
