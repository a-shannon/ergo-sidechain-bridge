...
| Active internal slice | **FED-6-LAB - isolated federated mint-admission target composition.** The fresh same-lifetime Ergo campaign at commit `49218bcd` remains the exact local source checkpoint: setup, refundable source-lock creation and the separately checked, authorized, durably reserved, transported and canonically confirmed three-input transition produced non-refundable reserve successor `1d4d8c7c2ecc5d8efdc73bebb333130e9fd5ad50fbb9832fdc753c53a6100b`. The FED-1 producer now exposes the canonical source-proof profile as exact SCALE bytes, and the source-locked Frontier patch contains a separate non-publishable `runtime-v4-lab` whose compile-time provider accepts only those public profile bytes and their exact derived profile ID. Malformed, non-canonical, missing or mismatched profile inputs fail the build; the LAB runtime enables only the federated pending-reservation proof profile, keeps legacy owner minting disabled, and does not change the main runtime's disabled activation. No node or chain spec selects the LAB runtime yet, so mint remains unavailable. The next objective is a dedicated LAB node/chain-spec composition and exact no-value target admission before any atomic mint execution campaign. Peg-out and recovery remain later boundaries. This lane is self-operated research evidence only: it does not establish independent custody, external review, Gate 5, trustless status, deployment readiness or support for public funds. |
...
| Latest completed slice | **FED-6-LAB compile-time source-proof profile join.** TypeScript now emits the exact SCALE encoding of the normalized FED-1 federation epoch, threshold, ordered public keys, validity bound and verifier profile ID; the same fresh process-owned attestation session exposes those public bytes without persisting private signing material. The source-locked Frontier patch adds an isolated `runtime-v4-lab` compile-time provider for that exact profile and keeps both the main runtime and legacy peg-in mint execution disabled. A reference vector, fresh dynamic TS-to-Rust join, malformed-input build negatives, clean patch application, exact source-lock replay and pinned Frontier/Ergo source verification pass. This closes a serialization and static-registration boundary only: no node selects the LAB runtime, no reservation or mint was executed, and funds authority, Gate 5, trustless status and readiness remain false. |
...
| Following boundary | **FED-6-LAB dedicated Frontier target and no-value federated reservation admission.** Wire `runtime-v4-lab` into an explicitly named, non-publishable node/runtime selection and deterministic chain spec that consume the exact public FED-1 profile bytes. Prove that the exact target accepts the application-bound source proof into one idempotent pending reservation and rejects missing, stale, replayed or mismatched profiles and bridge/token/runtime identities. The public/main runtime must remain disabled, the historical owner mint must remain unreachable, and no token supply may change in this boundary. |
| Next implementation action | **Add the isolated LAB node/chain-spec composition and exact target admission matrix.** Reuse the existing source-locked node only through a compile-time LAB runtime selection; bind genesis/runtime identity to the exact profile ID, expose no dynamic proof plugin or owner-mint fallback, and exercise one canonical reservation plus single-fault profile, proof, replay and application-identity negatives. Stop before atomic mint execution. After this boundary is green and independently reviewed, run a new fresh paired Ergo/Frontier campaign that binds its exact committed-reserve receipt to the accepted reservation and direct-parent mint callback. Peg-out and recovery remain out of scope, and every public-network, funds, Gate-5, trustless and readiness boundary remains false. Keep issues #1 and #2 open for external review and independently operated evidence. |
