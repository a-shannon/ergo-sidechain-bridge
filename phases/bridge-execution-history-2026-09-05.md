# Execution History Through 2026-09-05

Historical snapshot of the plan at source commit
`907e8c8deca5ed51e2c647d9d0b75a3c7e36beb9`. The original text below is retained
for package specifications and checkpoint provenance, including its superseded
priorities. For current work order and status, use the
[active execution plan](bridge-execution-plan.md).
...
| Active internal slice | **FED-6-LAB - V142 terminal diagnosis and V143 node-identity-boundary preparation.** V142 ran exactly once from integrated commit `9ea01e0215ae8c8cd0458283c62d1017b735b8b0`, crossed the earlier node/runtime classifier boundaries, created its canonical request and entered V11, then failed closed on the explicitly untrusted worker phase hint `source target node identity validation`. The current batch replaces only that broad diagnostic phase with four finite source-owned boundaries: canonical node-identity decoding, exact chain-name validation, exact node-name validation and exact node-version validation. The RPC batch and order, acceptance predicates and accepted bytes remain unchanged. After this batch is committed, publicly promoted, integrated and green on the exact integrated commit, V143 must use a wholly fresh chain-42 synthetic identity, spec, ports and mutable roots. Gate 5, funds authority, trustless status, deployment readiness and support for public funds remain false. |
| Public coordination | Public `main` remains protected by both required CI jobs, one approving review from someone other than the last pusher, stale-review dismissal, conversation resolution, linear history and disabled force-push/deletion. Pull request #4 merged into `a-shannon/research-alpha` as exact commit `9ea01e0215ae8c8cd0458283c62d1017b735b8b0`; integration run `33895139160` passed the dependency audit, pinned-source rebuild and public-audit gate. The node-identity-boundary batch starts from that exact green commit and must use a new guarded non-force feature-branch promotion plus exact-head and exact-integration hosted closure before V143 begins. [Issue #1](https://github.com/a-shannon/ergo-sidechain-bridge/issues/1) owns independent review of exact commit `ee0686b84483e6f0af85c764e93b8a43383cc54a`; [issue #2](https://github.com/a-shannon/ergo-sidechain-bridge/issues/2) owns external target and custody evidence for a later independently operated FED-6 campaign. Private vulnerability reporting is enabled. The public issues remain parallel assurance lanes and do not authorize or describe FED-6-LAB. |
...
| Latest completed slice | **FED-6-LAB V142 terminal node-identity diagnosis.** V142 passed pure validation, strict TypeScript, exact-integration checks and independent critical review, then was invoked exactly once from integrated commit `9ea01e0215ae8c8cd0458283c62d1017b735b8b0`. It used a fresh patched Ergo checkout, created a 3,948,062-byte authority-safe spec and a 2,567-byte canonical request, and entered V11. It then stopped on the untrusted worker phase hint `source target node identity validation`. Receipt and artifact outputs remained absent; one journal entry and one opaque calibration entry are terminal and uninspected; no Node, Frontier, Cargo, Rust or Java process survived. V142 is terminal and none of its identity, request, spec, ports or mutable roots may be reused. This result changes no proof, contract, transaction, authorization, funds authority or release claim. |
| FED-6 local validation map | **Invariant:** retained bridge code may model and journal the exact setup lifecycle, but no local result, journal row, injected tool path, error message or injected port may expose a reusable signer or transport capability or become funds authority. V7 freezes the dual-node read-only target and same-target JVM check; V8 persists and revalidates the exact reservation across a real file-backed close/reopen boundary; V9 creates one single-use transport authorization and durable attempt, then persists an exact accepted or ambiguous V1 outcome before confirmation. V10 preserves those boundaries while starting the primary miner before the sole POST and requiring the same primary/witness process identities through canonical confirmation. V11 preserves all historical bytes and adds a separately digest-bound process-local response classification; a successful HTTP response is still not canonical admission. **Frozen inputs:** exact integrated bridge commit `9ea01e0215ae8c8cd0458283c62d1017b735b8b0`, source, patch, toolchain, build-report, base-spec and runtime identities; V142 request/spec and all V112-V142 mutable campaign state are terminal or abandoned and ineligible for reuse, with V134 abandoned before request creation. **Reusable checks:** build-environment, V11 receipt, WP-08A dependency, WP-07 containment, exact source/patch, setup/check, dual-layout, deadline-composition and offline application-runner matrices remain reusable while their transitive inputs stay unchanged. **Changed-input closure:** only the four-way node-identity failure vocabulary, observation phase assignments, isolated negative fixtures and this checkpoint; the compatibility API, RPC batch and order, accepted bytes, proof formats, contracts, transaction bytes, process implementation and source build remain unchanged. **Next due execution:** after exact source promotion and exact-integration hosted checks, one fresh V143 lineage must either project one finer finite node-identity failure phase or reach canonical tracker confirmation. Official target approval, authenticated route-retirement evidence, source-consensus authentication, global replay import, peg-out, recovery and activation remain unvalidated. |
| Following boundary | **FED-6-LAB V143 fresh node-identity-classified application-checkpoint campaign.** Never retry V142 or consume any earlier mutable state. After the four-way node-identity classifier is committed, published, integrated and green on the exact integrated commit, create one wholly fresh request, ports, processes, storage and authorization lineage. Run the complete owned application-checkpoint path once and require either canonical tracker confirmation or one terminal finite source phase without emitting arbitrary diagnostics. |
| Next implementation action | **Close and promote the four-way node-identity classifier, then run V143 once.** Run the focused observation/acceptance and direct-consumer matrices, TypeScript, architecture check and independent review; stage only the source-failure registry, observation module, their direct tests and this plan checkpoint; run staged and exact-range publication guards; create a new guarded feature-branch pull request; and require exact-head plus exact-integration hosted checks. Then create the wholly fresh local-only V143 lineage. If V143 canonically confirms the exact tracker successor, move directly to burn/checkpoint binding, global replay insertion, external-fee payout and profile-specific recovery. Public networks, existing wallet material, real funds, Gate 5, trustless status and readiness remain outside this LAB action. |
...
```text
frozen foundations
  -> canonical tracker admission (V142 reached node-identity validation; promote the four finite node-identity subphases, then use one wholly fresh bounded V143 campaign)
...
|---|---|---|---|
| V142 source/build/request inputs and locked tools | Exact bridge/source root, patch, command, tool, cache-source, build-report, base-spec, runtime and canonical request identities | Source relocation or any source, patch, command, toolchain, cache-source, generated-byte or request drift | Reuse immutable pins for focused root-cause falsification; rebuild only on drift and never reuse V142 mutable campaign state or identity |
...
| Mutable local campaign evidence | One exact fresh process and storage lineage | Terminal attempt, process replacement, request drift or target-state change | V112-V133 and V135-V142 are terminal; V134 was abandoned. Create V143 only after the four-way node-identity phase projection is promoted and integrated with exact hosted closure, and never reuse an earlier attempt as authority |
...
exact packet-to-mint-to-application-burn-to-checkpoint join. At that campaign
checkpoint, the next slice was construction of the tracker candidate consuming
that exact checkpoint, not replay or payout work.
...
source baseline revalidated. This closed only the FED-6-LAB application-
checkpoint join; tracker admission had not begun at that checkpoint.

**Checkpoint-bound tracker candidate implementation checkpoint:** the V4
composition root retains the exact V3 application checkpoint and the setup
family's already authenticated tracker compiler request/receipt without adding
a compiler capability to the execution root. It selects the single ordinal-0
tracker issuance and its retained canonical confirmation, waits for the same
inclusion to remain visible strictly after the checkpoint's admission start,
rematerializes the exact setup transaction output, and passes that real EIP-12
box plus the exact checkpoint statement to the compiler-bound tracker builder.
The builder independently requires the compiled ErgoTree, singleton NFT,
value, dense R4-R9 genesis registers and creation-height policy before it emits
  one deterministic proofless transaction. The composed launch closure carries
  the exact production setup execution batch, its live JVM tracker receipt and
  its rematerialized tracker output into the real compiler-bound builder; the
  campaign-root closure separately proves that the root forwards those same
  request, receipt, output and statement identities. A copied compiler receipt
  and isolated mutations of value, ErgoTree, token shape, register cardinality,
  every R4-R9 value, lower, unsafe and upper creation-height bounds, statement
  identity, canonical confirmation, construction boundaries and nested authority
  claims all reject while owned capabilities are torn down. The grouped
  architecture, tracker, campaign-root and launch closure passes 289/289 tests under the compiler-locked
Node 24.14.0. The resulting header context remains
synthetic: no mined `0x0401` anchor, JVM reduction, target-node check, signing,
submission, tracker admission, replay insertion, payout, funds authority, Gate
5, trustless status or readiness follows. The next executable boundary is the
real mined-anchor and no-submit tracker-check join, not replay or payout work.

**Checkpoint-anchor implementation checkpoint:** the V5 composition root
encodes the exact application checkpoint as
`bridge_event_root || statementId`, consumes a separate one-shot mining
credential, restarts both owned nodes over their existing data directories,
and mines the exact 64-byte value under extension key `0x0401`. It then stops
both miners before observation and restarts the same pair in non-mining mode.
The observer requires the exact prior process header in a bounded contiguous
header window, exact primary/witness genesis and canonical-header agreement, one exact
`0x0401` field, and independent recomputation of the extension Merkle root and
membership proof. The process result, observation and V5 receipt reject
isolated drift in the prior or mined snapshot, anchor height or header,
checkpoint binding, genesis, extension field, process provenance, credential
ownership and nested authority claims. The exact locked patched-node build and
the conditional real-process lifecycle test both pass. This closes local
checkpoint-to-Ergo-anchor provenance only. It does not authenticate Ergo proof
of work or source finality, replace the V4 synthetic tracker context, execute
the tracker in the JVM, call target-node acceptance, sign, submit, admit the
tracker, insert global replay state, release funds, close Gate 5, or establish
trustless status or readiness. The next executable boundary is the exact
real-anchor tracker candidate plus JVM and same-target no-submit check join.

**Observed-anchor tracker-check runtime checkpoint:** fresh create-only campaign
V6e executed against exact bridge commit
`709bb356912cb5445c4b4d6a006ce8dba0fcc33e` and canonical request SHA-256
`672f5cfbd479c0eb4567ad06c45cfcd3671603cf3ebc3115c84d8d3900460d44`.
The exact V6 receipt digest is
`627393a39a7bfe1eeac056b73d40957cc27cf2a9cd91642f6c5399ba961f599d`.
In one owned isolated-devnet lifetime, the root completed setup, committed-vault
consumption, source-bound mint, application burn, federated checkpoint, mined
`0x0401` anchor, observed-header tracker construction, local WASM signing,
pinned JVM reduction and same-target `/transactions/check`. The anchor was at
height `173`, the tracker context used current height `174`, the proofless
transaction was 3,926 bytes, the signed transaction was 3,982 bytes, and its
transaction ID remained
`097379dd885cacb8630a39c982359047070adb2ccccf994492588d59428eec6d`.
The receipt is canonical, its domain-separated digest recomputes exactly, every
ephemeral capability was disposed, and all six owned listeners were absent
after completion. Signed bytes were not persisted. Tracker submission,
broadcast, admission, replay insertion, payout, source deterministic finality,
Ergo proof-of-work authentication, funds authority, Gate 5, trustless status
and production readiness remain false. V7 subsequently closed the moving-target
ambiguity without changing those authority boundaries.

**Frozen observed-anchor tracker-check runtime checkpoint:** fresh campaign
V7b executed against exact bridge commit
`46aef79bd8101bf4a3e2ba5ce7645f3876d21540` and canonical request SHA-256
`22635d33edf3c2255b2d353e25125cdebba67d9b40229cea71b1e0906c39525e`.
The exact root receipt digest is
`298daaaf3724a04523490061ce4d4aebb9a6cbcab18dc12be80f47773c0b8d05`.
The campaign repeated the full setup-to-check join, stopped mining before the
tracker action, kept both nodes read-only, and required the exact start and end
snapshots to remain equal while revalidating the retained checkpoint and
`0x0401` membership. The same frozen target accepted the locally signed
candidate through `/transactions/check`. All monitored listeners were closed
after completion. Signed bytes were not persisted, and tracker submission,
broadcast, admission, replay insertion, payout, source deterministic finality,
Ergo proof-of-work authentication, funds authority, Gate 5, trustless status
and production readiness remain false.

The exact process-proven V7 root now authorizes one capability-free reservation
identity after revalidating its complete source, checkpoint, anchor, target,
candidate and JVM-check bindings. The dedicated SQLite adapter stores that
shape immutably through its API and detects uncoordinated binding drift, exact
retries and logical conflicts. Its digest is a local consistency check, not
external authentication. A narrow consumer now requires the exact in-process
authorization and a result sealed by a constructor-proven writable file-backed
`StateTracker` store. The hidden store identity binds the immutable local
continuity identity and canonical location, so in-memory stores, bypassed
constructors, database replacement and cross-store fan-out reject while an
exact reopen/retry remains stable. The consumer also recomputes the durable
digest and emits a process-proven reservation receipt with every execution
boundary still false. Plain storage calls do not create that progression
receipt. V8 closes the fresh reservation, tracker-input, anchor and target
revalidation join across a real file-backed close/reopen boundary. V9 then
adds one single-use authorization, durable attempt and fixed owned-loopback
transport without exposing a reusable signer or submitter.

Campaign V9k, bound to request SHA-256
`e53a9b89cec6cab194164bf5d14aa96bb949e488e5e0fd2d13a4cd7c4aa89db1`,
persisted exactly one transport attempt and its terminal outcome but did not
establish canonical tracker confirmation before the worker terminated. The
campaign is closed and must not be retried automatically. Its historical
accepted-versus-ambiguous outcome is not evidence because private runtime state
is deliberately excluded. Commit `ca724de3` preserves future failures through a
process-proven, data-safe root receipt and canonical worker parser while keeping
tracker admission false. Commit `387fb435` makes that failure decision-complete
without transport by adding a versioned diagnostic bound to the exact
transaction, execution target and confirmation budget. Its bounded categories
cover deadline source, pending/not-found/late-confirmed observation, observer
failure, monotonic-clock failure and a fail-closed unclassified phase failure.
They do not claim rollback or replacement without a later chain observation.
The next implementation boundary is one completely fresh V9 request and owned
runtime lineage, with a no-transport preflight followed by at most one newly
authorized loopback transport attempt. Any failure is terminal for that fresh
campaign. No global replay insertion or payout work precedes a canonically
confirmed tracker admission.

Fresh campaign V20 was bound to request SHA-256
`f49a4bc2d8472d4d5447030f558967599ba326b8ec2fc641d8399b7668a25be2`.
Its exact patched Frontier and Ergo source baselines passed, the Frontier
application runner accepted the source as a strict descendant of its temporary
root, and every create-only destination was empty before launch. The bounded
worker later terminated during worker-receipt validation without producing a
canonical command receipt or a typed transport-failure receipt. No artifact
root was published, and all matching processes and listeners were absent after
teardown. V20 is terminal and its private journal is neither inspected nor
promoted.

Commit `73fa570d` makes a future opaque pre-transport failure actionable without
making it authoritative. The worker can emit only an allowlisted phase or
producer-to-consumer binding label; the parent accepts only one exactly framed
line and exposes it as an explicitly untrusted hint. Unknown, multiline or
otherwise malformed diagnostics remain opaque. Hints are thrown before command
receipt construction and create-only publication, and they cannot select a
retry or establish transport, admission or funds authority. The next runtime
lineage must use a new request and fresh source/destination roots. It receives at
most one attempt after the same source, topology and empty-state preflights; a
...
Fresh lineages V25-V29 are terminal. Their failed routes bounded path/toolchain
remapping, a stable junction alias, fixed Wasm metadata and every retained
exact-lock checkout tested. V29's same-checkout packet remains valid historical
evidence, but fresh-source rebuilds did not reproduce its chain-spec/runtime
bytes. V29 therefore cannot seed a campaign from another checkout, and no V29
request, generated chain spec or runtime state may be reused.
...
`d3028c94f7c1bf73660008c1cad13749e41a16c1bd1a5d60d0a8968378fc83ad`
recomputes exactly with the repository implementation. The two native binary
...
runner consumes only the process-proven mint receipt, passes its exact compiled
profile, reservation statement and source-proof envelope to the Rust
TestClient, commits that environment into the runner input digest, requires the
...
checkpoint`. That V3 checkpoint still established no tracker admission; the V4
candidate described above now consumes it without gaining authority. No source consensus,
...
| Exact canonical Frontier patch, application-burn overlay, named Rust TestClient producer, same-process runner and pure transcript consumer | The runner requires an external canonical scratch checkout, revalidates exact patch and tool identities, applies the overlay only for one bounded offline Cargo execution, consumes producer stdout directly, reverses the overlay and revalidates the baseline before returning. The Rust test executes the reviewed mint-to-burn application path, decodes the actual PegOut log, checks the runtime leaf/root and supply/fee conservation; the pure consumer independently rebuilds the proof-relevant result and rejects isolated drift. This closes local application-burn producer provenance only. Source finality, checkpoint authority, tracker admission, global replay insertion, payout and Gate 5 remain false | Re-run when either patch, runner/process boundary, deployed application identity, reservation statement, named Rust test, marker schema, ABI decoder, burn format, conservation constants, tool pin or consumer changes |
| Exact V2 packet, packet-bound V2 mint receipt, retained synthetic source-attestation session and V1 checkpoint receipt | The V3 continuation accepts only exact process-owned objects, revalidates packet, mint, signer and retained-session provenance immediately before checkpoint production, binds one target descriptor across all three receipts and enforces `packet -> mint -> checkpoint`; a copied object, invalid checkpoint or disposal closes the capability. This proves local lifecycle ordering only. The application-burn receipt, source consensus/finality, Ergo anchor, tracker, replay and payout remain unbound | Re-run when the packet or mint receipt schema, retained-session lifecycle, checkpoint statement/receipt, target descriptor or V3 continuation changes |
| Exact V3 application checkpoint, setup-family tracker compiler binding, ordinal-0 tracker setup issuance and retained canonical confirmation | The V4 root waits for the same tracker inclusion beyond the admission start, rematerializes the exact setup output, and constructs one deterministic proofless tracker transaction from that box and statement. Copied compiler provenance, box/statement drift and post-construction confirmation replacement reject. The synthetic header proves only local construction; no anchor, check or admission authority follows | Re-run when the V3 checkpoint, setup issuance/confirmation shape, tracker compiler request/receipt, exact box materializer, tracker builder, admission window or V4 root changes |
| Exact V3 application checkpoint, V4 tracker candidate, process-owned second mining phase, exact prior node snapshot and two stable owned Ergo-node observations | The V5 root mines `bridge_event_root || statementId` under `0x0401` over the existing isolated-devnet data directories, stops mining, and requires exact prior-header ancestry, dual-node canonical agreement, one exact extension field, recomputed extension-root membership and unchanged process provenance. This proves local checkpoint-to-anchor provenance only. It does not authenticate Ergo proof of work or source finality, replace the V4 synthetic tracker context, perform JVM or node acceptance, admit the tracker, authorize funds, close Gate 5, or establish trustless status | Re-run when the V3 checkpoint, V4 candidate identity, `0x0401` encoding, second-mining lifecycle, prior-snapshot binding, observer, extension proof, process ownership or V5 root changes |
| Exact V5 observed anchor, canonical Autolykos V2 header bytes, confirmed tracker setup output, same-process compiler receipt, local WASM signer and same owned node checker | V6e consumes the observed `0x0401` anchor at the oldest context slot, constructs the exact tracker successor, signs the frozen transaction locally, and receives `PASS` from the same node's `/transactions/check`. The create-only receipt recomputes to `627393a3...f599d`, persists no signed bytes, tears down every capability and listener, and keeps submission, broadcast, admission, replay, payout, funds, Gate 5, trustless and readiness boundaries false | Re-run when the V5 receipt, observed-header normalizer/parser, setup or compiler identity, signer/checker, node build, request digest, tracker transaction bytes or V6 root changes; do not rerun for unrelated work |
...
## GitHub Milestone Promotion Cadence

Every important completed milestone is promoted to the public GitHub review
surface. Publication means a guarded push of the exact green commit to the
active public branch and pull request, followed by exact-head hosted CI and a
short public status update. It does not imply merge, tag, release, activation,
deployment, funds authority, Gate 5 closure, trustless status, or production
readiness.

An important milestone is one that closes at least one named work-package
Definition of Done, fixes a critical/high funds-safety or consensus boundary,
freezes a versioned proof or transaction format, obtains exact target-node
acceptance, completes a reproducible lifecycle/recovery campaign, or closes an
independent-review blocker. Test-only commits, diagnostic instrumentation,
terminal campaign attempts without a reusable source change, and partial
refactors are not milestones and stay local.

| Milestone boundary | Required GitHub action | Promotion evidence |
|---|---|---|
| Coherent source or protocol milestone | Push the guarded exact commit to the active review branch and update its pull request | Applicable closeout checks, independent review when strict/critical, staged and range publication guards, exact-head hosted CI |
| Reproducible non-mainnet campaign milestone | Publish only sanitized source, fixtures and evidence that are intended for review; never publish runtime state or synthetic custody material | Exact source/request identity, target acceptance result, negative cases, recovery result and explicit residual blockers |
| Work-package completion | Update the canonical plan and public pull-request summary in the same promotion batch or an immediately following docs commit | Definition of Done mapping, files changed, reused validations and remaining nonclaims |
| Gate or claim boundary | Publish the evidence packet only after every deciding producer-to-consumer join and independent review is complete | Exact gate command, exact commit, immutable evidence links and explicit authority/claim limits |
| Release candidate, tag or merge to `main` | Require a separate explicit decision; branch publication alone never authorizes it | All applicable release gates, protected-branch review, exact-head CI and publication guard |

Promotion follows one reusable sequence: freeze the candidate commit; run only
the due validation closure; run the staged publication guard; commit; run the
range publication guard; push through the guarded wrapper; wait for all required
checks on that exact head; then record the public URL and achieved/open
boundaries. A failed hosted check is fixed in a new commit and never hidden by
rewriting published history. Security-sensitive reproductions remain in the
private disclosure path even when their remediation milestone is public.
