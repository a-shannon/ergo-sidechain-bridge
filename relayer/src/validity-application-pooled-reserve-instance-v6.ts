/**
 * Pure composition of the reviewed V6 compiler family into one settlement
...
import {
  type ValidityApplicationPooledReserveBurnFamilyV6CompilerRequest,
  type ValidityApplicationPooledReserveBurnFamilyV6Identity,
...
export const VALIDITY_APPLICATION_POOLED_RESERVE_INSTANCE_V6_SCHEMA =
  'e2s.validity-application-pooled-reserve-instance.v6' as const;

export interface BuildValidityApplicationPooledReserveInstanceV6Input {
...
export interface ValidityApplicationPooledReserveInstanceV6Candidate {
  readonly schema:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_INSTANCE_V6_SCHEMA;
  readonly version: 6;
  ...
  readonly contracts:
    ValidityApplicationPooledReserveBurnFamilyV6Identity['contracts'];
  readonly relations: {
    readonly exactV4SourceRuntimeRetained: true;
    readonly v6SettlementLineageIsDistinct: true;
...
export function buildValidityApplicationPooledReserveInstanceV6(
  input: BuildValidityApplicationPooledReserveInstanceV6Input,
): Readonly<ValidityApplicationPooledReserveInstanceV6Candidate> {
  assertExactKeys(input, [
    'compilerRequest',
    'compilerBatchJson',
  ], 'pooled-reserve V6 instance input');
  ...
  if (
    ...
    throw new Error(
      'pooled-reserve V6 target settlement lineage must differ from the V4 source runtime lineage',
    );
  }

  const candidate = deepFreeze({
    schema: VALIDITY_APPLICATION_POOLED_RESERVE_INSTANCE_V6_SCHEMA,
    version: 6 as const,
    ...
    relations: {
      ...
      v6SettlementLineageIsDistinct: true as const,
...
export function assertCompiledValidityApplicationPooledReserveInstanceV6Candidate(
  value: unknown,
...
> {
  if (value === null || typeof value !== 'object' || !candidates.has(value)) {
    throw new Error(
      'pooled-reserve V6 instance was not built from the reviewed compiler family in this process',
    );
  }
}
