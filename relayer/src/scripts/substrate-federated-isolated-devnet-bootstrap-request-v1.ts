...
export interface BootstrapCommandRequestV1 {
  readonly schema:
...
export interface SubstrateFederatedIsolatedDevnetBootstrapRequestRootInputV1 {
  readonly build:
...
export interface SubstrateFederatedIsolatedDevnetBootstrapRequestMaterialV1 {
  readonly input:
    Readonly<SubstrateFederatedIsolatedDevnetBootstrapRequestRootInputV1>;
  readonly requestBytes: Uint8Array;
}

export function loadCanonicalBootstrapRequestV1(
  requestPath: string,
  bridgeRoot: string,
  worktreeRoot: string,
): Readonly<SubstrateFederatedIsolatedDevnetBootstrapRequestRootInputV1> {
...
export function loadCanonicalBootstrapRequestMaterialBoundToSha256V1(
  requestPath: string,
  bridgeRoot: string,
  worktreeRoot: string,
  expectedRequestSha256Hex: string,
): Readonly<SubstrateFederatedIsolatedDevnetBootstrapRequestMaterialV1> {
  if (!/^[0-9a-f]{64}$/u.test(expectedRequestSha256Hex)) {
    throw new Error('bootstrap request digest is invalid');
  }
  return loadCanonicalBootstrapRequestInternal(
...
function loadCanonicalBootstrapRequestInternal(
  ...
  const input: SubstrateFederatedIsolatedDevnetBootstrapRequestRootInputV1 = {
    build: {
      worktreeRoot,
      bridgeRoot,
