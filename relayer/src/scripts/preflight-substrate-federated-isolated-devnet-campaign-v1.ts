...
export function preflightSubstrateFederatedIsolatedDevnetCampaignFromArgumentsV1(
  ...
  const relayerRoot = realpathSync(resolve(scriptDirectory, '..', '..'));
  const bridgeRoot = realpathSync(resolve(relayerRoot, '..'));
  const worktreeRoot = realpathSync(resolve(bridgeRoot, '..'));
