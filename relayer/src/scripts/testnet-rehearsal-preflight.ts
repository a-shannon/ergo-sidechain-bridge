...
import { preflightTestnetRehearsal } from '../testnet-rehearsal-preflight.js';
...
function usage(): void {
  console.error(
    'Usage: npm run rehearsal:preflight -- --prebroadcast <completed-testnet-prebroadcast-evidence.md> [--approvals <aggregate-approvals-v2.json>] [--json-out <report.json>]',
  );
}
...
if (args.help || !args.prebroadcast) {
  usage();
...
if (args.jsonOut) {
  ...
  if (output.errors.length > 0) {
    ...
    console.log(formatOfflineReportJsonWriteLine('preflight report', args.jsonOut));
  }
}
