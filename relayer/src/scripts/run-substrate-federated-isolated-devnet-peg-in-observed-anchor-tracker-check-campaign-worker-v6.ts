...
const WORKER_FAILURE_PREFIX =
  'isolated observed-anchor-tracker-check campaign worker failed';

export async function runSubstrateFederatedIsolatedDevnetPegInObservedAnchorTrackerCheckCampaignWorkerFromArgumentsV6(
...
>> {
  if (
    ...
    throw new Error(
      'isolated observed-anchor-tracker-check campaign worker arguments are invalid',
    );
  }
  if (process.platform !== 'win32') {
    throw new Error(
      'isolated observed-anchor-tracker-check campaign worker requires Windows',
    );
  }
  ...
  const { bridgeRoot, worktreeRoot } =
    resolveCanonicalObservedAnchorTrackerCheckCampaignWorkerRootsV6(scriptDirectory);
  ...
  return buildSubstrateFederatedIsolatedDevnetPegInObservedAnchorTrackerCheckCampaignWorkerReceiptV6(
    result.receipt,
    argv[3],
    pegIn,
  );
...
export function resolveCanonicalObservedAnchorTrackerCheckCampaignWorkerRootsV6(
...
export function formatSafeObservedAnchorTrackerCheckCampaignWorkerFailureV6(
  _error: unknown,
): string {
...
async function main(): Promise<void> {
  const receipt =
    await runSubstrateFederatedIsolatedDevnetPegInObservedAnchorTrackerCheckCampaignWorkerFromArgumentsV6(
...
if (invokedPath === import.meta.url) {
  main().catch(error => {
    process.stderr.write(
      formatSafeObservedAnchorTrackerCheckCampaignWorkerFailureV6(error),
    );
    process.exitCode = 1;
  });
}
