...
import {
  buildEip0045BridgeApplicationTrackerBindingRejectionContextV2,
...
if (finality.trustedAnchorDigestHex !== args.trustedAnchorDigest) {
  throw new Error(
    'the explicit trust-anchor digest does not match the rejection candidate',
  );
}
...
const fixture =
  await buildEip0045BridgeApplicationTrackerBindingRejectionContextV2({
    plan,
...
console.log(
  'Boundary: this is a synthetic contract-rejection fixture for an alternate '
  + 'source application. This command does not establish proof validity; the '
  + 'separate pinned runtime matrix must verify the receipt and confirm that '
  + 'the unchanged tracker rejects the profile. No activation, signing, node '
  + 'check, submission, broadcast, Gate 5 closure, or funds authority follows.',
);
...
function usage(): never {
  throw new Error(
    'Usage: npm run '
    + 'proof:validity-application-tracker:binding-rejection-fixture -- '
    + '--candidate-dir <absolute completed rejection candidate directory> '
