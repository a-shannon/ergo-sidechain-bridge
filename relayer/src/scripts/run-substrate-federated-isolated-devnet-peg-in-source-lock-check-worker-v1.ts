...
export async function runSubstrateFederatedIsolatedDevnetPegInSourceLockCheckWorkerFromArgumentsV1(
  argv: readonly string[],
): Promise<Readonly<SubstrateFederatedIsolatedDevnetPegInSourceLockWorkerReceiptV1>> {
  if (
    argv.length !== 8
    || argv[0] !== '--request'
    || argv[1] === undefined
    ...
    throw new Error('isolated peg-in source-lock worker arguments are invalid');
  }
  if (process.platform !== 'win32') {
    throw new Error('isolated peg-in source-lock worker requires Windows');
  }
