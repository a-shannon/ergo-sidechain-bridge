...
import {
  formatSafeTrackerTransportCampaignCommandFailureV11,
  parseSubstrateFederatedIsolatedDevnetPegInTrackerTransportCampaignCommandReceiptV11,
  runSubstrateFederatedIsolatedDevnetPegInTrackerTransportCampaignCommandFromArgumentsV11,
} from './run-substrate-federated-isolated-devnet-peg-in-tracker-transport-campaign-v11.js';
...
const ROOT_FAILURE_RECEIPT_DIGEST = '23'.repeat(32);
...
describe('isolated tracker transport campaign command V11', () => {
  let root: string;
  ...
  beforeEach(() => {
    ...
    root = mkdtempSync(join(tmpdir(), 'e2s-tracker-command-v11-'));
  ...
  it('publishes one request-bound canonical-confirmation receipt', async () => {
    ...
    expect(processInput.args[1]).toMatch(
      /run-substrate-federated-isolated-devnet-peg-in-tracker-transport-campaign-worker-v11\.ts$/u,
    );
    ...
    expect(receipt.version).toBe(11);
    expect(receipt.executionReceipt).toMatchObject({
      version: 11,
      rootResponseClassificationDigestHex:
        workerReceipt.responseClassification.classificationDigestHex,
      responseClassification: workerReceipt.responseClassification,
    });
  ...
  it('classifies checkout discovery separately from command arguments', async () => {
    ...
    expect(formatSafeTrackerTransportCampaignCommandFailureV11(failure)).toBe(
      'isolated peg-in tracker transport campaign V11 failed: command phase failed: repository layout\n',
    );
  ...
  it('reports the bounded preflight phase without leaking its cause', async () => {
    ...
    const safe = formatSafeTrackerTransportCampaignCommandFailureV11(failure);
    expect(safe).toBe(
      'isolated peg-in tracker transport campaign V11 failed: preflight phase failed: offline Frontier dependency closure\n',
    );
  ...
  it('rejects untyped worker diagnostics without publishing them', async () => {
    ...
    const safe = formatSafeTrackerTransportCampaignCommandFailureV11(failure);
    expect(safe).toBe(
      'isolated peg-in tracker transport campaign V11 failed: command phase failed: worker receipt\n',
    );
    expect(safe).not.toContain(privateDiagnostic);
    expect(() => readFileSync(outputPath, 'utf8')).toThrow();
  });
  ...
  it.each([
    ...
    async workerPhase => {
      ...
      expect(formatSafeTrackerTransportCampaignCommandFailureV11(failure)).toBe(
        `isolated peg-in tracker transport campaign V11 failed: untrusted worker phase hint: ${workerPhase}\n`,
      );
      expect(() => readFileSync(outputPath, 'utf8')).toThrow();
    },
  );
  ...
  it('relays only allowlisted worker bindings without publishing a receipt', async () => {
    mocked.process.mockRejectedValue(new BoundedProcessExitError({
      ...
      stdout: '',
      stderr:
        'isolated tracker transport campaign worker V11 failed: producer-to-consumer binding changed: tracker candidate input\n',
    }));
    ...
    expect(formatSafeTrackerTransportCampaignCommandFailureV11(failure)).toBe(
      'isolated peg-in tracker transport campaign V11 failed: untrusted worker binding hint: tracker candidate input\n',
    );
    expect(() => readFileSync(outputPath, 'utf8')).toThrow();
  });
  ...
  it.each([
    ...
    async checkedSubmissionFailureCode => {
      mocked.process.mockRejectedValue(new BoundedProcessExitError({
        ...
        stdout: '',
        stderr:
          `isolated tracker transport campaign worker V11 failed: checked submission failed: ${checkedSubmissionFailureCode}\n`,
      }));
      ...
      expect(formatSafeTrackerTransportCampaignCommandFailureV11(failure)).toBe(
        `isolated peg-in tracker transport campaign V11 failed: untrusted worker checked-submission failure hint: ${checkedSubmissionFailureCode}\n`,
      );
      expect(() => readFileSync(outputPath, 'utf8')).toThrow();
    },
  );
  ...
  it('rejects unknown or multiline worker phase diagnostics as opaque', async () => {
    for (const diagnostic of [
      'isolated tracker transport campaign worker V11 failed: phase failed: private phase\n',
      'isolated tracker transport campaign worker V11 failed: producer-to-consumer binding changed: private binding\n',
      'isolated tracker transport campaign worker V11 failed: checked submission failed: private_code\n',
      'private prelude\nisolated tracker transport campaign worker V11 failed: checked submission failed: authority_binding\n',
  ...
  it('fails closed on process loss without publishing or retrying', async () => {
    ...
    expect(mocked.process).toHaveBeenCalledOnce();
    expect(formatSafeTrackerTransportCampaignCommandFailureV11(failure)).toBe(
      'isolated peg-in tracker transport campaign V11 failed: command phase failed: worker launch\n',
    );
    expect(() => readFileSync(outputPath, 'utf8')).toThrow();
  });
  ...
  it('registers the explicit local tracker transport command', () => {
    ...
    expect(
      packageJson.scripts[
        'federated:isolated:peg-in-tracker-transport:execute-local:v11'
      ],
    ).toBe(
      'npm run node:guard && tsx src/scripts/run-substrate-federated-isolated-devnet-peg-in-tracker-transport-campaign-v11.ts',
    );
    expect(
      packageJson.scripts[
...
function successWorkerReceipt(requestSha256Hex: string) {
  const legacyV10Receipt = successWorkerReceiptV10(requestSha256Hex);
  ...
  const body = {
    ...
    rootResponseClassificationDigestHex:
      responseClassification.classificationDigestHex,
    ...
    checks: {
      ...
      exactResponseClassificationDigestValidated: true,
      exactRootResponseClassificationDigestCopied: true,
    ...
    boundaries: {
      ...
      responseClassificationProjectedFromSameProcessRuntimeProvenance: true,
...
function failureWorkerReceipt(requestSha256Hex: string) {
  const legacyV10Receipt = failureWorkerReceiptV10(requestSha256Hex);
  const responseClassification =
    createSubstrateFederatedIsolatedDevnetTrackerTransportResponseClassificationV1({
      status: 'ambiguous',
  ...
  const body = {
    ...
    requestSha256Hex,
    pegIn: pegIn(),
    rootFailureReceiptDigestHex: ROOT_FAILURE_RECEIPT_DIGEST,
    rootResponseClassificationDigestHex:
      responseClassification.classificationDigestHex,
    staticProjectionManifestDigestHex:
      SUBSTRATE_FEDERATED_ISOLATED_DEVNET_PEG_IN_TRACKER_TRANSPORT_CAMPAIGN_STATIC_PROJECTION_MANIFEST_DIGEST_V11,
...
function successWorkerReceiptV10(requestSha256Hex: string) {
  const body = {
    schema:
      SUBSTRATE_FEDERATED_ISOLATED_DEVNET_PEG_IN_TRACKER_TRANSPORT_CAMPAIGN_WORKER_RECEIPT_V10_SCHEMA,
    version: 10 as const,
  ...
  return {
    ...
    receiptDigestHex: sha256CanonicalJson(
      body,
      LEGACY_WORKER_RECEIPT_DIGEST_DOMAIN,
    ),
  };
}

function failureWorkerReceiptV10(requestSha256Hex: string) {
  const transport = {
    authorization: {
      expectedTransactionIdHex: EXPECTED_TRANSACTION_ID,
  ...
  return {
    ...
    receiptDigestHex: sha256CanonicalJson(
      body,
      LEGACY_WORKER_FAILURE_RECEIPT_DIGEST_DOMAIN,
    ),
  };
}
