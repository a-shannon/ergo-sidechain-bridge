...
const usage = [
  'Usage: npm run governance:prerequisite-map -- --candidate <committee-governance-evidence.md> --validator-commit <7-40 hex> --validator-report-out <report.md> --out <map.md> [--review-packet-out <packet.md>]',
  'Builds a Gate 6 committee governance prerequisite map from the current validator result.',
  'The command reads only the provided guarded Markdown evidence target and writes a validator report, prerequisite map, and optional external review packet inside the bridge repository.',
  'It does not read private deployment state, runtime databases, environment files, wallet material, perform node/RPC requests, rotate keys, sign, submit, deploy, broadcast, close Gate 6, or authorize release claims.',
];
...
const validation = read.errors.length === 0 ? validateCommitteeGovernanceEvidence(read.markdown) : undefined;
const command = buildCommand({ candidate, validatorCommit });
const validationReport = buildCommitteeGovernanceValidationReport({
  command: `npm run governance:validate -- ${candidate} --report-out <report.md>`,
  workingDirectory: 'ergo-sidechain-bridge/relayer',
  validatedTarget: candidate,
  readErrors: read.errors,
  validation,
});
const prerequisiteMap = buildCommitteeGovernancePrerequisiteMap({
  validatorCommit,
...
writeMarkdown(validatorReportOutput, formatCommitteeGovernanceValidationReportMarkdown(validationReport), '--validator-report-out');
writeMarkdown(mapOutput, formatCommitteeGovernancePrerequisiteMapMarkdown(prerequisiteMap), '--out');
if (reviewPacket && reviewPacketOutput) {
  writeMarkdown(
    reviewPacketOutput,
    formatCommitteeGovernanceReviewPacketMarkdown(reviewPacket),
    '--review-packet-out',
  );
}

console.log(`Committee governance validation report written: ${validatorReportOut}`);
console.log(`Committee governance prerequisite map written: ${out}`);
if (args.reviewPacketOut) console.log(`Committee governance review packet written: ${args.reviewPacketOut}`);
...
function buildCommand(options: {
  ...
  const parts = [
    'npm run governance:prerequisite-map --',
    '--candidate',
    options.candidate,
    '--validator-commit',
