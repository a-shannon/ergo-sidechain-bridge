...
export async function runSubstrateFederatedIsolatedDevnetPegInSourceLockExecutionWorkerFromArgumentsV1(
  argv: readonly string[],
): Promise<Readonly<SubstrateFederatedIsolatedDevnetPegInSourceLockExecutionWorkerReceiptV1>> {
  if (
    argv.length !== 8
    || argv[0] !== '--request'
    || argv[1] === undefined
    ...
    throw new Error('isolated source-lock execution worker arguments are invalid');
  }
  if (process.platform !== 'win32') {
    throw new Error('isolated source-lock execution worker requires Windows');
  }
