...
import {
  ...
  type ValidityApplicationPooledReserveBurnSettlementV6Packet,
...
VALIDITY_APPLICATION_POOLED_RESERVE_BURN_SETTLEMENT_V6_ACCEPTANCE_FIXTURE_SCHEMA =
  'e2s.validity-application-pooled-reserve-burn-settlement-jvm-fixture.v6' as const;

export interface ValidityApplicationPooledReserveBurnSettlementV6AcceptanceFixture {
  readonly schema:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_BURN_SETTLEMENT_V6_ACCEPTANCE_FIXTURE_SCHEMA;
  readonly version: 6;
...
let fixturePromise:
Promise<Readonly<
ValidityApplicationPooledReserveBurnSettlementV6AcceptanceFixture
>> | undefined;
...
>> {
  ...
  try {
    ...
    if (canonicalJson(roundTrip) !== canonicalJson(eip12UnsignedTransaction)) {
      throw new Error('WASM changed the exact V6 unsigned transaction');
    }
    ...
    if (unsignedTransactionIdHex !== packet.transaction.txId) {
      throw new Error('V6 unsigned transaction ID drifted');
    }
    unsignedId.free?.();
    ...
    if (
      ...
      throw new Error('proofless transaction bytes differ from V6 settlement');
    }
...
async function buildCompiledInstance() {
  ...
  return buildValidityApplicationPooledReserveInstanceV6({
    compilerRequest,
...
let genesisPromise: ReturnType<
  typeof buildValidityApplicationPooledReserveBurnFamilyV6CompilerFixtureInput
> | undefined;
...
Promise<BuildValidityApplicationPooledReserveBurnSettlementV6Input> {
  ...
  const claim: BuildValidityApplicationPooledReserveBurnSettlementV6Input[
  'claim'
  ] = {
    trackerIdentity: {
      sidechainHeight: SIDECHAIN_HEIGHT,
...
Promise<Readonly<ValidityApplicationPooledReserveBurnSettlementV6Packet>> {
  return buildValidityApplicationPooledReserveBurnSettlementV6(
...
async function buildSyntheticSettlementState(
  ...
  const trackerIssuance = await issueSingleton({
    label: 'pooled-reserve V6 synthetic tracker predecessor',
  ...
  const duplicatePreventionIssuance = await issueSingleton({
    label: 'pooled-reserve V6 synthetic DUP predecessor',
  ...
  const reserveIssuance = await issueSingleton({
    label: 'pooled-reserve V6 synthetic reserve predecessor',
...
async function buildExactFeeBox(source: Eip12Box): Promise<Eip12Box> {
  ...
  }, 'pooled-reserve V6 burn acceptance external fee');
  return transaction.outputs[0];
}

function canonicalUnsignedShape(
  packet: Readonly<ValidityApplicationPooledReserveBurnSettlementV6Packet>,
...
function serializeInputExtensions(
    ...
    for (let index = 0; index < inputs.len(); index += 1) {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            `V6 settlement input ${index} ContextExtension keys drifted`,
          );
        }
  ...
  if (output.length !== 3) {
    throw new Error('V6 settlement transaction must contain three inputs');
  }
