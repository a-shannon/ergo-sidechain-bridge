/**
 * Exact, non-executable target-check request for the distinct V6 setup.
...
export const VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_REQUEST_V6_SCHEMA =
  'e2s.validity-application-pooled-reserve-target-check-request.v6' as const;
export const VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_REQUEST_V6_STATUS =
  'blocked_non_authorizing_request' as const;
export const VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_RECEIPT_V6_SCHEMA =
  'e2s.validity-application-pooled-reserve-target-check-receipt.v6' as const;
...
const TRANSACTION_DIGEST_DOMAIN =
  'E2S_VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_REQUEST_V6_TRANSACTION';
...
export interface BuildValidityApplicationPooledReserveTargetCheckRequestV6Input {
...
export interface ValidityApplicationPooledReserveTargetCheckRequestV6Candidate {
  readonly schema:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_REQUEST_V6_SCHEMA;
  readonly version: 6;
  readonly status:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_REQUEST_V6_STATUS;
  ...
  readonly receiptPolicy: Readonly<{
    readonly requiredReceiptSchema:
      typeof VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_RECEIPT_V6_SCHEMA;
  ...
  readonly boundaries: Readonly<{
    ...
    readonly localPredicateClosureReplayedByRequest: false;
...
export function buildValidityApplicationPooledReserveTargetCheckRequestV6(
  input: BuildValidityApplicationPooledReserveTargetCheckRequestV6Input,
): Readonly<ValidityApplicationPooledReserveTargetCheckRequestV6Candidate> {
  assertExactKeys(input, [
    'cutoverEligibility',
    'provisioningPlan',
  ], 'pooled-reserve V6 target-check request input');
  assertValidityApplicationPooledReserveCutoverEligibilityV6Provenance(
  ...
  const transactions = deepFreeze([
    bindTransaction(
      ...
      input.cutoverEligibility.targetV6.transactionIdentities
        .trackerIssuanceTxIdHex,
      input.cutoverEligibility.targetV6.transactionIdentities.trackerBoxIdHex,
    ),
    bindTransaction(
      'duplicate-prevention-issuance',
  ...
  if (
    ...
    throw new Error('V6 target-check transactions and outputs must be pairwise distinct');
  }
  ...
  const binding = {
    schema:
      VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_REQUEST_V6_SCHEMA,
    version: 6 as const,
    status:
    ...
    source: {
      cutoverEligibilityCandidateDigestHex: fixedHex(
        ...
        32,
        'V6 cutover-eligibility candidate digest',
      ),
    ...
    target: {
      ...
      localPredicateClosure: deepFreeze({
    ...
    receiptPolicy: {
      requiredReceiptSchema:
        VALIDITY_APPLICATION_POOLED_RESERVE_TARGET_CHECK_RECEIPT_V6_SCHEMA,
    ...
    boundaries: {
      ...
      localPredicateClosureReplayedByRequest: false as const,
...
export function assertValidityApplicationPooledReserveTargetCheckRequestV6Provenance(
  value: unknown,
...
> {
  if (value === null || typeof value !== 'object' || !requests.has(value)) {
    throw new Error(
      'pooled-reserve V6 target-check request was not built in this process',
    );
  }
}
...
function assertEligibilityPlanBinding(
  ...
  const target = eligibility.targetV6;
  if (
    ...
    || target.finalityPolicy.approvedTrustAnchorDigestHex
      !== provisioning.profile.approvedTrustAnchorDigestHex
    || sha256CanonicalJson(target.localPredicateClosure)
      !== sha256CanonicalJson(provisioning.localPredicateClosure)
  ) {
    throw new Error('V6 target-check request does not bind the exact eligibility plan');
  }
  for (const role of [
    'tracker',
    'duplicatePrevention',
    ...
    if (
      ...
      throw new Error(`V6 target-check request ${role} contract binding drifted`);
    }
  }
}
...
function assertEligibilityRemainsBlocked(
  ...
  if (
    ...
    throw new Error('V6 cutover eligibility is no longer a blocked precondition');
  }
}
...
function assertProvisioningRemainsUnsigned(
  ...
  if (
    ...
    || provisioning.localPredicateClosure.replayedByThisPlan !== false
    || provisioning.localPredicateClosure.targetNodeAcceptanceEstablished
      !== false
    || Object.values(provisioning.boundaries).some(value => value !== false)
  ) {
    throw new Error('V6 provisioning is no longer an unsigned non-authorizing plan');
  }
}
