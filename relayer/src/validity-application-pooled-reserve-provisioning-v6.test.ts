...
import {
  ...
  type BuildValidityApplicationPooledReserveProvisioningV6Input,
...
const SOURCE_PATH = resolve(
  import.meta.dirname,
  'validity-application-pooled-reserve-provisioning-v6.ts',
);
...
beforeAll(async () => {
  ...
  compiled = buildValidityApplicationPooledReserveInstanceV6({
    compilerRequest,
...
describe('validity application pooled-reserve provisioning V6', () => {
  it('builds one deterministic, network-bound unsigned plan', async () => {
    const first = await buildValidityApplicationPooledReserveProvisioningV6(
      buildInput(),
    );
    const second = await buildValidityApplicationPooledReserveProvisioningV6(
      buildInput(),
    );

    expect(second).toEqual(first);
    expect(first.schema)
      .toBe(VALIDITY_APPLICATION_POOLED_RESERVE_PROVISIONING_V6_SCHEMA);
    expect(first.version).toBe(6);
    expect(first.localPredicateClosure).toEqual(
      VALIDITY_APPLICATION_POOLED_RESERVE_LOCAL_PREDICATE_CLOSURE_V6,
    );
    expect(first.localPredicateClosure).toMatchObject({
      prooflessTransactionIdHex:
        'e9511548e623756c917496e13f41fbfd65703deedd8a521288b082aedb800a83',
      prooflessTransactionBytes: 6_600,
      protectedInputAcceptanceCount: 2,
      isolatedNegativeCaseCount: 10,
      replayedByThisPlan: false,
    ...
    expect(Object.isFrozen(first.transactions.trackerIssuance.eip12Tx.inputs))
      .toBe(true);
    expect(() =>
      assertValidityApplicationPooledReserveProvisioningV6Provenance(first)
    ).not.toThrow();
    expect(() =>
      assertValidityApplicationPooledReserveProvisioningV6Provenance(
  ...
  it('rejects network, profile, provenance, and schema drift', async () => {
    ...
    for (const [label, replacement] of networkCases) {
      ...
      await expect(buildValidityApplicationPooledReserveProvisioningV6({
        ...input,
        targetNetwork,
    ...
    } as unknown as BuildValidityApplicationPooledReserveProvisioningV6Input))
  ...
  it('has an exact source-only static dependency closure', () => {
    ...
    expect(imports).toEqual([
      ...
      './validity-application-pooled-reserve-burn-settlement-v6.js',
      './validity-application-pooled-reserve-historical-replay-genesis-v4.js',
      './validity-application-pooled-reserve-instance-v6.js',
      './validity-application-pooled-reserve-replay-cutover-v6.js',
    ]);
...
function buildInput(): BuildValidityApplicationPooledReserveProvisioningV6Input {
