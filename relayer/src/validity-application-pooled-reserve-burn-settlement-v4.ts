/**
 * Pure V4 pooled-reserve burn-settlement planner.
 *
 * This module reconstructs authenticated tracker and DUP state from caller
 * supplied history, binds one canonical burn proof to exact V4 singleton
...
export const
VALIDITY_APPLICATION_POOLED_RESERVE_BURN_SETTLEMENT_V4_SCHEMA =
  'e2s.validity-application-pooled-reserve-burn-settlement.v4' as const;
export const VALIDITY_APPLICATION_POOLED_RESERVE_TRACKER_KEY_V4_DOMAIN =
  'E2S_SPV_VALIDITY_APPLICATION_KEY_V4' as const;
export const VALIDITY_APPLICATION_POOLED_RESERVE_TRACKER_VALUE_V4_DOMAIN =
  'E2S_SPV_VALIDITY_APPLICATION_VALUE_V4' as const;
export const VALIDITY_APPLICATION_POOLED_RESERVE_TRACKER_VALUE_V4_BYTES = 370;

const TRACKER_VALUE_VERSION = 4 as const;
...
const TRACKER_VALUE_BYTES =
  VALIDITY_APPLICATION_POOLED_RESERVE_TRACKER_VALUE_V4_BYTES;
...
export interface ValidityApplicationPooledReserveTrackerIdentityV4 {
  readonly sidechainIdHex: string;
  readonly sidechainHeight: string | number | bigint;
  readonly executionBlockHashHex: string;
}

export interface ValidityApplicationPooledReserveTrackerHistoryEntryV4 {
  readonly key: string;
  readonly value: string;
}

export interface ValidityApplicationPooledReserveTrackerValueV4Input {
...
export interface ValidityApplicationPooledReserveTrackerValueV4 {
  readonly version: 4;
...
export interface ValidityApplicationPooledReserveBurnClaimV4 {
  readonly trackerIdentity: Omit<
    ValidityApplicationPooledReserveTrackerIdentityV4,
    'sidechainIdHex'
  >;
...
export interface BuildValidityApplicationPooledReserveBurnSettlementV4Input {
  ...
  readonly claim: ValidityApplicationPooledReserveBurnClaimV4;
  readonly currentErgoHeight: number;
  readonly creationHeight: number;
  readonly feeNanoErg?: string | number | bigint;
}

export interface ValidityApplicationPooledReserveBurnSettlementV4Packet {
  readonly schema:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_BURN_SETTLEMENT_V4_SCHEMA;
  readonly version: 4;
  ...
  readonly tracker: {
    ...
    readonly decodedValue: ValidityApplicationPooledReserveTrackerValueV4;
  };
  readonly burn: {
  ...
  readonly invariants: {
    readonly exactV4TrackerEntryProved: true;
...
export function deriveValidityApplicationPooledReserveTrackerKeyV4Hex(
  input: ValidityApplicationPooledReserveTrackerIdentityV4,
): string {
  assertExactKeys(input, [
    'sidechainIdHex',
    'sidechainHeight',
    'executionBlockHashHex',
  ], 'pooled-reserve V4 tracker identity');
  return blake2b256Hex(Buffer.concat([
    Buffer.from(
      VALIDITY_APPLICATION_POOLED_RESERVE_TRACKER_KEY_V4_DOMAIN,
      'ascii',
    ),
...
export function encodeValidityApplicationPooledReserveTrackerValueV4Hex(
  input: ValidityApplicationPooledReserveTrackerValueV4Input,
): string {
  ...
  if (encoded.length !== TRACKER_VALUE_BYTES) {
    throw new Error('pooled-reserve V4 tracker value internal length mismatch');
  }
  return encoded.toString('hex');
}

export function decodeValidityApplicationPooledReserveTrackerValueV4(
  valueHex: string,
): ValidityApplicationPooledReserveTrackerValueV4 {
  const bytes = Buffer.from(fixedHex(
    valueHex,
    TRACKER_VALUE_BYTES,
    'pooled-reserve V4 tracker value',
  ), 'hex');
  ...
  if (!bytes.subarray(0, expectedDomain.length).equals(expectedDomain)) {
    throw new Error('pooled-reserve V4 tracker value domain mismatch');
  }
  ...
  if (offset !== bytes.length) {
    throw new Error('pooled-reserve V4 tracker value was not fully consumed');
  }
...
export function getValidityApplicationPooledReserveTrackerDigestV4Hex(
  ...
  if (normalized.length === 0) {
    return fixedHex(
      tracker_application_v2_empty_digest(),
      33,
      'empty pooled-reserve V4 tracker digest',
    );
  }
...
buildValidityApplicationPooledReserveBurnSettlementV4(
  input: BuildValidityApplicationPooledReserveBurnSettlementV4Input,
  ...
  assertExactKeys(input, [
    ...
    'currentErgoHeight',
    'creationHeight',
  ], 'pooled-reserve V4 burn-settlement input', ['feeNanoErg']);
  assertExactKeys(input.trackerState, [
    'dataInput',
    'history',
  ], 'pooled-reserve V4 tracker state');
  assertExactKeys(input.reserveState, [
    'predecessor',
  ], 'pooled-reserve V4 reserve state');
  assertExactKeys(input.duplicatePreventionState, [
    'predecessor',
    'historyKeys',
  ], 'pooled-reserve V4 duplicate-prevention state');
  ...
  requireEqual(
    fixedHex(
      ...
      32,
      'burn application lineage profile ID',
    ),
  ...
  if (trackerHistory.length === 0) {
    throw new Error('pooled-reserve V4 tracker history must not be empty');
  }
  ...
  const trackerKeyHex =
    deriveValidityApplicationPooledReserveTrackerKeyV4Hex(trackerIdentity);
  ...
  const [
    ...
    normalizeEip12Box(
      ...
      'pooled-reserve V4 tracker data input',
    ),
    normalizeEip12Box(
      snapshot.reserveState.predecessor,
      'pooled-reserve V4 reserve predecessor',
    ),
    normalizeEip12Box(
      snapshot.duplicatePreventionState.predecessor,
      'pooled-reserve V4 duplicate-prevention predecessor',
    ),
    normalizeEip12Box(
      snapshot.feeFundingInput,
      'pooled-reserve V4 fee funding input',
    ),
  ]);
  ...
  const recipientErgoTreeHex = await normalizeErgoTreeHex(
    ...
    'pooled-reserve V4 payout ErgoTree',
  );
  ...
  requireEqual(
    burnLeaf.assetIdHex,
    ZERO_ASSET_ID_HEX,
    'pooled-reserve V4 native ERG asset lane',
  );
  ...
  if (!proofEnvelope.ok) {
    throw new Error(
      `pooled-reserve V4 burn inclusion rejected: ${
  ...
  if (claim.leafCount !== trackerValue.burnLeafCount) {
    throw new Error(
      'pooled-reserve V4 leafCount must equal tracker burnLeafCount',
    );
  }
  ...
  if (!binding.ok) {
    throw new Error(
      `pooled-reserve V4 payout binding rejected: ${binding.errors.join('; ')}`,
    );
  }
  ...
  if (burnAmount < MIN_BOX_VALUE) {
    throw new Error(
      'pooled-reserve V4 burn amount is below the minimum payout box value',
    );
  }
  ...
  if (dupHistory.includes(burnLeaf.burnIdHex)) {
    throw new Error(
      'pooled-reserve V4 burn ID is already present in replay history',
    );
  }
  ...
  const contextExtension = Object.freeze({
  ...
  const transaction = await materializeUnsignedTransaction({
    inputs: [
      { ...reservePredecessor, extension: contextExtension },
      {
        ...duplicatePreventionPredecessor,
        extension: contextExtension,
      },
      { ...feeFundingInput, extension: {} },
    ],
  ...
  }, 'validity application pooled-reserve burn settlement V4');
  ...
  const result = deepFreeze({
    ...
    invariants: Object.freeze({
      exactV4TrackerEntryProved: true as const,
...
assertValidityApplicationPooledReserveBurnSettlementV4Packet(
  ...
  ValidityApplicationPooledReserveBurnSettlementV4Packet
> {
  if (value === null || typeof value !== 'object' || !packets.has(value)) {
    throw new Error(
      'pooled-reserve V4 burn-settlement packet was not built in this process',
    );
  }
}
...
function normalizeTrackerValueInput(
  input: ValidityApplicationPooledReserveTrackerValueV4Input,
): ValidityApplicationPooledReserveTrackerValueV4Input {
  assertExactKeys(input, [
    'bridgeEventRootHex',
    'checkpointCommitmentHex',
    'anchorHeaderIdHex',
...
function normalizeTrackerHistory(
  history: readonly ValidityApplicationPooledReserveTrackerHistoryEntryV4[],
): ValidityApplicationPooledReserveTrackerHistoryEntryV4[] {
  if (!Array.isArray(history)) {
    throw new Error('pooled-reserve V4 tracker history must be an array');
  }
  ...
  return history.map((entry, index) => {
    assertExactKeys(entry, [
      'key',
      'value',
    ], `pooled-reserve V4 tracker history entry ${index}`);
    const key = fixedHex(
      entry.key,
      TRACKER_KEY_BYTES,
      `pooled-reserve V4 tracker history entry ${index} key`,
    );
    if (seen.has(key)) {
      throw new Error('pooled-reserve V4 tracker history contains duplicate keys');
    }
    seen.add(key);
    const value = fixedHex(
      entry.value,
      TRACKER_VALUE_BYTES,
      `pooled-reserve V4 tracker history entry ${index} value`,
    );
    decodeValidityApplicationPooledReserveTrackerValueV4(value);
...
function trackerGetProof(
  ...
  if (!history.some(entry => entry.key === key)) {
    throw new Error(
      'pooled-reserve V4 tracker history does not contain the derived key',
    );
  }
  ...
  return Object.freeze({
    ...
    valueHex: fixedHex(
      raw.value_hex,
      TRACKER_VALUE_BYTES,
      'pooled-reserve V4 tracker proof value',
    ),
    getProofHex: variableHex(
      raw.get_proof_hex,
      'pooled-reserve V4 tracker get proof',
    ),
    digestHex: fixedHex(
      raw.digest_hex,
      33,
      'pooled-reserve V4 tracker digest',
    ),
  });
}

function assertClaimShape(
  claim: ValidityApplicationPooledReserveBurnClaimV4,
): void {
  assertExactKeys(claim, [
    ...
    'burnProof',
    'recipientErgoTreeHex',
  ], 'pooled-reserve V4 burn claim');
  assertExactKeys(claim.trackerIdentity, [
    'sidechainHeight',
    'executionBlockHashHex',
  ], 'pooled-reserve V4 burn tracker identity');
  assertExactKeys(claim.burnLeaf, [
    ...
    'amountNanoErg',
  ], 'pooled-reserve V4 burn leaf', ['assetIdHex']);
  if (!Array.isArray(claim.burnProof)) {
    throw new Error('pooled-reserve V4 burn proof must be an array');
  }
  claim.burnProof.forEach((step, index) => {
    assertExactKeys(step, [
      'side',
      'hashHex',
    ], `pooled-reserve V4 burn proof step ${index}`);
  });
}

function normalizeClaim(
  claim: ValidityApplicationPooledReserveBurnClaimV4,
): ValidityApplicationPooledReserveBurnClaimV4 {
  return Object.freeze({
    ...
    burnProof: Object.freeze(claim.burnProof.map((step, index) => {
      if (step.side !== 'left' && step.side !== 'right') {
        throw new Error(`pooled-reserve V4 burn proof step ${index} side is invalid`);
      }
      return Object.freeze({
        side: step.side,
        hashHex: fixedHex(
          step.hashHex,
          32,
          `pooled-reserve V4 burn proof step ${index} hash`,
        ),
      });
    })),
...
function assertTrackerDataInput(input: {
  ...
  if (
    ...
    throw new Error(
      'pooled-reserve V4 tracker data input is not the exact compiled singleton',
    );
  }
  if (
    ...
    throw new Error(
      'pooled-reserve V4 tracker latest height predates the claimed entry',
    );
  }
  if (stamp < 0 || stamp > input.currentErgoHeight) {
    throw new Error('pooled-reserve V4 tracker stamp is invalid or in the future');
  }
}

function validateTrackerValueBindings(input: {
  trackerValue: ValidityApplicationPooledReserveTrackerValueV4;
  compiled: Readonly<ValidityApplicationPooledReserveInstanceV4Candidate>;
  burnApplicationBinding: ReturnType<
    typeof decodePooledReserveBurnApplicationBindingV4
  >;
  claim: ValidityApplicationPooledReserveBurnClaimV4;
  currentErgoHeight: number;
}): void {
  ...
  for (const [label, actual, expected] of bindings) {
    requireEqual(actual, expected, `pooled-reserve V4 tracker ${label}`);
  }
  if (
    input.trackerValue.anchorHeaderHeight > input.currentErgoHeight
  ) {
    throw new Error('pooled-reserve V4 tracker anchor is in the future');
  }
  if (
    ...
    throw new Error(
      'pooled-reserve V4 tracker anchor lacks required confirmations',
    );
  }
  if (
    ...
    throw new Error(
      'pooled-reserve V4 tracker burn count differs from claim leafCount',
    );
  }
}
...
function assertReservePredecessor(input: {
  ...
  if (
    ...
    throw new Error(
      'pooled-reserve V4 reserve predecessor has wrong contract, NFT, profile, or AVL policy',
    );
  }
  const inputValue = positiveLong(
    input.reserve.value,
    'pooled-reserve V4 reserve value',
  );
  if (inputLiability < 0n || inputLiability > inputValue) {
    throw new Error('pooled-reserve V4 reserve liability is invalid');
  }
  if (
    ...
    throw new Error(
      'pooled-reserve V4 reserve value or liability is insufficient for burn',
    );
  }
  ...
  if (
    ...
    throw new Error(
      'pooled-reserve V4 reserve successor would violate conservation or minimum value',
    );
  }
  return {
    inputValue,
...
function normalizeDupHistory(historyKeys: readonly string[]): string[] {
  if (!Array.isArray(historyKeys)) {
    throw new Error('pooled-reserve V4 replay history must be an array');
  }
  const normalized = historyKeys.map((key, index) => fixedHex(
    key,
    32,
    `pooled-reserve V4 replay history key ${index}`,
  ));
  if (new Set(normalized).size !== normalized.length) {
    throw new Error('pooled-reserve V4 replay history contains duplicate keys');
  }
  return normalized;
}
...
function assertDuplicatePreventionPredecessor(input: {
  ...
  if (
    ...
    || input.duplicatePrevention.assets[0].tokenId !== fixedHex(
      ...
      32,
      'pooled-reserve V4 duplicate-prevention NFT',
    )
    ...
    throw new Error(
      'pooled-reserve V4 duplicate-prevention identity or history digest mismatch',
    );
  }
}
...
function assertPureFeeFunding(box: Eip12Box, fee: bigint): void {
  if (
    ...
    throw new Error(
      'pooled-reserve V4 fee funding must be exact pure ERG with no registers',
    );
  }
}
...
function assertExactMaterializedSettlement(input: {
  ...
  contextExtension: Readonly<Record<'0' | '1' | '2' | '3', string>>;
  ...
  const topologyOk =
    ...
    && exactRecord(tx.eip12Tx.inputs[0].extension, input.contextExtension)
    && exactRecord(tx.eip12Tx.inputs[1].extension, input.contextExtension)
    && Object.keys(tx.eip12Tx.inputs[2].extension).length === 0;
  if (!topologyOk) {
    throw new Error('pooled-reserve V4 burn-settlement topology drifted');
  }
  ...
  if (outputChecks.some(passed => !passed)) {
    throw new Error('pooled-reserve V4 burn-settlement outputs drifted');
  }
}
...
function normalizeCreationHeight(
  ...
  const normalized = positiveHeight(
    creationHeight,
    'pooled-reserve V4 successor creation height',
  );
  if (
    ...
    throw new Error(
      'pooled-reserve V4 successor creation height is outside the reviewed window',
    );
  }
  return normalized;
}
...
function assertCreationHeightAfterInputs(
  ...
  if (inputs.some(input => creationHeight < input.creationHeight)) {
    throw new Error(
      'pooled-reserve V4 successor creation height predates a protected input',
    );
  }
}
...
function minerFee(value: string | number | bigint): bigint {
  const fee = positiveLong(value, 'pooled-reserve V4 miner fee');
  if (fee < MIN_BOX_VALUE || fee > MAX_MINER_FEE) {
    throw new Error(
      'pooled-reserve V4 miner fee is outside the reviewed bound',
    );
  }
  return fee;
}
...
function assertDistinctBoxIds(boxes: readonly Eip12Box[]): void {
  if (new Set(boxes.map(box => box.boxId)).size !== boxes.length) {
    throw new Error(
      'pooled-reserve V4 tracker and transaction boxes must be distinct',
    );
  }
}
...
function assertObservedBoxesNotFuture(
  ...
  if (boxes.some(box => box.creationHeight > currentErgoHeight)) {
    throw new Error(
      'pooled-reserve V4 observed input creation height is in the future',
    );
  }
}
