...
export const VALIDITY_APPLICATION_POOLED_RESERVE_REPLAY_CUTOVER_V6_SCHEMA =
  'e2s.validity-application-pooled-reserve-replay-cutover.v6' as const;

const PACKET_DIGEST_DOMAIN =
  'E2S_VALIDITY_APPLICATION_POOLED_RESERVE_REPLAY_CUTOVER_V6';
...
export interface BuildValidityApplicationPooledReserveReplayCutoverV6Input {
...
export interface ValidityApplicationPooledReserveReplayCutoverV6Packet {
  readonly schema:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_REPLAY_CUTOVER_V6_SCHEMA;
  readonly version: 6;
  ...
  readonly invariants: {
    ...
    readonly targetV6LineageRebound: true;
    readonly replayDigestPreserved: true;
    readonly exactV6ContractAndSingletonBound: true;
...
export async function buildValidityApplicationPooledReserveReplayCutoverV6(
  input: BuildValidityApplicationPooledReserveReplayCutoverV6Input,
): Promise<Readonly<ValidityApplicationPooledReserveReplayCutoverV6Packet>> {
  assertExactKeys(input, [
    ...
    'creationHeight',
    'feeNanoErg',
  ], 'pooled-reserve V6 replay-cutover input', ['feeNanoErg']);
  assertCompiledValidityApplicationPooledReserveInstanceV6Candidate(
  ...
  if (
    sourceLineageProfileIdHex
      !== fixedHex(
        ...
        32,
        'compiled V6 source runtime lineage profile ID',
      )
  ) {
    throw new Error(
  ...
  const targetLineageProfileIdHex = fixedHex(
    ...
    32,
    'compiled V6 target lineage profile ID',
  );
  if (targetLineageProfileIdHex === sourceLineageProfileIdHex) {
    throw new Error(
      'pooled-reserve V6 replay cutover requires a distinct target settlement lineage',
    );
  }
  ...
  const nftIdHex = fixedHex(
    ...
    32,
    'compiled V6 duplicate-prevention NFT ID',
  );
  if (nftIdHex !== genesisInput.boxId) {
    throw new Error(
      'V6 duplicate-prevention NFT ID must equal its exact genesis input box ID',
    );
  }
  const propositionHex = variableHex(
    compiled.contracts.duplicatePrevention.receipt.propositionHex,
    'compiled V6 duplicate-prevention proposition',
  );
  ...
  const transaction = await materializeUnsignedTransaction({
    ...
    dataInputs: [],
    outputs,
  }, 'validity application pooled-reserve replay cutover V6');
  ...
  const targetLineage = Object.freeze({
    ...
    duplicatePreventionContractIdHex: fixedHex(
      ...
      32,
      'compiled V6 duplicate-prevention contract ID',
    ),
  });
  const binding = {
    schema: VALIDITY_APPLICATION_POOLED_RESERVE_REPLAY_CUTOVER_V6_SCHEMA,
    version: 6 as const,
    sourceReplay,
    targetLineage,
    registers,
    ...
    invariants: Object.freeze({
      ...
      targetV6LineageRebound: true as const,
      replayDigestPreserved: true as const,
      exactV6ContractAndSingletonBound: true as const,
...
export function assertValidityApplicationPooledReserveReplayCutoverV6Provenance(
  value: unknown,
): asserts value is Readonly<
  ValidityApplicationPooledReserveReplayCutoverV6Packet
> {
  if (value === null || typeof value !== 'object' || !packets.has(value)) {
    throw new Error(
      'pooled-reserve V6 replay-cutover packet was not built in this process',
    );
  }
}
...
async function normalizeGenesisInput(
  ...
  const box = await normalizeEip12Box(
    value,
    'V6 duplicate-prevention genesis input',
  );
  if (box.boxId !== fixedHex(
    expectedBoxIdHex,
    32,
    'compiled V6 duplicate-prevention genesis input box ID',
  )) {
    throw new Error(
      'V6 duplicate-prevention genesis input does not match the compiled lineage',
    );
  }
  if (box.assets.length !== 0) {
    throw new Error('V6 duplicate-prevention genesis input must be pure ERG');
  }
  if (Object.keys(box.additionalRegisters).length !== 0) {
    throw new Error(
      'V6 duplicate-prevention genesis input must be register-free',
    );
  }
  return box;
}
...
function appendChange(
  ...
  if (value < 0n) {
    throw new Error('V6 duplicate-prevention genesis input is underfunded');
  }
  if (value === 0n) return;
  if (value < MIN_BOX_VALUE) {
    throw new Error('V6 duplicate-prevention issuance change would be dust');
  }
  outputs.push({
    value,
    ergoTree: variableHex(ergoTree, 'V6 replay-cutover change ErgoTree'),
    assets: [],
    additionalRegisters: {},
    creationHeight,
  });
...
function assertExactSingletonOutput(input: {
  ...
  if (drift.length > 0) {
    throw new Error(
      `V6 duplicate-prevention singleton output drifted: ${drift.join(', ')}`,
    );
  }
}
...
function validCreationHeight(value: number, minimum: number): number {
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new Error('V6 duplicate-prevention creation height must be positive');
  }
  if (value < minimum) {
    throw new Error(
      'V6 duplicate-prevention issuance predates its genesis input',
    );
  }
  return value;
}
