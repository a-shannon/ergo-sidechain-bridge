...
import {
  ...
  type BuildValidityApplicationPooledReserveBurnSettlementV6Input,
...
let baseInput: BuildValidityApplicationPooledReserveBurnSettlementV6Input;
let compiled: Readonly<ValidityApplicationPooledReserveInstanceV6Candidate>;
let trackerHistory:
...
let claim: BuildValidityApplicationPooledReserveBurnSettlementV6Input['claim'];
...
describe('validity application pooled-reserve burn settlement V6', () => {
  it('separates the retained V4 source runtime from the V6 target lineage', () => {
  ...
  it('builds one deterministic unsigned V6 settlement transaction', async () => {
    const first =
      await buildValidityApplicationPooledReserveBurnSettlementV6(buildInput());
    const second =
      await buildValidityApplicationPooledReserveBurnSettlementV6(buildInput());

    expect(second).toEqual(first);
    assertValidityApplicationPooledReserveBurnSettlementV6Packet(first);
    ...
    expect(first.transaction.eip12Tx.inputs[0].extension)
      .toEqual(first.contextExtensions.reserve);
    expect(first.transaction.eip12Tx.inputs[1].extension)
      .toEqual(first.contextExtensions.duplicatePrevention);
    expect(first.transaction.eip12Tx.inputs[2].extension)
      .toEqual(first.contextExtensions.feeFunding);
    expect(first.contextExtensions.reserve).toEqual({});
    expect(Object.keys(first.contextExtensions.duplicatePrevention))
      .toEqual(['0', '1', '2', '3']);
    expect(first.contextExtensions.feeFunding).toEqual({});
  ...
  it('uses the reviewed miner fee and native-ERG lane defaults', async () => {
    ...
    const packet =
      await buildValidityApplicationPooledReserveBurnSettlementV6({
        ...withoutFee,
  ...
  it('rejects reserve profile, contract, and liability drift', async () => {
    ...
    }, 'wrong V6 reserve profile fixture');
    await expect(buildValidityApplicationPooledReserveBurnSettlementV6({
      ...buildInput(),
    ...
    }, 'insufficient V6 reserve liability fixture');
    await expect(buildValidityApplicationPooledReserveBurnSettlementV6({
      ...buildInput(),
    ...
    }, 'wrong V6 reserve contract fixture');
    await expect(buildValidityApplicationPooledReserveBurnSettlementV6({
      ...buildInput(),
  ...
  it('rejects unknown fields and forged process-owned candidates', async () => {
    await expect(buildValidityApplicationPooledReserveBurnSettlementV6({
      ...buildInput(),
      verified: true,
    } as unknown as BuildValidityApplicationPooledReserveBurnSettlementV6Input))
  ...
  it('snapshots mutable inputs and freezes the completed packet', async () => {
    ...
    const building =
      buildValidityApplicationPooledReserveBurnSettlementV6(input);
    ...
    expect(Object.isFrozen(packet.contextExtensions.reserve)).toBe(true);
  ...
  it('rejects forged packet objects outside the process-owned builder', () => {
    expect(() =>
      assertValidityApplicationPooledReserveBurnSettlementV6Packet({
        schema: 'e2s.validity-application-pooled-reserve-burn-settlement.v6',
        version: 6,
      }),
...
function buildInput(): BuildValidityApplicationPooledReserveBurnSettlementV6Input {
  return {
...
): Promise<BuildValidityApplicationPooledReserveBurnSettlementV6Input[
  'trackerState'
]> {
  const history = [{
