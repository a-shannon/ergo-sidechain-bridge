...
import {
  ...
  type BuildValidityApplicationPooledReserveReplayCutoverV5Input,
} from './validity-application-pooled-reserve-replay-cutover-v5.js';

const INSERT_ONLY_AVL_FLAGS = 0x01;

describe('pooled-reserve V5 global replay cutover', () => {
  it('rebinds the exact global V4 replay state into the V5 DUP singleton', async () => {
    const input = await fixture();
    const packet = await buildValidityApplicationPooledReserveReplayCutoverV5(
      input,
    );
    ...
    expect(Object.values(packet.boundaries).every(value => value === false))
      .toBe(true);
    expect(() =>
      assertValidityApplicationPooledReserveReplayCutoverV5Provenance(packet)
    ).not.toThrow();
    expect(() =>
      assertValidityApplicationPooledReserveReplayCutoverV5Provenance(
  ...
  it('feeds the imported replay state into the local V5 settlement constructor', async () => {
    ...
    const settlement = await buildValidityApplicationPooledReserveBurnSettlementV5({
      ...settlementInput,
  ...
  it('rejects source-lineage, replay-state, provenance and genesis drift', async () => {
    ...
    await expect(buildValidityApplicationPooledReserveReplayCutoverV5({
      ...base,
...
): Promise<BuildValidityApplicationPooledReserveReplayCutoverV5Input> {
  const settlement =
    await buildValidityApplicationPooledReserveBurnSettlementV5FixtureInput();
