/**
 * Pure V5 pooled-reserve burn-settlement planner.
 *
 * This module reconstructs authenticated tracker and DUP state from caller
 * supplied history, binds one canonical burn proof to exact V5 singleton
...
export const
VALIDITY_APPLICATION_POOLED_RESERVE_BURN_SETTLEMENT_V5_SCHEMA =
  'e2s.validity-application-pooled-reserve-burn-settlement.v5' as const;
...
export interface ValidityApplicationPooledReserveBurnClaimV5 {
...
export interface BuildValidityApplicationPooledReserveBurnSettlementV5Input {
  ...
  readonly claim: ValidityApplicationPooledReserveBurnClaimV5;
  readonly currentErgoHeight: number;
  readonly creationHeight: number;
  readonly feeNanoErg?: string | number | bigint;
}

export interface ValidityApplicationPooledReserveBurnSettlementV5Packet {
  readonly schema:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_BURN_SETTLEMENT_V5_SCHEMA;
  readonly version: 5;
...
buildValidityApplicationPooledReserveBurnSettlementV5(
  input: BuildValidityApplicationPooledReserveBurnSettlementV5Input,
  ...
  assertExactKeys(input, [
    ...
    'currentErgoHeight',
    'creationHeight',
  ], 'pooled-reserve V5 burn-settlement input', ['feeNanoErg']);
  ...
  assertExactKeys(input.reserveState, [
    'predecessor',
  ], 'pooled-reserve V5 reserve state');
  assertExactKeys(input.duplicatePreventionState, [
    'predecessor',
    'historyKeys',
  ], 'pooled-reserve V5 duplicate-prevention state');
  assertClaimShape(input.claim);

  const compiled = input.compiledInstance;
  assertCompiledValidityApplicationPooledReserveInstanceV5Candidate(compiled);
  ...
  const [
    ...
    normalizeEip12Box(
      ...
      'pooled-reserve V5 reserve predecessor',
    ),
    normalizeEip12Box(
      snapshot.duplicatePreventionState.predecessor,
      'pooled-reserve V5 duplicate-prevention predecessor',
    ),
    normalizeEip12Box(
      snapshot.feeFundingInput,
      'pooled-reserve V5 fee funding input',
    ),
  ]);
  ...
  const recipientErgoTreeHex = await normalizeErgoTreeHex(
    ...
    'pooled-reserve V5 payout ErgoTree',
  );
  ...
  requireEqual(
    burnLeaf.assetIdHex,
    ZERO_ASSET_ID_HEX,
    'pooled-reserve V5 native ERG asset lane',
  );
  ...
  if (!proofEnvelope.ok) {
    throw new Error(
      `pooled-reserve V5 burn inclusion rejected: ${
  ...
  if (claim.leafCount !== trackerValue.burnLeafCount) {
    throw new Error(
      'pooled-reserve V5 leafCount must equal tracker burnLeafCount',
    );
  }
  ...
  if (!binding.ok) {
    throw new Error(
      `pooled-reserve V5 payout binding rejected: ${binding.errors.join('; ')}`,
    );
  }
  ...
  if (burnAmount < MIN_BOX_VALUE) {
    throw new Error(
      'pooled-reserve V5 burn amount is below the minimum payout box value',
    );
  }
  ...
  if (dupHistory.includes(burnLeaf.burnIdHex)) {
    throw new Error(
      'pooled-reserve V5 burn ID is already present in replay history',
    );
  }
  ...
  }, 'validity application pooled-reserve burn settlement V5');
...
assertValidityApplicationPooledReserveBurnSettlementV5Packet(
  ...
  ValidityApplicationPooledReserveBurnSettlementV5Packet
> {
  if (value === null || typeof value !== 'object' || !packets.has(value)) {
    throw new Error(
      'pooled-reserve V5 burn-settlement packet was not built in this process',
    );
  }
}
...
function assertClaimShape(
  claim: ValidityApplicationPooledReserveBurnClaimV5,
): void {
  assertExactKeys(claim, [
    ...
    'burnProof',
    'recipientErgoTreeHex',
  ], 'pooled-reserve V5 burn claim');
  assertExactKeys(claim.trackerIdentity, [
    'sidechainHeight',
    'executionBlockHashHex',
  ], 'pooled-reserve V5 burn tracker identity');
  assertExactKeys(claim.burnLeaf, [
    ...
    'amountNanoErg',
  ], 'pooled-reserve V5 burn leaf', ['assetIdHex']);
  if (!Array.isArray(claim.burnProof)) {
    throw new Error('pooled-reserve V5 burn proof must be an array');
  }
  claim.burnProof.forEach((step, index) => {
    assertExactKeys(step, [
      'side',
      'hashHex',
    ], `pooled-reserve V5 burn proof step ${index}`);
  });
}

function normalizeClaim(
  claim: ValidityApplicationPooledReserveBurnClaimV5,
): ValidityApplicationPooledReserveBurnClaimV5 {
  return Object.freeze({
    ...
    burnProof: Object.freeze(claim.burnProof.map((step, index) => {
      if (step.side !== 'left' && step.side !== 'right') {
        throw new Error(`pooled-reserve V5 burn proof step ${index} side is invalid`);
      }
      return Object.freeze({
        side: step.side,
        hashHex: fixedHex(
          step.hashHex,
          32,
          `pooled-reserve V5 burn proof step ${index} hash`,
        ),
      });
    })),
...
function assertReservePredecessor(input: {
  ...
  if (
    ...
    throw new Error(
      'pooled-reserve V5 reserve predecessor has wrong contract, NFT, profile, or AVL policy',
    );
  }
  const inputValue = positiveLong(
    input.reserve.value,
    'pooled-reserve V5 reserve value',
  );
  if (inputLiability < 0n || inputLiability > inputValue) {
    throw new Error('pooled-reserve V5 reserve liability is invalid');
  }
  if (
    ...
    throw new Error(
      'pooled-reserve V5 reserve value or liability is insufficient for burn',
    );
  }
  ...
  if (
    ...
    throw new Error(
      'pooled-reserve V5 reserve successor would violate conservation or minimum value',
    );
  }
  return {
    inputValue,
...
function normalizeDupHistory(historyKeys: readonly string[]): string[] {
  if (!Array.isArray(historyKeys)) {
    throw new Error('pooled-reserve V5 replay history must be an array');
  }
  const normalized = historyKeys.map((key, index) => fixedHex(
    key,
    32,
    `pooled-reserve V5 replay history key ${index}`,
  ));
  if (new Set(normalized).size !== normalized.length) {
    throw new Error('pooled-reserve V5 replay history contains duplicate keys');
  }
  return normalized;
}
...
function assertDuplicatePreventionPredecessor(input: {
  ...
  if (
    ...
    || input.duplicatePrevention.assets[0].tokenId !== fixedHex(
      ...
      32,
      'pooled-reserve V5 duplicate-prevention NFT',
    )
    ...
    throw new Error(
      'pooled-reserve V5 duplicate-prevention identity or history digest mismatch',
    );
  }
}
...
function assertPureFeeFunding(box: Eip12Box, fee: bigint): void {
  if (
    ...
    throw new Error(
      'pooled-reserve V5 fee funding must be exact pure ERG with no registers',
    );
  }
}
...
function assertExactMaterializedSettlement(input: {
  ...
  if (!topologyOk) {
    throw new Error('pooled-reserve V5 burn-settlement topology drifted');
  }
  ...
  if (outputChecks.some(passed => !passed)) {
    throw new Error('pooled-reserve V5 burn-settlement outputs drifted');
  }
}
...
function normalizeCreationHeight(
  ...
  const normalized = positiveHeight(
    creationHeight,
    'pooled-reserve V5 successor creation height',
  );
  if (
    ...
    throw new Error(
      'pooled-reserve V5 successor creation height is outside the reviewed window',
    );
  }
  return normalized;
}
...
function assertCreationHeightAfterInputs(
  ...
  if (inputs.some(input => creationHeight < input.creationHeight)) {
    throw new Error(
      'pooled-reserve V5 successor creation height predates a protected input',
    );
  }
}
...
function minerFee(value: string | number | bigint): bigint {
  const fee = positiveLong(value, 'pooled-reserve V5 miner fee');
  if (fee < MIN_BOX_VALUE || fee > MAX_MINER_FEE) {
    throw new Error(
      'pooled-reserve V5 miner fee is outside the reviewed bound',
    );
  }
  return fee;
}
...
function assertDistinctBoxIds(boxes: readonly Eip12Box[]): void {
  if (new Set(boxes.map(box => box.boxId)).size !== boxes.length) {
    throw new Error(
      'pooled-reserve V5 tracker and transaction boxes must be distinct',
    );
  }
}
...
function assertObservedBoxesNotFuture(
  ...
  if (boxes.some(box => box.creationHeight > currentErgoHeight)) {
    throw new Error(
      'pooled-reserve V5 observed input creation height is in the future',
    );
  }
}
