/**
 * Pure, source-only provisioning plan for the distinct V6 settlement lineage.
...
import {
  ...
  type ValidityApplicationPooledReserveReplayCutoverV6Packet,
...
export const VALIDITY_APPLICATION_POOLED_RESERVE_PROVISIONING_V6_SCHEMA =
  'e2s.validity-application-pooled-reserve-provisioning.v6' as const;

export const VALIDITY_APPLICATION_POOLED_RESERVE_LOCAL_PREDICATE_CLOSURE_V6 =
  Object.freeze({
    schema:
      'e2s.validity-application-pooled-reserve-local-predicate-closure.v6' as const,
    sigmaStateCommit: 'f78deadd668f801e7fae3bc884283f79c6f484fa',
    acceptanceSpecSha256Hex:
      'c2b0479348d0aca0e752a0bd74b85bbec95e2b5bbc8bbc342bcee757e55a9bc3',
    acceptanceFixtureSha256Hex:
      'c0bf416f1b23efe2fb11f9759433e931aa70576ca96cd74255fcf8b3ed592fa9',
    prooflessTransactionIdHex:
      'e9511548e623756c917496e13f41fbfd65703deedd8a521288b082aedb800a83',
    prooflessTransactionBytes: 6_600 as const,
    protectedInputAcceptanceCount: 2 as const,
    isolatedNegativeCaseCount: 10 as const,
    replayedByThisPlan: false as const,
    targetNodeAcceptanceEstablished: false as const,
  });

const PLAN_DIGEST_DOMAIN =
  'E2S_VALIDITY_APPLICATION_POOLED_RESERVE_PROVISIONING_V6';
...
export interface ValidityApplicationPooledReserveProvisioningValuesV6 {
...
export interface ValidityApplicationPooledReserveProvisioningFeesV6 {
...
export interface ValidityApplicationPooledReserveProvisioningHeightsV6 {
  readonly trackerIssuance: number;
  readonly duplicatePreventionIssuance: number;
  readonly pooledReserveIssuance: number;
}

export interface BuildValidityApplicationPooledReserveProvisioningV6Input {
  readonly compiledInstance: Readonly<
    ValidityApplicationPooledReserveInstanceV6Candidate
  >;
  ...
  readonly values: ValidityApplicationPooledReserveProvisioningValuesV6;
  readonly fees?: ValidityApplicationPooledReserveProvisioningFeesV6;
  readonly creationHeights:
    ValidityApplicationPooledReserveProvisioningHeightsV6;
}

export interface ValidityApplicationPooledReserveProvisioningV6Plan {
  readonly schema:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_PROVISIONING_V6_SCHEMA;
  readonly version: 6;
  readonly planDigestHex: string;
  readonly localPredicateClosure:
    typeof VALIDITY_APPLICATION_POOLED_RESERVE_LOCAL_PREDICATE_CLOSURE_V6;
  ...
  readonly invariants: Readonly<{
    ...
    readonly exactCompiledV6ContractFamilyBound: true;
    ...
    readonly globalV4ReplayStateBoundIntoV6Plan: true;
...
export async function buildValidityApplicationPooledReserveProvisioningV6(
  input: BuildValidityApplicationPooledReserveProvisioningV6Input,
): Promise<Readonly<ValidityApplicationPooledReserveProvisioningV6Plan>> {
  assertExactKeys(input, [
    ...
    'fees',
    'creationHeights',
  ], 'pooled-reserve V6 provisioning input', ['fees']);
  assertCompiledValidityApplicationPooledReserveInstanceV6Candidate(
  ...
  if (input.fees !== undefined) {
    ...
    ], 'pooled-reserve V6 provisioning fees', [
      'trackerIssuanceNanoErg',
      'duplicatePreventionIssuanceNanoErg',
      'pooledReserveIssuanceNanoErg',
    ]);
  ...
  const profileRegister = encodeCollByteRegister(Buffer.from(
    fixedHex(compiled.lineageProfileIdHex, 32, 'V6 lineage profile ID'),
    'hex',
  ));
  const trackerRegisters = {
    ...
    R9: encodeCollByteRegister(Buffer.from(
      fixedHex(
        ...
        32,
        'V6 approved trust-anchor digest',
      ),
      'hex',
    )),
  };
  ...
  const [trackerIssuance, replayCutover, pooledReserveIssuance] =
    await Promise.all([
      buildSingletonIssuance({
        label: 'validity application pooled-reserve V6 tracker issuance',
      ...
      buildSingletonIssuance({
        label: 'validity application pooled-reserve V6 reserve issuance',
  ...
  const lineage = Object.freeze({
    ...
    duplicatePreventionNftIdHex: fixedHex(
      ...
      32,
      'V6 duplicate-prevention NFT ID',
    ),
  ...
  const binding = {
    ...
    invariants: Object.freeze({
      ...
      exactCompiledV6ContractFamilyBound: true as const,
      ...
      globalV4ReplayStateBoundIntoV6Plan: true as const,
...
export function assertValidityApplicationPooledReserveProvisioningV6Provenance(
  value: unknown,
...
> {
  if (value === null || typeof value !== 'object' || !packets.has(value)) {
    throw new Error(
      'pooled-reserve V6 provisioning plan was not built in this process',
    );
  }
}
...
function bindTargetNetwork(
...
): ValidityApplicationPooledReserveProvisioningV6Plan['targetNetwork'] {
  if (
    input.ergoNetworkId !== 'ergo-testnet'
    || input.ergoAddressNetworkPrefix !== 16
  ) {
    throw new Error(
      'V6 provisioning is restricted to the exact Ergo testnet network prefix',
    );
  }
  const sourceNetworkIdHex = fixedHex(
    input.sourceNetworkIdHex,
    32,
    'V6 source network ID',
  );
  ...
  if (
    ...
    throw new Error(
      'V6 target network does not match the compiled settlement profile',
    );
  }
...
): ValidityApplicationPooledReserveProvisioningV6Plan['contracts'] {
...
async function normalizeGenesis(
  ...
  const box = await normalizeEip12Box(input, `V6 ${label} genesis input`);
  if (box.assets.length !== 0) {
    throw new Error(`V6 ${label} genesis input must be pure ERG`);
  }
  if (Object.keys(box.additionalRegisters).length !== 0) {
    throw new Error(`V6 ${label} genesis input must be register-free`);
  }
  if (
    box.boxId
      !== fixedHex(expectedBoxIdHex, 32, `V6 ${label} genesis input box ID`)
  ) {
    throw new Error(
      `V6 ${label} genesis input does not match the compiled lineage`,
    );
  }
  return box;
}
...
function assertExactSingletonOutput(input: {
  ...
  if (drift.length > 0) {
    throw new Error(`V6 singleton output drifted: ${drift.join(', ')}`);
  }
}

function normalizeHeights(
  heights: ValidityApplicationPooledReserveProvisioningHeightsV6,
  ...
  for (const [label, actual, minimum] of checks) {
    if (actual < minimum) {
      throw new Error(`V6 ${label} issuance predates its genesis input`);
    }
  }
  return result;
}
...
function assertPairwiseDistinct(ids: readonly string[]): void {
  if (new Set(ids).size !== ids.length) {
    throw new Error(
      'V6 tracker, duplicate-prevention, and pooled-reserve genesis inputs must be pairwise distinct',
    );
  }
}
