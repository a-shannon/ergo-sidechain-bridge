...
export interface ResolvedEvidenceOutputPath {
  path?: string;
  errors: string[];
}

export interface EvidenceOutputPathOptions {
...
export function resolveEvidenceOutputPath(
  target: string,
  options: EvidenceOutputPathOptions = {},
): ResolvedEvidenceOutputPath {
  const trimmedTarget = target.trim();
  const errors = validateEvidenceOutputPath(target, options);
...
export function validateEvidenceOutputPath(
  target: string,
  options: EvidenceOutputPathOptions = {},
): string[] {
  const optionName = options.optionName ?? '--out';
  const workspaceRoot = resolve(options.workspaceRoot ?? process.cwd());
  const bridgeRoot = resolve(options.bridgeRoot ?? resolve(workspaceRoot, '..'));
  const trimmedTarget = target.trim();
  const normalized = normalizeEvidenceOutputTarget(target);
  const errors: string[] = [];

  if (trimmedTarget.length === 0) {
    errors.push(`${optionName} must not be empty`);
  }
  if (extname(basename(normalized)) !== '.md') {
    errors.push(`${optionName} must be a Markdown file`);
  }
  ...
  if (hasClaimEscalatingEvidenceOutputTarget(normalized)) {
...
function normalizeEvidenceOutputTarget(target: string): string {
  return target.trim().replace(/\\/g, '/').toLowerCase();
}

function hasClaimEscalatingEvidenceOutputTarget(normalized: string): boolean {
